...
const SHARP_MIN_MONEY_OVER_BETS_PCT: Record<FootballSport, number> = { NFL: 20, NCAAF: 40 };
...
function isSharpSource(play: Pick<TrendPlay, "betsPct" | "moneyPct">, sport: FootballSport) {
  ...
  return Number.isFinite(bets)
    ...
    && money - bets >= SHARP_MIN_MONEY_OVER_BETS_PCT[sport];
}
...
async function buildFootballPublicDataFresh(sport:FootballSport,{persist=false}:{persist?:boolean}={}){
  ...
  try {
    ...
    dk={
      splits,
      errors:[
        `Live DraftKings refresh failed: ${errorMessage}`,
        `Serving ${splits.length} retained DraftKings market sides from the last successful snapshots instead of failing the public NFL payload.`,
      ],
      filter:{
        eventGroup:"stored-snapshots",
        content:"stored-snapshots",
        dateRange:"current-week",
        label:"Stored DraftKings snapshots",
        source:"option",
      },
  ...
  const snapshotMap=new Map(snapshotExisting.map((row)=>[snapshotKey(row),row]));
