...
function trendEzpzPick(play: any): NflEzpzPick | null {
  const gapPct = Number(play.gapPct);
  if (!Number.isFinite(gapPct) || gapPct < 15) return null;
  ...
  return {
    ...
    score: Math.round(gapPct * 10) / 10,
    tier: "15%+ Gap Trend",
    qualification: `Handle − Bets gap +${gapPct.toFixed(1)}% • 15%+ Trend gate`,
    gapPct,
...
export async function buildFootballPublicData(
  ...
  const [propRows, propTracker] = await Promise.all([
