...
export function summarizeTracker(rows: SheetRow[]): TrackerSummaryRow[] {
  ...
  return [...groups.entries()].map(([betType, bets]) => {
    ...
    return {
      betType,
      status: wins > losses ? "WINNING" : wins === losses ? "EVEN" : "LOSING",
      wins,
      losses,
      pushes,
...
export function lastSevenDaysTotals(rows: SheetRow[], todayEt = todayEtString()) {
  const today = dateToUtcNoon(todayEt);
  const cutoff = new Date(today);
  cutoff.setUTCDate(cutoff.getUTCDate() - 6);
  return totalsFromRows("LAST 7 DAYS", rows.filter((row) => {
...
function isGreenPlayType(type: string, greenSet: Set<string>) {
  const normalized = normalizeBetTypeText(type);
  return greenSet.has(normalized) || GREEN_BET_TYPES.has(normalized);
}
...
export function buildBestPlays(slateRows: SheetRow[], greenSet: Set<string>): BestPlay[] {
  ...
  for (const row of slateRows) {
    ...
    if (mlGrade && mlGrade !== "NON-EDGE MONEYLINE" && row["Better ML"]) {
      plays.push({
        playType: mlGrade,
        game,
        play: row["Better ML"],
        oddsLine: row["ML Odds"] || "",
        score: toNumber(row["ML Grade"].match?.(/\d+(\.\d+)?/)?.[0]) || "Model edge",
        isGreen: isGreenPlayType(mlGrade, greenSet),
      });
    }

    const nrfiGrade = normalizeBetTypeText(row["NRFI Grade"]);
    if (["ELITE NRFI", "STRONG NRFI", "YRFI"].includes(nrfiGrade)) {
      plays.push({ playType: nrfiGrade, game, play: nrfiGrade.includes("YRFI") ? "YRFI" : "NRFI", oddsLine: "", score: "High confidence", isGreen: isGreenPlayType(nrfiGrade, greenSet) });
    }

    const awayK = String(row["Away Pitcher K + Grade"] ?? "").trim();
    const awayType = normalizeBetTypeText(awayK);
    if (awayK && awayType && awayType !== "PASS") {
      plays.push({ playType: awayType, game, play: awayK, oddsLine: "Pitcher Ks", score: getScore(row["Away Pitcher K Score"]), isGreen: isGreenPlayType(awayType, greenSet) });
    }

    const homeK = String(row["Home Pitcher K + Grade"] ?? "").trim();
    const homeType = normalizeBetTypeText(homeK);
    if (homeK && homeType && homeType !== "PASS") {
      plays.push({ playType: homeType, game, play: homeK, oddsLine: "Pitcher Ks", score: getScore(row["Home Pitcher K Score"]), isGreen: isGreenPlayType(homeType, greenSet) });
    }
  }
  return plays.filter((p) => p.isGreen).sort((a, b) => toNumber(b.score) - toNumber(a.score));
}
