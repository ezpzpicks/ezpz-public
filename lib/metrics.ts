...
export function summarizeTracker(rows: SheetRow[]): TrackerSummaryRow[] {
  ...
  return [...groups.entries()]
    .map(([betType, bets]) => {
      ...
      const unitsWon = bets.reduce(
        (sum, b) => sum + profitUnits(b["Odds/Line"], b.Result),
        0,
      );
      const status: TrackerSummaryRow["status"] =
        wins > losses ? "WINNING" : wins === losses ? "EVEN" : "LOSING";
      return {
        betType,
        status,
        wins,
...
export function trackerRowsLastSevenDays(
  rows: SheetRow[],
  todayEt = todayEtString(),
) {
...
export function summarizeTrackerLastSevenDays(
  rows: SheetRow[],
  todayEt = todayEtString(),
) {
...
function totalsFromRows(label: string, rows: SheetRow[]): RecordTotals {
  ...
  const decisions = wins + losses;
  const unitsWon = completed.reduce(
    (sum, b) => sum + profitUnits(b["Odds/Line"], b.Result),
    0,
  );
  return {
...
export function lastSevenDaysGreenTotals(
  rows: SheetRow[],
  last7Summary: TrackerSummaryRow[],
  todayEt = todayEtString(),
) {
  const winningTypes = new Set(
    last7Summary.filter((s) => s.wins > s.losses).map((s) => s.betType),
  );
  return totalsFromRows(
    "LAST 7 DAYS GREEN BETS",
    trackerRowsLastSevenDays(rows, todayEt).filter((row) => {
      return winningTypes.has(normalizeBetTypeText(row["Bet Type"]));
    }),
  );
}

export function overallGreenTotals(
  rows: SheetRow[],
  summary: TrackerSummaryRow[],
) {
  const winningTypes = new Set(
    summary.filter((s) => s.wins > s.losses).map((s) => s.betType),
  );
  const greenRows = rows.filter((row) =>
    winningTypes.has(normalizeBetTypeText(row["Bet Type"])),
  );
  return totalsFromRows("OVERALL GREEN BETS", greenRows);
}

export function pendingGreenCount(
  rows: SheetRow[],
  summary: TrackerSummaryRow[],
) {
  const winningTypes = new Set(
...
export function buildBestPlays(
  slateRows: SheetRow[],
  greenSet: Set<string>,
): BestPlay[] {
  ...
  for (const row of slateRows) {
    ...
    if (
      mlGrade &&
      mlGrade !== "NON-EDGE MONEYLINE" &&
      row["Better ML"] &&
      isGreenPlayType(mlGrade, greenSet)
    ) {
      plays.push({
        ...base,
        playType: mlGrade,
    ...
    if (
      ["ELITE NRFI", "STRONG NRFI", "YRFI"].includes(nrfiGrade) &&
      isGreenPlayType(nrfiGrade, greenSet)
    ) {
      plays.push({
        ...base,
        playType: nrfiGrade,
    ...
    if (
      ...
      awayType &&
      awayType !== "PASS" &&
      isGreenPlayType(awayType, greenSet)
    ) {
      plays.push({
        ...base,
        playType: awayType,
    ...
    if (
      ...
      homeType &&
      homeType !== "PASS" &&
      isGreenPlayType(homeType, greenSet)
    ) {
      plays.push({
        ...base,
        playType: homeType,
