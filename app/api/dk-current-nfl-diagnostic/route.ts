...
async function page(market:string,horizon:string,p:number){
 const u=new URL(URL_BASE); u.searchParams.set("tb_eg","NFL"); u.searchParams.set("tb_edate",horizon); u.searchParams.set("tb_emt",market); u.searchParams.set("tb_page",String(p));
 ...
 return {market,horizon,p,status:res.status,bytes:html.length,games,dateSelect:p===1?dateSelect:""};
}
export async function GET(){
 const out:any[]=[]; for(const horizon of ["today","n7days"]){for(const m of ["Spread","Total"]){for(let p=1;p<=10;p++){const r=await page(m,horizon,p); out.push(r); if(!r.games.length)break;}}}
