...
import { syncBetMgmFootballMarket } from "../../../../lib/footballBetMgmMarket";
...
async function runCron(request: NextRequest) {
  ...
  try {
    const result = await syncBetMgmFootballMarket(raw as FootballSport);
    return NextResponse.json(result, { headers: { "Cache-Control": "no-store, max-age=0" } });
  } catch (error) {
    console.error("BetMGM football snapshot failed", error);
    return NextResponse.json(
