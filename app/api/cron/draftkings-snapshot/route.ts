...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // GitHub wakes this endpoint several times per hour, but DraftKings is only
    // contacted when an uncaptured game reaches its single ~15-minute window.
    if (!targets.length) {
      return NextResponse.json({
        ok: true,
        skipped: true,
        reason: "No uncaptured game is currently 7-23 minutes from first pitch.",
      });
    }

    const publicDataUrl = new URL(
      "/api/public-data?source=cron&tracking=15m",
      request.url,
    );
    const response = await fetch(publicDataUrl, {
      ...
      cache: "no-store",
      headers: {
        "x-ezpz-background-snapshot": "true",
      },
    });
    ...
    return NextResponse.json({
      ok: true,
      snapshotType: "ONE_PER_GAME_APPROX_15_MINUTES",
      targetGames: targets.map((row) => String(row.Game || gameKey(row))),
