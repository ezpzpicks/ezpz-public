...
function dailyPickObject(decision:DailyLockDecision,today:string,nowMs:number):AnyRow{
  ...
  const rule=decision.earlyPremium?`Early-game premium cleared: V2 Market Gap ${play.v2MarketGap.toFixed(1)} >= ${decision.threshold.toFixed(0)}`:`Normal daily threshold cleared: V2 Market Gap ${play.v2MarketGap.toFixed(1)} >= ${decision.threshold.toFixed(0)}`;
...
async function postProcessMlbPayload(request:NextRequest,payload:AnyRow){
  ...
  let todayLock=dailyPicks.find(pick=>isoDate(pick.date)===today)||null;
  if(!todayLock&&isV2ScheduledCapture(request)&&today>=MLB_TREND_V2_LAUNCH_DATE){const decision=chooseDailyTrendLock(scored,Array.isArray(payload.slateToday)?payload.slateToday:[],nowMs);if(decision){todayLock=dailyPickObject(decision,today,nowMs);try{await appendV2Rows("daily",[dailyPickRow(todayLock)]);dailyPicks.push(todayLock)}catch(error){console.error("Trend v2 daily lock persistence failed",error)}}}
  ...
  todayLock=dailyPicks.find(pick=>isoDate(pick.date)===today)||todayLock;
  ...
  if(todayLock)filteredAiPicks.push(todayLock);
  ...
  const status=payload.aiSelectorStatus||{};const finalCount=filteredAiPicks.filter((pick:AnyRow)=>pick?.snapshotStatus==="FINAL_PREGAME").length;
  payload.aiSelectorStatus={...status,mode:finalCount===filteredAiPicks.length&&filteredAiPicks.length?"FINAL_PREGAME":"LIVE_PREVIEW",message:todayLock?`MLB Trend v2 daily pick locked: ${todayLock.play}. No second Trend v2 pick can be added today.`:filteredAiPicks.length?"HOT Best Plays are final; MLB Trend v2 is still ranking the slate for its single daily trend pick.":"MLB Trend v2 is ranking the slate. No daily trend pick locks unless the market-gap threshold is reached.",updatedAt:nowET(),candidateCount:Number(status.candidateCount||0)+scored.filter(play=>play.v2Direction).length,selectedCount:filteredAiPicks.length,trendV2DailyLocked:Boolean(todayLock)};
  return payload;
}
