...
function nameTokens(value: unknown) {
  return normalizeText(value)
    .split(" ")
    .filter((token) => token.length >= 2 && !["over", "under", "line", "so", "ks", "strikeouts", "pitcher"].includes(token));
}

function namesShareAtLeastTwoTokens(a: unknown, b: unknown) {
  const aTokens = Array.from(new Set(nameTokens(a)));
  const bTokens = new Set(nameTokens(b));
  let shared = 0;
  for (const token of aTokens) {
    if (bTokens.has(token)) shared += 1;
  }
  return shared >= 2 || (aTokens.length === 1 && bTokens.has(aTokens[0]));
}
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  if (["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"].includes(playType) || market === "PITCHER STRIKEOUTS") {
    ...
    const haystack = [selection, row["Player"], row["Pitcher"], row["Name"], row["Play"], row["Pick"]].join(" ");
    const pitcherMatch =
      namesShareAtLeastTwoTokens(play.play, rowPitcher) ||
      namesShareAtLeastTwoTokens(play.play, haystack);

    const marketLooksLikePitcherK =
      market === "PITCHER STRIKEOUTS" ||
      market.includes("STRIKEOUT") ||
      market.includes("K PROP") ||
      ["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"].includes(betType);

    return marketLooksLikePitcherK && pitcherMatch;
  }
...
function applyFavoriteInfoToPlays(plays: Play[], trackerRows: SheetRow[], today: string) {
  return plays.map((play) => {
    ...
    const match =
      matchingRows.find((row) => isTrueValue(firstValue(row, ["Favorite Pick", "Handpicked", "Handpicked Pick", "Favorite"]))) ||
      matchingRows[0];
...
export async function GET() {
  try {
    ...
    const completedTrackerRows = trackerRows.filter((row) => ["WIN", "LOSS", "PUSH"].includes(String(row["Result"] || "").trim().toUpperCase()));
    const qualifiedTrackerRows = completedTrackerRows.filter((row) => isGreenType(row["Bet Type"] || row["Market"]));
    const handpickedCompletedRows = completedTrackerRows.filter((row) => isTrueValue(row["Favorite Pick"] || row["Handpicked"] || row["Favorite"]));
    const handpickedTodayRows = trackerRows.filter((row) => normalizeDate(row["Date"] || row["date"]) === today && isTrueValue(row["Favorite Pick"] || row["Handpicked"] || row["Favorite"]));
