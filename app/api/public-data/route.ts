...
function buildTrendRecordRows(completedRows: SheetRow[]): TrendRecordResult[] {
  ...
  for (const row of completedRows) {
    const market = trackerMarket(row);
    const result = resultCode(row.Result);
    const trendScoreDetails = String(row["Trend Score Details"] || "").trim();
    const finalSnapshot = textKey(
      row["Public Split Match Confidence"] || "",
    ).includes("15 minute tracking snapshot");
    if (!market || !result || !trendScoreDetails || !finalSnapshot) continue;

    const storedPlay = parseStoredTrendPlay(row);
    const frozenTier = storedPlay?.tier;
    const frozenScore = Number(storedPlay?.score);
    const frozenAt = String(storedPlay?.frozenAt || "").trim();
    const officialSnapshot =
      storedPlay?.snapshotStatus === "FINAL_PREGAME" &&
      Boolean(frozenAt) &&
      storedPlay.baseScore != null;
    if (!officialSnapshot) continue;
    if (
      frozenTier !== "Good" &&
      frozenTier !== "Strong" &&
      frozenTier !== "Elite"
    ...
    if (!Number.isFinite(frozenScore)) continue;
