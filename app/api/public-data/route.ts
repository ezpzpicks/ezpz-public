...
async function requestAiExternalReviews(candidates: AiSelectorCandidate[]): Promise<{
      ...
      reviews: {
        ...
        items: {
          ...
          properties: {
            ...
            why: { type: "array", items: { type: "string" }, maxItems: 4 },
          ...
          content: [
            {
              type: "input_text",
              text:
                "You are the external-context reviewer for a sports prediction selector. Research only verifiable information available before the listed game starts. Focus on confirmed lineups and scratches, injuries, role or workload changes, bullpen usage, pitcher velocity or availability, weather, travel/rest, umpire information when reliably reported, and meaningful market news. Never use a final score, in-game information, rumors without support, or hindsight. Do not select bets. Return only small evidence-based adjustments from -6 to +6. Use 0 when no verified outside context changes the case. A critical conflict is reserved for evidence that materially invalidates the saved pregame assumption, such as a scratched starter or changed role.",
            },
          ],
        },
        {
  ...
  for (const review of parsed.reviews || []) {
    ...
    reviews.set(review.candidateId, {
      ...
      why: Array.isArray(review.why) ? review.why.filter(Boolean).slice(0, 4) : [],
      risks: Array.isArray(review.risks) ? review.risks.filter(Boolean).slice(0, 4) : [],
    });
  }
  return {
    reviews,
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    return {
      ...
      rejectionReason,
      whySelected: [...new Set(candidate.whySelected.filter(Boolean))].slice(0, 7),
      risks: [...new Set(candidate.risks.filter(Boolean))].slice(0, 6),
  ...
  // Correlation protection: only the strongest AI selection from a single game
  // survives. Rejected candidates are still stored for comparison and testing.
  const byGame = new Map<string, AiSelectorCandidate[]>();
  for (const candidate of finalized.filter((item) => item.selected)) {
    const group = byGame.get(candidate.gameKey) || [];
    group.push(candidate as AiSelectorCandidate);
    byGame.set(candidate.gameKey, group);
  }
  for (const group of byGame.values()) {
    group.sort(
      (a, b) =>
        b.estimatedProbability - a.estimatedProbability ||
        b.aiScore - a.aiScore ||
        b.estimatedAdvantage - a.estimatedAdvantage,
    );
    for (const candidate of group.slice(1)) {
      candidate.selected = false;
      candidate.rejectionReason = `Correlation protection selected ${group[0].play} as the stronger play from this game`;
      candidate.risks = [...new Set([...candidate.risks, "Another qualifying play from the same game ranked higher"])];
    }
  }
...
async function buildAiPickSelector(args: {
  ...
  // A final market snapshot can already exist even when this request is a normal
  // public-page refresh. Previously, captureRequested gated every finalization
  // path, so a snapshot that was saved successfully by the background job could
  // remain stuck as a LIVE AI preview forever if that same job timed out or ended
  // before the AI selector ran. Allow any later request to finish the AI lock once
  // either the slate or the persisted DraftKings feed proves the snapshot is final.
  const finalSnapshotGameKeys = new Set(
    draftKings.splits
      .filter((split) => split.snapshotStatus === "FINAL_PREGAME")
      .map(
        (split) =>
          `${isoPublicDate(split.date)}|${normalizeTeam(split.awayTeam)}|${normalizeTeam(split.homeTeam)}`,
      ),
  ...
  const targetGameKeys = new Set(
    slateRows
      .filter((row) => {
        ...
        const finalSnapshotExists =
          slateHasFinalPregameSnapshot(row) || finalSnapshotGameKeys.has(gameKey);
