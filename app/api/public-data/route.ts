...
function extractPitcherFromSelection(value: unknown) {
  ...
  // "George Kirby 6+ SO" -> "George Kirby"
  // "Kirby, George 6.4 (OVER) Line 5.5" -> "Kirby, George"
  // "George Kirby Over 5.5 Ks" -> "George Kirby"
  const withoutParens = text.split("(", 1)[0].trim();
  const beforeLine = withoutParens.replace(/\b(Line|Over|Under|Ks|SO|Strikeouts)\b.*$/i, "").trim();
  const beforeNumber = beforeLine.match(/^(.*?)(?=\s+\d+(?:\.\d+)?|\s+\d+\+)/);
  return (beforeNumber ? beforeNumber[1] : beforeLine).trim();
}

function pitcherNameVariants(value: unknown) {
  const raw = extractPitcherFromSelection(value);
  ...
  if (raw.includes(",")) {
    ...
    variants.add(normalizeText(`${first} ${last}`));
  } else {
    const parts = raw.split(/\s+/).filter(Boolean);
    if (parts.length >= 2) {
      const first = parts[0];
      const last = parts.slice(1).join(" ");
      variants.add(normalizeText(`${last} ${first}`));
    }
  }
...
function namesMatch(a: unknown, b: unknown) {
  const aVariants = pitcherNameVariants(a);
  const bVariants = pitcherNameVariants(b);
  if (!aVariants.size || !bVariants.size) return false;
  ...
  for (const av of aVariants) {
    for (const bv of bVariants) {
      ...
      const aTokens = av.split(" ").filter(Boolean).sort().join(" ");
      const bTokens = bv.split(" ").filter(Boolean).sort().join(" ");
      if (aTokens && aTokens === bTokens) return true;
    }
  }
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  const marketRaw = String(row["Market"] || "").trim();
  const market = marketRaw.toUpperCase();
  const marketNorm = normalizeText(marketRaw);
  const selection = String(row["Selection"] || row["Pick"] || row["Play"] || row["Player"] || row["Pitcher"] || "").trim();
  ...
  if (playType.includes("MONEYLINE") || betType.includes("MONEYLINE") || marketNorm.includes("moneyline")) {
    return marketNorm.includes("moneyline") && selectionNorm && (selectionNorm === playerTeamText || playText.includes(selectionNorm) || playerTeamText.includes(selectionNorm));
  }

  if (playType.includes("NRFI") || playType === "YRFI" || betType.includes("NRFI") || betType === "YRFI") {
    const rowLooksLikeNRFI = marketNorm.includes("nrfi") || marketNorm.includes("yrfi") || betType.includes("NRFI") || betType === "YRFI";
    return rowLooksLikeNRFI && (selectionNorm === gameText || gameText.includes(selectionNorm) || selectionNorm.includes(gameText) || betType === playType);
  }

  const kTypes = ["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"];
  if (kTypes.includes(playType) || kTypes.includes(betType) || marketNorm.includes("strikeout") || marketNorm.includes("pitcher")) {
    const rowLooksLikeK = kTypes.includes(betType) || marketNorm.includes("strikeout") || marketNorm.includes("pitcher");
    return rowLooksLikeK && namesMatch(selection, play.play);
  }
...
function applyFavoriteInfoToPlays(plays: Play[], trackerRows: SheetRow[], today: string) {
  return plays.map((play) => {
    ...
    return {
      ...play,
      favoritePick: firstValue(match, ["Favorite Pick", "Handpicked", "Handpicked Pick", "Favorite"]),
...
export async function GET() {
  try {
    ...
    const handpickedTrackerRows = completedTrackerRows.filter((row) => isTrueValue(row["Favorite Pick"] || row["Handpicked"] || row["Favorite"]));
    const last7QualifiedRows = rowsFromLast7Days(qualifiedTrackerRows);
    const last7HandpickedRows = rowsFromLast7Days(handpickedTrackerRows);
    ...
    return NextResponse.json({
      ...
      tiles: {
        ...
        handpickedToday: buildTotals("Handpicked Plays Today", handpickedTrackerRows.filter((row) => normalizeDate(row["Date"] || row["date"] || "") === today)),
        handpickedLast7: buildTotals("Handpicked Plays - Last 7 Days", last7HandpickedRows),
        handpickedOverall: buildTotals("Handpicked Plays - Running Total", handpickedTrackerRows),
        pendingGreen,
        bestPlaysToday: bestPlays.length,
      },
      bestPlays,
      slateToday,
      betTrackerRows: trackerRaw,
