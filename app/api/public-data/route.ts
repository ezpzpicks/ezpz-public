...
function trendRecordScore(record: TrendRecord) {
  ...
  const reliability = Math.min(1, record.totalBets / 18);
...
function trendTier(score: number): TrendPlay["tier"] {
  if (score >= 80) return "Strong Trend Play";
  if (score >= 70) return "Qualified Trend Play";
  return "Trend Lean";
}
...
function buildTrendPlayForSplit(
  ...
  // A balanced split by itself is context, not a Trend Play trigger.
  const hasDirectionalSignal = activeSignals.some(
    (signal) => signal.signalType === "Line Movement" || signal.signalKey !== "BALANCED_MARKET",
  );
  if (!hasDirectionalSignal) return null;
  ...
  if (!signalsWithHistory.length) return null;

  let score = signalsWithHistory.reduce((sum, signal) => sum + signal.score, 0) /
    signalsWithHistory.length;
  const allWindowsPositive = signalsWithHistory.every(
    (signal) =>
