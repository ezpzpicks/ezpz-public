...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v12-live-k-pending-review";
...
function aiGameResearchPayload(candidates: AiSelectorCandidate[]) {
  ...
  return {
    ...
    modelGameContext: anchor.slateRow
      ? Object.fromEntries(
...
function aiGameExternalReviewSchema(candidateCount: number) {
  return {
    ...
    properties: {
      gameKey: { type: "string" },
      contextSummary: { type: "string", minLength: 30, maxLength: 320 },
      startingPitching: { type: "string", minLength: 40, maxLength: 700 },
      bullpenAnalysis: { type: "string", minLength: 30, maxLength: 500 },
      recentTeamForm: { type: "string", minLength: 30, maxLength: 500 },
      historicalMatchup: { type: "string", minLength: 20, maxLength: 500 },
      candidateReviews: {
        type: "array",
        ...
        items: {
          ...
          properties: {
            ...
            researchSummary: { type: "string", minLength: 60, maxLength: 600 },
            selectionComparison: { type: "string", minLength: 30, maxLength: 420 },
            finalVerdict: { type: "string", minLength: 30, maxLength: 320 },
          },
        },
      },
    },
...
function sanitizeAiGameExternalReview(
  ...
  return {
    gameKey: String(review?.gameKey || "").trim(),
    contextSummary: sanitizeAiPublicText(review?.contextSummary),
    startingPitching: sanitizeAiPublicText(review?.startingPitching),
    bullpenAnalysis: sanitizeAiPublicText(review?.bullpenAnalysis),
    recentTeamForm: sanitizeAiPublicText(review?.recentTeamForm),
...
function aiGameExternalReviewIsComplete(
  ...
  if (
    review.contextSummary.length < 30 ||
    review.startingPitching.length < 40 ||
    review.bullpenAnalysis.length < 30 ||
    review.recentTeamForm.length < 30 ||
    review.historicalMatchup.length < 20 ||
  ...
  const returnedIds = new Set(review.candidateReviews.map((item) => item.candidateId));
  if (returnedIds.size !== expectedIds.size) return false;
  for (const candidateId of expectedIds) {
    if (!returnedIds.has(candidateId)) return false;
  }

  return review.candidateReviews.every(
    (item) =>
      expectedIds.has(item.candidateId) &&
      item.confidenceReason.length >= 1 &&
      item.why.length >= 3 &&
      item.researchSummary.length >= 60 &&
      item.selectionComparison.length >= 30 &&
      item.finalVerdict.length >= 30,
  );
}
...
function aiExternalReviewsFromGameReview(
  ...
  const candidateReviews = new Map(
    gameReview.candidateReviews.map((item) => [item.candidateId, item] as const),
  );
  const reviews = new Map<string, AiExternalReview>();
  for (const candidate of candidates) {
    const item = candidateReviews.get(candidate.candidateId);
    if (!item) continue;
...
async function fetchSingleAiGameExternalReviews(
  ...
  try {
    response = await fetch("https://api.openai.com/v1/responses", {
      ...
      model,
      store: false,
      prompt_cache_key: `ezpz-ai-selector:${AI_PICK_SELECTOR_VERSION}:game-review`,
      tools: [
        {
          type: "web_search",
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: `You are the final MLB research analyst for EZPZ AI Picks. Research only information verifiably available before this game starts. Research this one game once, then independently review every supplied candidate for that game. Reuse the shared researched facts across every candidate and do not run separate searches per wager.

The shared game research must read like a specific MLB matchup report. Begin with the actual probable or confirmed starting pitchers and the key players who directly affect the supplied wagers. For each pitcher prop, center its candidate review on that pitcher and the opposing projected lineup. For a moneyline or total, explain how both starters, the lineups, and the bullpens affect that selection. For NRFI/YRFI, emphasize each starter's first-inning profile and the top-of-order hitters.
...
First-inning terminology is literal and non-negotiable. YRFI means Yes Run First Inning: at least one run is expected in the first inning, so it is a pro-scoring signal. NRFI means No Run First Inning, so it is an anti-scoring signal. ELITE YRFI conflicts with a full-game Under and must never be described as favoring a low-scoring start or supporting an Under. ELITE NRFI conflicts with a full-game Over and must never be described as supporting an Over. Even when the directions align, a one-inning signal is context only and is not sufficient proof of a nine-inning total. If a candidate's firstInningSignal.relationshipToCandidate is CONFLICTS, set that candidate's approved value to false and do not cite that signal positively.

For every Pitcher Strikeouts candidate, treat the supplied AI score of 85 as a neutral selector baseline, not an 85% win probability and not proof that the wager should be selected. Independently pressure-test each prop before adjusting it. Specifically answer through that candidate's analysis: (1) Is the K-model projection meaningfully separated from the sportsbook line? (2) Does the expected opposing lineup support the Over/Under, emphasizing individual hitters and handedness-specific strikeout tendencies? (3) Does the pitcher's arsenal/pitch mix create favorable or unfavorable whiff matchups against this lineup? (4) Do the pitcher's last 3-5 starts support the current strikeout skill without overreacting to a tiny sample? (5) Are expected innings, pitch count, batters faced, rest and leash sufficient for the prop line? (6) Is opponent or batter history meaningful after accounting for sample size and current roster? (7) Does today's actual/projected lineup materially differ from the generic team strikeout profile? (8) Does the supplied running and recent record for this exact EZPZ K grade support or weaken confidence? (9) Is there any evidence that makes the model projection misleading? (10) After all of that, does this prop deserve to beat the 85 baseline and compete with the other AI candidates in this game? A negative net assessment must set approved to false, lower the score and exclude the prop; a positive, evidence-backed assessment may set approved to true and raise it. Do not reward a prop merely because its model grade is Strong Over/Over/Lean Over/Strong Under/Under/Lean Under.

Fully populate every shared field and return exactly one candidateReviews item for every supplied candidateId, with no omissions or duplicates. Set approved to true only when the completed shared and candidate-specific research supports publishing that wager as an EZPZ AI Pick. Set approved to false when the verified matchup, lineup, workload, form, market or historical evidence produces a materially negative net assessment, even if the supplied pre-review AI score is 100. approved=false is a final veto and is not reserved for critical conflicts. Neutral research may return approved=true with an adjustment of 0 when the supplied quantitative case remains sound. Do not reject a wager merely because an optional split or small-sample history could not be verified. startingPitching must name both starters and any prop pitcher requiring extra detail. bullpenAnalysis must describe recent availability or explicitly say that no material workload concern was verified. recentTeamForm must summarize both teams' meaningful recent performance. historicalMatchup must include sample context or state that no meaningful sample was found. Each selectionComparison must explain why that exact wager should be approved or rejected using its supplied score, probability, edge and matchup evidence. Each finalVerdict must be direct, wager-specific and consistent with approved. Never substitute generic phrases such as “no outside context changed the play” for analysis.

Spend each candidate's explanation budget on WHY that wager was approved or rejected, not on a separate risks section. Each WHY array should contain 3-5 detailed, concise, non-redundant bullets, prioritizing the details unique to that selection; the shared starting-pitching, bullpen, team-form and historical sections will be reused for every candidate. Include player-specific bullets whenever verified information supports them. Do not generate public risk bullets. Still use negative evidence internally when choosing approved, the adjustment or criticalConflict. Return only a small evidence-based adjustment from -6 to +6; use 0 when research does not change the quantitative case. Keep approved consistent with the net assessment: materially negative research must return approved=false, while approved=true means the wager still deserves publication after research. A critical conflict is reserved for evidence that invalidates a saved pregame assumption, such as a scratched starter or changed role.

Do not include URLs, markdown links, citations, domains, publication names, source labels, bracketed source markers, or phrases such as according to. Return only the required JSON object. Use concise, specific analysis so every candidate review finishes within the shared game token budget.`,
            },
          ],
        },
        {
  ...
  const gameReview = sanitizeAiGameExternalReview(parsed);
...
async function requestSingleAiGameExternalReviews(
  ...
  if (cached && Date.now() - cached.savedAt < AI_GAME_RESEARCH_CACHE_TTL_MS) {
  ...
  const request = fetchSingleAiGameExternalReviews(candidates);
...
async function requestAiExternalReviews(
  ...
  const candidatesByGame = new Map<string, AiSelectorCandidate[]>();
  for (const candidate of candidates) {
    const gameCandidates = candidatesByGame.get(candidate.gameKey) || [];
    gameCandidates.push(candidate);
    candidatesByGame.set(candidate.gameKey, gameCandidates);
  }
  const gameGroups = [...candidatesByGame.values()];
  ...
  const worker = async () => {
    while (true) {
      ...
      nextIndex += 1;
      if (index >= gameGroups.length) return;
      const gameCandidates = gameGroups[index];
      if (!gameCandidates?.length) return;
      try {
        // One Responses request researches the game once and returns a separate
        // decision for every eligible candidate in that game. The request itself
        // also caps built-in tool use at one, so a full slate cannot fan out into
        // per-pick or multi-search billing.
        const gameReviews = await requestSingleAiGameExternalReviews(gameCandidates);
        for (const candidate of gameCandidates) {
          ...
          if (review) {
            ...
            errors.set(
              ...
              "Shared game research did not return this candidate decision",
            );
          }
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        for (const candidate of gameCandidates) {
          errors.set(candidate.candidateId, message);
        }
      }
    }
  };
  ...
  await Promise.all(
    Array.from({ length: Math.min(concurrency, gameGroups.length) }, () => worker()),
  );
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    const livePitcherStrikeoutPreview =
      candidate.market === "Pitcher Strikeouts" && snapshotStatus === "LIVE";
    ...
    // Pitcher props may enter the live board at their neutral 85 baseline so
    // users can see that they are under review. The final snapshot still needs
    // 86+, the probability/value gates, and an affirmative external review.
    ...
    const selectionThresholds = candidate.market === "Pitcher Strikeouts"
      ? {
          score: livePitcherStrikeoutPreview ? 85 : 86,
          probability: 55,
          advantage: 0.5,
    ...
    const rejectionReason = blocked
      ...
      : thresholdFailure;
    const livePitcherReviewNote =
      livePitcherStrikeoutPreview && preliminarySelected
        ? "Pending final strikeout review: final selection still requires AI score 86+, estimated probability 55%+, estimated advantage 0.5%+, and final AI approval."
        : "";
    return {
      ...candidate,
      aiScore,
      ...
      whySelected: sanitizeAiPublicList(
        livePitcherReviewNote
          ? [livePitcherReviewNote, ...candidate.whySelected]
      ...
      dataStatus: [
        ...new Set(
          [livePitcherReviewNote, ...candidate.dataStatus].filter(Boolean),
        ),
      ].slice(0, 5),
...
function aiStoredFinalDecisionIsTerminal(pick: AiPick) {
  ...
  // Shared game research is intentionally attempted only once per final
  // snapshot. A failed request remains safely rejected instead of triggering
  // another paid web-search call on every public refresh.
  if (pick.externalReviewStatus === "REVIEW_ERROR") return true;
  return (
