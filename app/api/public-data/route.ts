...
const FROZEN_TREND_GRADING_VERSION = "frozen-h2h-exact-v3";
  ...
  try {
    ...
    for (const matrixRow of trendMatrix.rows) {
      ...
      const storedTrendIsFinal =
        ...
        storedTrendPlay.gradingVersion === FROZEN_TREND_GRADING_VERSION;
...
function frozenTrendPlaysFromRows(
  ...
  for (const gameItems of byGame.values()) {
    const gamePlays = gameItems.map((item) => item.play);
