...
async function buildAiPickSelector(args: {
  ...
  const targetGameKeys = new Set(
    slateRows
      .filter((row) =>
        // A normal public refresh must recognize a snapshot that was already
        // locked by the scheduled/background request. Only the live 15-minute
        // window itself requires an explicit capture request.
