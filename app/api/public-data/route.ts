...
async function requestAiExternalReviews(candidates: AiSelectorCandidate[]): Promise<{
  ...
  const maxOutputTokens = Math.max(
    1200,
    Math.min(4000, Number(process.env.EZPZ_AI_MAX_OUTPUT_TOKENS || 2600)),
  );
...
async function persistAiPickRows(existingRows: SheetRow[], finalPicks: AiPick[]) {
  ...
  for (const pick of finalPicks) {
    ...
    const sameFinalResult =
      parsed?.snapshotStatus === "FINAL_PREGAME" &&
      parsed.result === pick.result &&
      parsed.units === pick.units &&
      parsed.resultUpdated === pick.resultUpdated;
    if (sameFinalResult) continue;
...
function aiRowMatchesGame(pick: AiPick, row: SheetRow) {
  ...
  const rowAway = normalizeTeam(row["Away Team"] || "");
  const rowHome = normalizeTeam(row["Home Team"] || "");

  // Team columns are the strongest game identity when they match. Some tracker
  // rows use abbreviations (WSH/PHI) while AI rows use full names, so a mismatch
  // here must fall through to the Game text instead of rejecting the row.
  if (rowAway && rowHome && rowAway === pickAway && rowHome === pickHome) {
    return true;
  }

  const rowGame = textKey(row.Game || row["Game Label"] || "");
  if (!rowGame) return false;
  if (rowGame === textKey(pick.game)) return true;

  // Accept common aliases such as "Nationals @ Phillies" while still requiring
  // both teams, so a YRFI/NRFI result cannot be borrowed from another game.
  const awayAliases = [pickAway, ...(MLB_TEAM_ALIASES[pickAway] || [])]
    .map(textKey)
    .filter(Boolean);
  const homeAliases = [pickHome, ...(MLB_TEAM_ALIASES[pickHome] || [])]
    .map(textKey)
    .filter(Boolean);
  return awayAliases.some((alias) => rowGame.includes(alias)) &&
    homeAliases.some((alias) => rowGame.includes(alias));
}
...
function aiPickTrackerMatch(pick: AiPick, trackerRows: SheetRow[]) {
  ...
  return trackerRows.find((row) => {
    ...
    const type = normalizeType(row["Bet Type"] || row.Market || row.Selection || "");
    ...
    if (
      !String(row.Market || "").toLowerCase().includes("pitcher strikeout") &&
      !isPitcherKType(type)
    ) return false;
...
function overlayAiPickResults(
  ...
  return picks.map((pick) => {
    ...
    // Keep a stored result only when no authoritative graded source is available.
    // This allows corrected tracker/trend grades to repair an AI record automatically.
    if (!authoritativeResult) return pick;
