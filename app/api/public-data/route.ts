...
async function persistAiPickRows(existingRows: SheetRow[], finalPicks: AiPick[]) {
  ...
  for (const pick of finalPicks) {
    ...
    // Always replace the stored candidate row. The AI review fields can change
    // without the grading fields changing, so comparing only Result/Units would
    // incorrectly discard a completed research review.
