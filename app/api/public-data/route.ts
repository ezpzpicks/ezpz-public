...
function aiCandidateResearchPayload(candidate: AiSelectorCandidate, compact = false) {
  ...
  return {
    ...
    existingRisks: candidate.risks.slice(0, 3),
...
function aiExternalReviewSchema(compact = false) {
  ...
  return {
    ...
    properties: {
      ...
      mainRisk: { type: "string", minLength: 10, maxLength: 280 },
...
function sanitizeAiExternalReview(review: AiExternalReview): AiExternalReview {
  return {
    ...
    mainRisk: sanitizeAiPublicText(review.mainRisk),
...
async function requestSingleAiExternalReview(
  ...
  const compactInstruction = compact
    ? "This is a retry after an incomplete response. Be concise. Use 1-3 sentences per narrative field and no more than five WHY bullets, while still completing every required field."
...
Fully populate every required field. startingPitching must name the relevant starter or prop pitcher. bullpenAnalysis must describe recent availability or explicitly say that no material workload concern was verified. recentTeamForm must summarize meaningful recent performance. historicalMatchup must include sample context or state that no meaningful sample was found. selectionComparison must explain why this wager cleared using the supplied score, probability, edge and matchup evidence. mainRisk must identify the clearest baseball-related failure path. finalVerdict must be direct and wager-specific. Never substitute generic phrases such as “no outside context changed the play” for analysis.

The WHY section should contain detailed, concise bullets, with at least three player-specific bullets when verified information exists. Risks must be genuine. Return only a small evidence-based adjustment from -6 to +6; use 0 when research does not change the quantitative case. A critical conflict is reserved for evidence that invalidates a saved pregame assumption, such as a scratched starter or changed role.
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    if (review) {
      ...
      candidate.risks.push(review.mainRisk);
