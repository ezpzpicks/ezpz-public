...
function favoriteValue(row: SheetRow) {
  // Handpicked status must come only from the actual Favorite Pick column.
  // No fallback columns, tags, notes, team/game matches, or inferred values.
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  const rowIsMoneyline = betType.includes("MONEYLINE") || market === "MONEYLINE";

  const playIsNrfi = playType.includes("NRFI") || playType === "YRFI";
  const rowIsNrfi = betType.includes("NRFI") || betType === "YRFI" || ["NRFI/YRFI", "NRFI", "YRFI"].includes(market);

  const playIsPitcherK = isPitcherKType(playType);
  const rowIsPitcherK = isPitcherKType(betType) || market.includes("STRIKEOUT") || market.includes("PITCHER");

  if (playIsMoneyline) {
    // Moneyline favorites must match the exact team chosen on the public play card.
    // Do not match by game label because that can spill the badge to the other team or another play.
    return rowIsMoneyline && Boolean(selectionNorm) && Boolean(playerTeamText) && selectionNorm === playerTeamText;
  }

  if (playIsNrfi) {
    // NRFI/YRFI favorites must match the exact game AND exact NRFI/YRFI type.
    // This specifically prevents one ELITE NRFI TRUE row from badging every ELITE NRFI on the slate.
    return rowIsNrfi && Boolean(selectionNorm) && Boolean(gameText) && selectionNorm === gameText && betType === playType;
  }

  if (playIsPitcherK) {
    // Pitcher props must match pitcher identity only within pitcher strikeout rows.
    // This prevents Nolan McLean from matching a Mets NRFI row.
    ...
    return rowIsPitcherK && (namesShareAtLeastTwoTokens(play.play, rowPitcher) || namesShareAtLeastTwoTokens(play.play, rowHaystack));
  }
...
function applyFavoriteInfoToPlays(plays: Play[], trackerRows: SheetRow[], today: string) {
  // Build a strict list of TODAY'S TRUE tracker rows, then consume each one once.
  // This prevents one TRUE row from creating multiple handpicked badges after sorting/filtering.
  const favoriteRows = trackerRows
    .map((row, index) => ({ row, index }))
    .filter(({ row }) => normalizeDate(row["Date"] || row["date"] || "") === today && isFavoriteRow(row));

  const usedFavoriteRowIndexes = new Set<number>();

  return plays.map((play) => {
    const match = favoriteRows.find(({ row, index }) => {
      if (usedFavoriteRowIndexes.has(index)) return false;
      return trackerRowMatchesPlay(row, play, today);
    });
    ...
    usedFavoriteRowIndexes.add(match.index);
    const row = match.row;
...
export async function GET() {
  try {
    ...
    const trackerRows: SheetRow[] = (trackerRaw as SheetRow[]).map((row: SheetRow): SheetRow => ({
      ...row,
    ...
    return NextResponse.json({
      ...
      tiles: {
        ...
        handpickedOverall: buildTotals("Handpicked Plays - Running Total", handpickedCompletedRows),
        pendingGreen,
