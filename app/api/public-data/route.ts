...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v13-compact-game-review";
...
Return candidateReviews in exactly the same order as the supplied candidates, with exactly one item for each. approved=true means the wager still deserves publication after research. approved=false is a final veto for materially negative evidence; it is not limited to critical conflicts. Use a small adjustment from -6 to +6, and use 0 when research does not change the supplied quantitative case. Each candidate needs two or three concise WHY bullets focused on the actual reason it cleared or failed—never a public risk list.

For Pitcher Strikeouts, treat the supplied 85 AI score as a neutral selector baseline, not 85% probability or automatic approval. Test the projection versus line, opposing lineup K/contact tendencies, pitcher arsenal/whiff fit, recent starts, innings/pitch count/leash, relevant history sample, and whether today’s lineup differs from generic team rates. A negative net assessment must be approved=false.
