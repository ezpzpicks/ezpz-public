...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v16-last7-form-gates-trend-reweight";
...
function aiRecordAdjustments(candidate: AiSelectorCandidate, completedTrackerRows: SheetRow[]) {
  ...
  // Pitcher props use the exact same "Last 7 Bets" classification displayed on
  // page.tsx. Historical form changes the mandatory grade instead of changing
  // the underlying EZPZ score. A Cold bucket is ineligible until its rolling
  // last-seven record naturally improves.
  if (candidate.market === "Pitcher Strikeouts") {
    ...
    const requiredScore = aiPitcherRequiredScore(lastSeven, form);
    candidate.pitcherBetTypeForm = form;
    candidate.pitcherBetTypeRecord = lastSeven.record;
    candidate.pitcherRequiredScore = requiredScore;

    if (form === "COLD") {
      candidate.protectionReasons.push(
        `${recordType} is Cold over its last 7 completed bets (${lastSeven.record}); Cold pitcher-prop bet types are excluded from AI Picks until the rolling record improves`,
      );
    ...
    if (form === "HOT") {
      candidate.whySelected.push(
        `${recordType} is Hot over its last 7 completed bets (${lastSeven.record}); pitcher-prop gates lowered to score 74+, estimated probability 50%+, and estimated advantage 1.5%+`,
      );
      candidate.historicalNotes.push(
        `${recordType} Last 7 Bets: Hot • ${lastSeven.record} • minimum score 74 • minimum probability 50% • minimum advantage 1.5%`,
      );
      return;
    }
    ...
    if (form === "NEUTRAL") {
      candidate.dataStatus.push(
        `${recordType} Last 7 Bets: Neutral • ${lastSeven.record} • minimum score 80 • minimum probability 52.5% • minimum advantage 3.25%`,
      );
      candidate.historicalNotes.push(
        `${recordType} Last 7 Bets: Neutral • ${lastSeven.record}`,
      );
      return;
    }

    candidate.dataStatus.push(
      `${recordType} Last 7 Bets: Need 7 Bets • ${lastSeven.record} • minimum score 86 • minimum probability 55% • minimum advantage 5%`,
    );
    candidate.historicalNotes.push(
  ...
  // Non-pitcher markets retain the broader historical adjustment layer added
  // previously.
  const overall = aiSummaryForType(completedTrackerRows, recordType);
  const recent = aiSummaryForType(rowsFromLast7Days(completedTrackerRows), recordType);
  const overallDecisions = overall.wins + overall.losses;
  const recentDecisions = recent.wins + recent.losses;
  const longTermAdjustment = aiLongTermRecordAdjustment(overall);
  const consistencyAdjustment = aiRecentRecordConsistencyAdjustment(overall, recent);
  const scoreAdjustment = longTermAdjustment + consistencyAdjustment;

  if (scoreAdjustment) {
    candidate.scoreAdjustment += scoreAdjustment;
    candidate.probabilityAdjustment += aiClamp(scoreAdjustment * 0.45, -3, 2.5);
  }

  if (overallDecisions) {
    const sampleNote = overallDecisions < 10 ? " (small sample; no material score boost)" : "";
    candidate.historicalNotes.push(
      `${recordType} running record: ${overall.wins}-${overall.losses}-${overall.pushes} (${overall.winPct.toFixed(1)}%)${sampleNote}`,
    );
    if (longTermAdjustment > 0) {
      candidate.whySelected.push(
        `${recordType} historical performance is ${overall.wins}-${overall.losses}-${overall.pushes} (${overall.winPct.toFixed(1)}%) across ${overallDecisions} decisions`,
      );
    } else if (longTermAdjustment < 0) {
  ...
  if (recentDecisions) {
    candidate.historicalNotes.push(
      `${recordType} last-7 record: ${recent.wins}-${recent.losses}-${recent.pushes} (${recent.winPct.toFixed(1)}%)`,
    );
    if (consistencyAdjustment > 0) {
      candidate.whySelected.push(
        `Recent ${recordType} results (${recent.wins}-${recent.losses}-${recent.pushes}) reinforce the positive long-term record`,
      );
    } else if (consistencyAdjustment < 0) {
      candidate.risks.push(
        `Recent ${recordType} results (${recent.wins}-${recent.losses}-${recent.pushes}) reinforce the weak long-term record`,
      );
    } else if (recent.winPct >= 65 && overallDecisions >= 10 && overall.winPct < 55) {
      candidate.dataStatus.push(
        `Recent ${recordType} hot streak was not boosted because the longer-term record is not yet supportive`,
      );
    }
  }
}
...
function aiCandidateResearchPayload(candidate: AiSelectorCandidate) {
  ...
  const pitcherProfile = candidate.market === "Pitcher Strikeouts"
  ...
  return {
    ...
    pitcherQualificationThresholds:
      pitcherProfile
        ? {
            score: candidate.pitcherRequiredScore || pitcherProfile.score,
            estimatedProbability: pitcherProfile.probability,
            estimatedAdvantage: pitcherProfile.advantage,
          }
        : undefined,
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    // Pitcher props enter with their differentiated strikeout-model score.
    // Last-7 bet-type form now controls the complete quantitative gate:
    // Hot = 74 / 50% / 1.5%; Neutral = 80 / 52.5% / 3.25%;
    // Small Sample = 86 / 55% / 5%. Cold is deterministically excluded.
    // Every FINAL_PREGAME selection still requires affirmative external approval.
    const pitcherProfile = aiPitcherQualificationProfile(
      candidate.pitcherBetTypeForm,
    );
    const pitcherFinalRequiredScore = candidate.pitcherRequiredScore || pitcherProfile.score;
    const pitcherLiveRequiredScore = Math.max(0, pitcherFinalRequiredScore - 1);
    const selectionThresholds = candidate.market === "Pitcher Strikeouts"
      ? {
          score: livePitcherStrikeoutPreview
            ? pitcherLiveRequiredScore
            : pitcherFinalRequiredScore,
          probability: pitcherProfile.probability,
          advantage: pitcherProfile.advantage,
          enforceProbability: pitcherProfile.enforceProbability,
        }
      : plusMoneyMoneyline
        ? { score: 80, probability: 0, advantage: AI_MINIMUM_ESTIMATED_ADVANTAGE, enforceProbability: false }
        : { score: 74, probability: 55, advantage: AI_MINIMUM_ESTIMATED_ADVANTAGE, enforceProbability: true };
    ...
    const livePitcherReviewNote =
      livePitcherStrikeoutPreview && preliminarySelected
        ? `Pending final strikeout review: ${candidate.bestPlayType || "Pitcher prop"} is ${candidate.pitcherBetTypeForm || "SAMPLE"} over its last 7 completed bets (${candidate.pitcherBetTypeRecord || "0-0-0"}); final selection requires AI score ${pitcherFinalRequiredScore}+, estimated probability ${pitcherProfile.probability.toFixed(1)}%+, estimated advantage ${pitcherProfile.advantage.toFixed(2)}%+, and final AI approval.`
        : "";
    return {
      ...candidate,
      aiScore,
