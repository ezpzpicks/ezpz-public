...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v3-single-review";
...
function aiGameIsAtOrPastStart(row: SheetRow, now = Date.now()) {
  const start = scheduledGameStart(row);
  return start != null && now >= start;
}
...
function aiProtectionChecks(candidate: AiSelectorCandidate) {
  ...
  // Do not disqualify an otherwise valid AI pick only because the game has started.
  // Final pregame selections remain visible after first pitch so the locked analysis
  // can still be reviewed and graded. The route still relies on the saved final
  // pregame snapshot; this does not turn in-game data into a new qualifying pick.
...
function aiCandidateResearchPayload(candidate: AiSelectorCandidate, compact = false) {
  const maxFields = compact ? 70 : 120;
  const maxValueLength = compact ? 360 : 700;
  return {
...
async function requestSingleAiExternalReview(
  ...
  const configuredMax = Number(process.env.EZPZ_AI_MAX_OUTPUT_TOKENS || 4200);
  const maxOutputTokens = Math.max(2400, Math.min(6500, configuredMax));
...
async function requestAiExternalReviews(
  ...
  const concurrency = Math.max(
    1,
    Math.min(3, Number(process.env.EZPZ_AI_REVIEW_CONCURRENCY || 2)),
  );
  let nextIndex = 0;
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    if (snapshotStatus === "FINAL_PREGAME" && !review) {
      candidate.protectionReasons.push(
    ...
    const selectionThreshold = candidate.market === "Pitcher Strikeouts" ? 86 : 74;
    const thresholdFailure =
      aiScore < selectionThreshold
        ? candidate.market === "Pitcher Strikeouts"
          ? `AI score ${aiScore} did not improve beyond the 85 strikeout-prop baseline`
        ...
        : estimatedProbability < 55
          ...
          : implied && advantage < (candidate.market === "Pitcher Strikeouts" ? 0.5 : 1.5)
            ? candidate.market === "Pitcher Strikeouts"
              ? `Estimated advantage ${advantage.toFixed(1)}% did not reach the 0.5% pitcher-prop threshold`
...
function aiRowMatchesGame(pick: AiPick, row: SheetRow) {
  ...
  if (rowAway && rowHome && rowAway === pickAway && rowHome === pickHome) {
    return true;
  }
  ...
  if (pickGame && rowGame === pickGame) return true;
...
function aiPickTrackerMatch(pick: AiPick, trackerRows: SheetRow[]) {
  ...
  if (pick.market === "First Inning") {
    ...
    const exactGame = sameSideRows.find((row) => aiRowMatchesGame(pick, row));
    ...
    const completedSameSide = sameSideRows.filter((row) => Boolean(resultCode(row.Result)));
    return completedSameSide.length === 1 ? completedSameSide[0] : null;
  }
  ...
  if (pick.market === "Pitcher Strikeouts") {
    ...
    return sameDateRows.find((row) => {
      ...
      if (pickLine && rowLine && Math.abs(pickLine - rowLine) > 0.001) return false;
  ...
  return sameDateRows.find((row) => {
    ...
    if (pick.market === "Moneyline") {
      return market === "Moneyline" &&
        normalizeTeam(teamFromSelection(row.Selection || row.Team || "")) ===
  ...
  return picks.map((pick) => {
    ...
    if (!source && (pick.market === "Moneyline" || pick.market === "Total")) {
      source = allGameTrendRows.find((row) => {
        if (isoPublicDate(row.Date || "") !== isoPublicDate(pick.date)) return false;
        if (!aiRowMatchesGame(pick, row)) return false;
...
function aiStoredFinalSelectionIsLocked(pick: AiPick) {
  ...
  return (
    ...
    pick.selected === true &&
    aiStoredReviewIsCurrentAndComplete(pick)
  );
}

function aiStoredReviewIsCurrentAndComplete(pick: AiPick) {
  if (pick.snapshotStatus !== "FINAL_PREGAME") return false;
  if (pick.externalReviewStatus !== "WEB_REVIEWED") return false;
  if (pick.selectorVersion !== AI_PICK_SELECTOR_VERSION) return false;

  // The detailed-review version always writes these labeled sections into the
  // saved arrays. Requiring them prevents an older generic WEB_REVIEWED row from
  // blocking a new review after the prompt/schema changes.
  const why = pick.whySelected.map((item) => item.toLowerCase());
  const hasStartingPitching = why.some((item) => item.startsWith("starting pitching:"));
  const hasBullpen = why.some((item) => item.startsWith("bullpen:"));
  const hasRecentForm = why.some((item) => item.startsWith("recent form:"));
  const hasDetailedSummary = sanitizeAiPublicText(pick.researchSummary).length >= 120;
  const hasVerdict = sanitizeAiPublicText(pick.verdict).length >= 30;

  return hasStartingPitching && hasBullpen && hasRecentForm && hasDetailedSummary && hasVerdict;
}
...
async function buildAiPickSelector(args: {
  ...
  // A game is fully reviewed only when every candidate currently produced for
  // that game has a complete review from this selector version. Using "any
  // reviewed row in the game" could strand another candidate on generic text.
    ...
    [...currentCandidateIdsByGame.entries()]
      .filter(([, candidateIds]) =>
        ...
        candidateIds.every((candidateId) => {
          ...
          return Boolean(storedPick && aiStoredReviewIsCurrentAndComplete(storedPick));
        }),
      )
      .map(([gameKey]) => gameKey),
  );
  ...
  const targetCandidates = candidates.filter((candidate) => {
    ...
    // Never send an already-published final selection through another AI review.
    // Rejected/incomplete candidates may still retry when the game requires it.
    return !storedPick || !aiStoredFinalSelectionIsLocked(storedPick);
  });
  ...
  if (targetCandidates.length) {
    ...
    await persistAiPickRows(workingStoredRows, finalTargetPicks);
  ...
  const resultChanged = withResults.some((pick, index) => {
  ...
  if (resultChanged) {
    await persistAiPickRows(workingStoredRows, withResults);
  }
