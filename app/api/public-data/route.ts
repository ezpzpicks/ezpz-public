...
function movementForSplit(
  ...
  // Price movement on the selected side is the primary RLM/confirmation signal.
  // Example: Over bets 19% -> 28% and odds -102 -> -118 means both ticket
  // share and selected-side implied probability increased, so that is
  // confirmation even if the posted total also moved down by 0.5 runs.
  // Only fall back to the total-line direction when the juice/price move is
  // too small to be meaningful. This prevents a total-line move from
  // incorrectly overriding a clear same-direction odds move.
  if (openingImpliedPct != null && currentImpliedPct != null) {
