...
function nameTokens(value: unknown) {
  return normalizeText(value)
    .split(" ")
    .filter((token) => token.length >= 2 && !["over", "under", "lean", "strong", "line", "so", "k", "ks", "pitcher", "strikeouts"].includes(token));
}

function namesLikelyMatch(a: unknown, b: unknown) {
  const aText = normalizeText(a);
  const bText = normalizeText(b);
  if (!aText || !bText) return false;
  if (aText.includes(bText) || bText.includes(aText)) return true;

  const aTokens = nameTokens(aText);
  const bTokens = nameTokens(bText);
  if (!aTokens.length || !bTokens.length) return false;

  const aSet = new Set(aTokens);
  const overlap = bTokens.filter((token) => aSet.has(token));

  // Pitcher names can appear as "First Last" on the slate and "Last, First" in bet_tracker.
  // Requiring two shared name tokens catches both formats without needing exact full-string equality.
  if (overlap.length >= 2) return true;

  // Fallback for rare one-token names/abbreviations.
  return overlap.length >= 1 && (aTokens.length === 1 || bTokens.length === 1);
}
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  if (["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"].includes(playType) || market === "PITCHER STRIKEOUTS") {
    const rowLooksLikeK = market === "PITCHER STRIKEOUTS" || ["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"].includes(betType);
    if (!rowLooksLikeK) return false;

    const playPitcher = play.play;
    ...
    return namesLikelyMatch(playPitcher, rowPitcher) || namesLikelyMatch(playPitcher, rowHaystack);
  }
...
export async function GET() {
  try {
    ...
    // Send tracker rows to the frontend with Date normalized to the same M/D/YYYY format as `today`.
    // This lets the page count Favorite Pick = TRUE rows even when Google Sheets stores dates as YYYY-MM-DD.
    const publicTrackerRows = trackerRaw.map((row) => ({
      ...row,
      Date: normalizeDate(row["Date"] || row["date"] || row["Bet Date"] || ""),
    }));

    const completedTrackerRows = publicTrackerRows.filter((row) => String(row["Result"] || "").trim());
    ...
    const pendingGreen = publicTrackerRows.filter((row) => {
      const result = String(row["Result"] || "").trim();
      return !result && isGreenType(row["Bet Type"] || row["Market"]);
    }).length;

    const bestPlays = buildBestPlaysFromSlate(slateTodayRaw, today, publicTrackerRows);
    ...
    return NextResponse.json({
      ...
      bestPlays,
      slateToday,
      betTrackerRows: publicTrackerRows,
