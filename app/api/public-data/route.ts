...
function aiResponseOutputTextPieces(payload: any) {
  const pieces: string[] = [];
...
function aiJsonTextCandidates(payload: any) {
  const pieces = aiResponseOutputTextPieces(payload);
  const candidates: string[] = [];

  // Prefer the final individual assistant text block. With tool-enabled Responses,
  // concatenating multiple output_text blocks can turn otherwise-valid JSON into
  // an invalid document. Work backwards because the final assistant message is
  // the structured review we want.
  for (const piece of [...pieces].reverse()) candidates.push(piece);

  // Keep the top-level helper text only as a fallback. Some Responses payloads
  // expose output_text directly while others require walking output/content.
...
function aiNormalizeStructuredJsonText(raw: string) {
  let text = String(raw || "").trim();
  if (!text) return "";

  // Defensive only: strict Structured Outputs should already be bare JSON, but
  // this lets us recover if a model/provider ever wraps it in a markdown fence.
  text = text
    .replace(/^```(?:json)?\s*/i, "")
    .replace(/\s*```$/i, "")
    .trim();
  ...
  if (firstObject >= 0 && lastObject > firstObject) {
    text = text.slice(firstObject, lastObject + 1).trim();
  }
  return text;
}

function aiParseStructuredGameReview(payload: any): AiGameExternalReview {
  const candidates = aiJsonTextCandidates(payload);
  if (!candidates.length) {
    throw new Error("OpenAI selector returned no structured game review");
  }

  let lastError = "unknown JSON parse error";
  for (const candidate of candidates) {
    const normalized = aiNormalizeStructuredJsonText(candidate);
    if (!normalized) continue;
    try {
      return JSON.parse(normalized) as AiGameExternalReview;
    } catch (error) {
  ...
  throw new Error(
    `OpenAI selector returned invalid structured game JSON (${String(payload?.status || "unknown")}): ${lastError}`,
  );
}
...
Keep selectionComparison and finalVerdict direct and wager-specific. Do not include URLs, citations, source labels, domains, markdown, or generic filler. Return only the required JSON object.`,
  ...
  const parsed = aiParseStructuredGameReview(payload);
...
async function requestSingleAiGameExternalReviews(
  ...
  const request = (async () => {
    try {
      ...
      const hitOutputLimit = /incomplete\s*\(\s*max_output_tokens\s*\)/i.test(message);
      const invalidStructuredJson = /invalid structured game JSON/i.test(message);

      // Keep normal calls at one request. Retry only recoverable transport/output
      // failures: token truncation gets a larger ceiling, while malformed JSON
      // gets one same-size retry. The second failure is allowed to surface so we
      // never create an unbounded/costly retry loop.
      if (!hitOutputLimit && !invalidStructuredJson) {
        throw error;
      }
      if (invalidStructuredJson) {
        return fetchSingleAiGameExternalReviews(candidates);
      }
...
async function buildAiPickSelector(args: {
  ...
  // retry. A recorded REVIEW_ERROR is non-terminal and may recover after the
  // short error cool-down, provided the game has not started.
  ...
  // rows, review errors, and unconfigured rows can still recover pregame, but
  // selector-version changes alone do not reopen a completed final decision.
