...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v8-first-inning-direction-safe";
...
First-inning terminology is literal and non-negotiable. YRFI means Yes Run First Inning: at least one run is expected in the first inning, so it is a pro-scoring signal. NRFI means No Run First Inning, so it is an anti-scoring signal. ELITE YRFI conflicts with a full-game Under and must never be described as favoring a low-scoring start or supporting an Under. ELITE NRFI conflicts with a full-game Over and must never be described as supporting an Over. Even when the directions align, a one-inning signal is context only and is not sufficient proof of a nine-inning total. If firstInningSignal.relationshipToCandidate is CONFLICTS, set approved to false and do not cite that signal positively.
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    const plusMoneyMoneyline =
      candidate.market === "Moneyline" && parseAmericanOdds(candidate.odds) > 0;
    // A plus-money underdog does not need to win 55-57% of the time to be a
    // valuable wager. Grade those plays against the market price instead:
    // require an elite selector score and a meaningful probability advantage.
    // Other markets retain the lower thresholds chosen for the selector.
    const selectionThresholds = candidate.market === "Pitcher Strikeouts"
      ? { score: 86, probability: 55, advantage: 0.5, enforceProbability: true }
      : plusMoneyMoneyline
        ? { score: 80, probability: 0, advantage: 2.5, enforceProbability: false }
        : { score: 74, probability: 55, advantage: 1.5, enforceProbability: true };
  ...
  // Grade every qualifying market independently. Same-game moneylines and
  // totals can both remain pending and receive their own final review.
