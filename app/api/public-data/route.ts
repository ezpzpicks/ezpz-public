...
function favoriteField(row: SheetRow, keys: string[]) {
  return firstValue(row, keys);
}

function trackerGameMatches(row: SheetRow, play: Play) {
  const rowGame = normalizeText(row["Game"] || row["Game Label"] || row["Matchup"] || "");
  const playGame = normalizeText(play.game);
  if (!rowGame || !playGame) return true;
  return rowGame === playGame || rowGame.includes(playGame) || playGame.includes(rowGame);
}
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  if (normalizeDate(row["Date"] || row["date"] || row["Bet Date"]) !== today) return false;
  if (!isFavoriteValue(favoriteField(row, ["Favorite Pick", "Handpicked", "Handpicked Pick", "Favorite"]))) return false;

  const rowType = normalizeType(row["Bet Type"] || row["Market"] || row["Type"] || "");
  const playType = normalizeType(play.playType);
  if (rowType && rowType !== playType) return false;
  if (!trackerGameMatches(row, play)) return false;

  const selection = trackerSelectionText(row);
  if (!selection) return true;

  if (playType.includes("MONEYLINE")) {
    const team = normalizeText(play.playerTeam || play.play);
    return !team || selection.includes(team) || team.includes(selection);
  }

  if (playType.includes("NRFI") || playType === "YRFI") {
    const firstInningPick = normalizeText(play.play);
    return selection.includes(firstInningPick) || selection.includes(playType.toLowerCase());
  }

  const playName = normalizeText(play.play);
  return !playName || selection.includes(playName) || playName.includes(selection);
}

function attachFavoriteMetadata(play: Play, trackerRows: SheetRow[], today: string): Play {
  const match = trackerRows.find((row) => trackerRowMatchesPlay(row, play, today));
  if (!match) return play;

  return {
    ...play,
    favoritePick: favoriteField(match, ["Favorite Pick", "Handpicked", "Handpicked Pick", "Favorite"]),
    favoriteRank: favoriteField(match, ["Favorite Rank", "Handpicked Rank", "Rank"]),
    favoriteTag: favoriteField(match, ["Favorite Tag", "Handpicked Tag", "Tag"]),
    favoriteNotes: favoriteField(match, ["Favorite Notes", "Handpicked Notes", "Notes"]),
  };
}
...
function buildBestPlaysFromSlate(rows: SheetRow[], today: string, trackerRows: SheetRow[] = []): Play[] {
  ...
  return plays
    .map((play) => attachFavoriteMetadata(play, trackerRows, today))
