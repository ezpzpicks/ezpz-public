...
function mergeDraftKingsPayload(
  ...
  current.splits.forEach((row) =>
    splitMap.set(draftKingsSplitKey(row), {
      ...row,
      retained: false,
  ...
  current.props.forEach((row) =>
    propMap.set(draftKingsPropKey(row), {
      ...row,
      retained: false,
      lastSeenAt: current.updatedAt,
    }),
...
function snapshotPayloadFromRows(rows: SheetRow[], today: string): DraftKingsPayload {
  ...
  for (const row of rows) {
    ...
    splits.push({
      ...
      betsPct,
      moneyPct,
      gapPct: toNumber(row["Public Gap %"]) || warning.gapPct,
      warning: String(row.Warning || warning.warning),
      warningNegative: truthyValue(row["Warning Negative"]) || warning.warningNegative,
      retained: true,
      lastSeenAt: snapshotTime,
    });
  }
...
export async function GET() {
  try {
    ...
    const persistence = await persistFinalPregameDraftKings(liveDraftKings, today);
