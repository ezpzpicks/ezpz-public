...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v6-premium-edge";
...
function aiCandidateFromBestPlay(
  ...
  // Pitcher strikeout props enter the AI selector at a neutral 85 baseline.
  // Their underlying K-model score/probability are still supplied to the final
  // review as evidence; 85 is only the AI-selection starting grade.
  const underlyingModelScore = parseScore(play.score);
  const modelScore = identity.market === "Pitcher Strikeouts" ? 85 : underlyingModelScore;
...
function aiCandidateResearchPayload(candidate: AiSelectorCandidate, compact = false) {
  const maxFields = compact ? 50 : 80;
  const maxValueLength = compact ? 300 : 500;
  return {
    ...
    existingWhy: candidate.whySelected.slice(0, compact ? 4 : 8),
...
function aiExternalReviewSchema(compact = false) {
  const long = compact ? 420 : 700;
  const medium = compact ? 300 : 500;
  return {
    type: "object",
    ...
    properties: {
      ...
      criticalConflictReason: { type: "string", maxLength: 260 },
      confidenceReason: {
        type: "array",
      ...
      why: {
        type: "array",
        items: { type: "string", maxLength: compact ? 240 : 320 },
        minItems: 3,
        maxItems: compact ? 7 : 10,
      },
      historicalNotes: {
        type: "array",
      ...
      researchSummary: { type: "string", maxLength: compact ? 700 : 1100 },
      verdict: { type: "string", maxLength: 420 },
      contextSummary: { type: "string", maxLength: 320 },
      startingPitching: { type: "string", minLength: 40, maxLength: long },
      bullpenAnalysis: { type: "string", minLength: 30, maxLength: medium },
      recentTeamForm: { type: "string", minLength: 30, maxLength: medium },
      historicalMatchup: { type: "string", minLength: 20, maxLength: medium },
      selectionComparison: { type: "string", minLength: 30, maxLength: medium },
      finalVerdict: { type: "string", minLength: 30, maxLength: 420 },
    },
  };
}
...
function aiExternalReviewIsComplete(review: AiExternalReview, candidateId: string) {
  return (
    ...
    review.startingPitching.length >= 40 &&
    review.bullpenAnalysis.length >= 30 &&
    review.recentTeamForm.length >= 30 &&
    review.historicalMatchup.length >= 20 &&
    review.selectionComparison.length >= 30 &&
    review.finalVerdict.length >= 30 &&
    review.researchSummary.length >= 80 &&
    review.why.length >= 3
  );
}

async function requestSingleAiExternalReview(
  candidate: AiSelectorCandidate,
  compact = false,
): Promise<AiExternalReview> {
  ...
  const searchContextSize: "low" | "medium" | "high" = ["low", "medium", "high"].includes(
    ...
    : "low";
  const configuredMaxRaw = Number(process.env.EZPZ_AI_MAX_OUTPUT_TOKENS || 3000);
  const configuredMax = Number.isFinite(configuredMaxRaw) ? configuredMaxRaw : 3000;
  const maxOutputTokens = compact
    ? Math.max(1800, Math.min(2400, configuredMax))
    : Math.max(2400, Math.min(4000, configuredMax));
  const configuredTimeoutRaw = Number(process.env.EZPZ_AI_REQUEST_TIMEOUT_MS || 55_000);
  const requestTimeoutMs = Number.isFinite(configuredTimeoutRaw)
    ? Math.max(15_000, Math.min(90_000, configuredTimeoutRaw))
    : 55_000;
  const candidatePayload = aiCandidateResearchPayload(candidate, compact);
  const schema = aiExternalReviewSchema(compact);
  const compactInstruction = compact
    ? "This is a retry after an incomplete response. Be concise. Use 1-3 sentences per narrative field and no more than seven WHY bullets, while still completing every required field."
    : "Use concise, specific analysis so the complete structured response finishes within the token budget.";
  ...
  try {
    response = await fetch("https://api.openai.com/v1/responses", {
      ...
      model,
      store: false,
      prompt_cache_key: `ezpz-ai-selector:${AI_PICK_SELECTOR_VERSION}:${compact ? "compact" : "full"}`,
      tools: [
        {
          type: "web_search",
      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: `You are the final MLB research analyst for EZPZ AI Picks. Research only information verifiably available before this game starts. Analyze this one candidate independently.

The public explanation must read like a specific MLB matchup report. Begin with the actual probable or confirmed starting pitchers and the key players who directly affect the wager. For a pitcher prop, center the report on that pitcher and the opposing projected lineup. For a moneyline or total, explain how both starters, the lineups, and the bullpens create the edge. For NRFI/YRFI, emphasize each starter's first-inning profile and the top-of-order hitters.

Use the supplied modelGameContext first. Verify and deepen the strongest relevant items: recent performance over the last 3-5 starts; opponent and batter history with sample context; meaningful home/road, day/night, handedness and pitch-mix splits; projected lineup strikeout/contact tendencies; workload, rest, role and expected leash; and recent bullpen availability. Do not invent statistics. If a requested split or history sample is unavailable or too small, state that it is not meaningful.

For a Pitcher Strikeouts candidate, treat the supplied AI score of 85 as a neutral selector baseline, not an 85% win probability and not proof that the wager should be selected. Independently pressure-test the prop before adjusting it. Specifically answer through your analysis: (1) Is the K-model projection meaningfully separated from the sportsbook line? (2) Does the expected opposing lineup support the Over/Under, emphasizing individual hitters and handedness-specific strikeout tendencies? (3) Does the pitcher's arsenal/pitch mix create favorable or unfavorable whiff matchups against this lineup? (4) Do the pitcher's last 3-5 starts support the current strikeout skill without overreacting to a tiny sample? (5) Are expected innings, pitch count, batters faced, rest and leash sufficient for the prop line? (6) Is opponent or batter history meaningful after accounting for sample size and current roster? (7) Does today's actual/projected lineup materially differ from the generic team strikeout profile? (8) Does the supplied running and recent record for this exact EZPZ K grade support or weaken confidence? (9) Is there any evidence that makes the model projection misleading? (10) After all of that, does this prop deserve to beat the 85 baseline and compete with the other AI candidates? A negative net assessment must set approved to false, lower the score and exclude the prop; a positive, evidence-backed assessment may set approved to true and raise it. Do not reward a prop merely because its model grade is Strong Over/Over/Lean Over/Strong Under/Under/Lean Under.

Fully populate every required field. Set approved to true only when the completed research supports publishing this wager as an EZPZ AI Pick. Set approved to false when the verified matchup, lineup, workload, form, market or historical evidence produces a materially negative net assessment, even if the supplied pre-review AI score is 100. approved=false is a final veto and is not reserved for critical conflicts. Neutral research may return approved=true with an adjustment of 0 when the supplied quantitative case remains sound. Do not reject a wager merely because an optional split or small-sample history could not be verified. startingPitching must name the relevant starter or prop pitcher. bullpenAnalysis must describe recent availability or explicitly say that no material workload concern was verified. recentTeamForm must summarize meaningful recent performance. historicalMatchup must include sample context or state that no meaningful sample was found. selectionComparison must explain why this wager should be approved or rejected using the supplied score, probability, edge and matchup evidence. finalVerdict must be direct, wager-specific and consistent with approved. Never substitute generic phrases such as “no outside context changed the play” for analysis.

Spend the explanation budget on WHY this wager was approved or rejected, not on a separate risks section. The WHY section should contain 5-10 detailed, concise, non-redundant bullets when verified information supports them, with at least three player-specific bullets when available. Prioritize starting pitching or the prop pitcher, opposing lineup matchup, recent performance, expected workload/leash, bullpen availability when relevant, recent team form, meaningful historical matchup evidence, and the specific model/market edge. Do not generate public risk bullets. Still use negative evidence internally when choosing approved, the adjustment or criticalConflict. Return only a small evidence-based adjustment from -6 to +6; use 0 when research does not change the quantitative case. Keep approved consistent with the net assessment: materially negative research must return approved=false, while approved=true means the wager still deserves publication after research. A critical conflict is reserved for evidence that invalidates a saved pregame assumption, such as a scratched starter or changed role.

Do not include URLs, markdown links, citations, domains, publication names, source labels, bracketed source markers, or phrases such as according to. Return only the required JSON object. ${compactInstruction}`,
            },
          ],
        },
        {
      ...
      text: {
        format: {
          type: "json_schema",
          name: compact ? "ezpz_ai_selector_review_compact" : "ezpz_ai_selector_review",
          strict: true,
          schema,
        },
      },
  ...
  if (!aiResponseCompletedWebSearch(payload)) {
    throw new Error("OpenAI selector did not complete the required web search");
  }

  const raw = aiResponseOutputText(payload).trim();
  if (!raw) throw new Error("OpenAI selector returned no structured review");

  let parsed: AiExternalReview;
  try {
    parsed = JSON.parse(raw) as AiExternalReview;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(
      `OpenAI selector returned invalid structured JSON (${String(payload?.status || "unknown")}): ${message}`,
    );
  }

  const review = sanitizeAiExternalReview(parsed);
  if (!aiExternalReviewIsComplete(review, candidate.candidateId)) {
    throw new Error("OpenAI selector returned an incomplete required review");
  }
  return review;
}

function aiExternalReviewErrorIsRetryable(error: unknown) {
  const message = (error instanceof Error ? error.message : String(error)).toLowerCase();
  return (
    message.includes("max_output_tokens") ||
    message.includes("incomplete") ||
    message.includes("invalid structured json") ||
    message.includes("no structured review") ||
    message.includes("required web search") ||
...
async function requestAiExternalReviews(
  ...
  const configuredConcurrency = Number(process.env.EZPZ_AI_REVIEW_CONCURRENCY || 2);
  const concurrency = Number.isFinite(configuredConcurrency)
    ? Math.max(1, Math.min(3, Math.floor(configuredConcurrency)))
    : 2;
  let nextIndex = 0;
  ...
  const worker = async () => {
    while (true) {
      ...
      if (!candidate) return;
      try {
        let review: AiExternalReview;
        try {
          review = await requestSingleAiExternalReview(candidate, false);
        } catch (firstError) {
          const firstMessage = firstError instanceof Error ? firstError.message : String(firstError);
          if (!aiExternalReviewErrorIsRetryable(firstError)) throw firstError;
          // One compact retry is intentionally limited to the individual play. A
          // truncated response can no longer consume the token budget for every
          // other candidate in the slate. Authentication and request-shape errors
          // are not retried because a second identical request would only add cost.
          try {
            review = await requestSingleAiExternalReview(candidate, true);
          } catch (retryError) {
            const retryMessage = retryError instanceof Error ? retryError.message : String(retryError);
            throw new Error(`${firstMessage}; compact retry failed: ${retryMessage}`);
          }
        }
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    const selectionThresholds = candidate.market === "Pitcher Strikeouts"
      ? { score: 88, probability: 58, advantage: 2.5 }
      : { score: 80, probability: 57, advantage: 2.5 };
    const thresholdFailure =
      aiScore < selectionThresholds.score
        ? `AI score ${aiScore} did not reach the ${selectionThresholds.score} premium-play threshold`
        : estimatedProbability < selectionThresholds.probability
          ...
          : implied && advantage < selectionThresholds.advantage
    ...
    return {
      ...
      whySelected: sanitizeAiPublicList(candidate.whySelected, 14),
  ...
  // Moneyline and full-game total bets represent competing primary exposure to
  // the same matchup. Keep only the strongest premium qualifier from that pair.
  // First-inning markets and pitcher props remain independent, so combinations
  // such as YRFI + Total Over are still allowed.
  const primaryWinnerByGame = new Map<string, (typeof finalized)[number]>();
  for (const pick of finalized) {
    if (!pick.selected || (pick.market !== "Moneyline" && pick.market !== "Total")) continue;
    const current = primaryWinnerByGame.get(pick.gameKey);
    if (!current) {
      primaryWinnerByGame.set(pick.gameKey, pick);
      continue;
    }
    const pickIsStronger =
      pick.aiScore > current.aiScore ||
      (pick.aiScore === current.aiScore && pick.estimatedAdvantage > current.estimatedAdvantage) ||
      (pick.aiScore === current.aiScore &&
        pick.estimatedAdvantage === current.estimatedAdvantage &&
        pick.estimatedProbability > current.estimatedProbability) ||
      (pick.aiScore === current.aiScore &&
        pick.estimatedAdvantage === current.estimatedAdvantage &&
        pick.estimatedProbability === current.estimatedProbability &&
        pick.candidateId.localeCompare(current.candidateId) < 0);
    if (pickIsStronger) primaryWinnerByGame.set(pick.gameKey, pick);
  }

  const portfolioResolved = finalized.map((pick) => {
    if (!pick.selected || (pick.market !== "Moneyline" && pick.market !== "Total")) return pick;
    const winner = primaryWinnerByGame.get(pick.gameKey);
    if (!winner || winner.candidateId === pick.candidateId) return pick;
    return {
      ...pick,
      selected: false,
      protectionStatus: "BLOCKED" as const,
      rejectionReason:
        `Same-game portfolio rule kept the stronger ${winner.market.toLowerCase()} play: ` +
        `${winner.play} (AI score ${winner.aiScore})`,
    };
  });

  return portfolioResolved.map(({ slateRow, bestPlay, trendPlay, baselineProbability, scoreAdjustment, probabilityAdjustment, protectionReasons, ...pick }) => pick as AiPick);
}
...
async function buildAiPickSelector(args: {
  ...
  // PENDING_FINAL_REVIEW (for example, if the review request was interrupted).
  // Those games must remain eligible for a retry instead of being skipped forever.
  ...
  // A final decision is immutable once its review completed. Interrupted or
  // unconfigured rows remain retryable, but selector-version changes alone do
  // not reopen a pick that was already published or rejected.
  ...
  // request to enter final review. A normal public refresh inside the same
  // 7-23 minute lock window is also allowed to finalize the candidate.
  // This makes the final-review state self-healing if a scheduler run is late.
    ...
    slateRows
      .filter((row) => {
        ...
        return (
          ...
          isFifteenMinuteTrackingWindow(row) ||
  ...
  const targetCandidates = candidates.filter((candidate) => {
    ...
    const retryingInterruptedFinal = Boolean(
      storedPick?.snapshotStatus === "FINAL_PREGAME" &&
        !aiStoredFinalDecisionIsTerminal(storedPick),
    );
    return !started || retryingInterruptedFinal;
  });
    ...
    if (reviewCandidates.length) {
      try {
        ...
        for (const candidate of reviewCandidates) {
          const reviewError = external.errors.get(candidate.candidateId);
          if (reviewError) {
            candidate.risks.push(`External context review failed: ${reviewError}`);
          }
        }
      } catch (error) {
        externalStatus = "REVIEW_ERROR";
...
function isAuthorizedBackgroundSnapshotRequest(request: NextRequest) {
  const secret = String(process.env.EZPZ_SNAPSHOT_SECRET || "").trim();
  if (!secret) return true;
  const authorization = String(request.headers.get("authorization") || "").trim();
  const suppliedSecret = String(
    request.headers.get("x-ezpz-snapshot-secret") || "",
  ).trim();
  return suppliedSecret === secret || authorization === `Bearer ${secret}`;
}
