  ...
  if (headers.length) {
    ...
    if (!currentHeaders.length) {
      await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `'${escapedSheetName(tabName)}'!A1:${columnLetter(headers.length - 1)}1`,
...
function currentLiveSplitsForGame(
  row: SheetRow,
  payload: DraftKingsPayload,
  market?: DraftKingsSplit["market"],
) {
  return payload.splits.filter(
    (item) => !item.retained && sameDraftKingsGame(row, item) && (!market || item.market === market),
  );
}

function currentLivePropsForGame(row: SheetRow, payload: DraftKingsPayload) {
  return payload.props.filter((item) => !item.retained && sameDraftKingsGame(row, item));
}
...
function selectedMoneylineSplit(row: SheetRow, payload: DraftKingsPayload) {
  ...
  return (
    currentLiveSplitsForGame(row, payload, "Moneyline").find(
...
function selectedTotalSplit(row: SheetRow, payload: DraftKingsPayload) {
  ...
  if (!side) return null;
  return currentLiveSplitsForGame(row, payload, "Total").find((item) => item.side === side) || null;
}
...
function selectedRunLineSplit(row: SheetRow, payload: DraftKingsPayload) {
  ...
  if (!selectedTeam) return null;
  return (
    currentLiveSplitsForGame(row, payload, "Run Line").find(
      (item) => item.selectionTeam === selectedTeam,
    ) || null
  );
}
...
function selectedPitcherProp(summary: unknown, row: SheetRow, payload: DraftKingsPayload) {
  ...
  const options = currentLivePropsForGame(row, payload).filter(
...
function splitSlateFields(prefix: string, item: DraftKingsSplit | null, updatedAt: string) {
  ...
  return {
    ...
    [`${prefix} Public Match Confidence`]: "Final-pregame selected-side match",
    "Public Data Updated": updatedAt,
  } as SheetRow;
}
...
function propSlateFields(prefix: string, item: DraftKingsProp | null, updatedAt: string, summary: unknown) {
  ...
  return {
    ...
    [`${prefix} Prop Popularity Updated`]: updatedAt,
    [`${prefix} Prop Popularity Match Confidence`]: exactSide && exactLine
...
function trackerSplitFields(item: DraftKingsSplit, updatedAt: string) {
  return {
    ...
    "Public Split Match Confidence": "Final-pregame selected-side match",
    "Public Split Snapshot Time": updatedAt,
  } as SheetRow;
}
...
function trackerPropFields(item: DraftKingsProp, updatedAt: string, exactMatch: boolean) {
  return {
    ...
    "Prop Popularity Match Confidence": exactMatch
      ? "Final-pregame exact pitcher side/line match"
      : "Final-pregame pitcher match",
    "Prop Popularity Snapshot Time": updatedAt,
  } as SheetRow;
}

function snapshotRecordFromSplit(item: DraftKingsSplit, snapshotTime: string): SheetRow {
  return {
    ...
    "Popularity Rank": "",
    Source: "DraftKings",
    "Match Confidence": "Final-pregame live market snapshot",
    "Source URL": DK_BETTING_SPLITS_URL,
  };
}

function snapshotRecordFromProp(item: DraftKingsProp, snapshotTime: string): SheetRow {
  return {
    "Snapshot Time ET": snapshotTime,
    ...
    Source: "DraftKings",
    "Match Confidence": "Final-pregame pitcher popularity snapshot",
...
async function persistFinalPregameDraftKings(
  ...
  const todayIso = isoPublicDate(today);
  const persistenceKey = `${todayIso}|${livePayload.updatedAt}|${livePayload.splits.length}|${livePayload.props.length}`;
  if (
  ...
  if (livePayload.stale || livePayload.status === "UNAVAILABLE") {
    draftKingsPersistenceCache = { key: persistenceKey, savedAt: Date.now(), result };
    return result;
  }
  const liveSplits = livePayload.splits.filter(
    (item) => !item.retained && isoPublicDate(item.date) === todayIso,
  );
  const liveProps = livePayload.props.filter(
    (item) => !item.retained && isoPublicDate(item.date) === todayIso,
  );
  if (!liveSplits.length && !liveProps.length) {
    draftKingsPersistenceCache = { key: persistenceKey, savedAt: Date.now(), result };
    return result;
  }
  ...
  try {
    ...
    const [slateMatrix, trackerMatrix, snapshotMatrix] = await Promise.all([
    ...
    const slateByGameKey = new Map<string, SheetRow>();

    for (const matrixRow of slateMatrix.rows) {
      const row = matrixRow.object;
      if (isoPublicDate(row.Date) !== todayIso) continue;
      const gameKey = String(row["Game Key"] || "").trim();
      if (gameKey) slateByGameKey.set(gameKey, row);

      const pregame = isPregameRow(row, now);
      const matchingSplits = liveSplits.filter((item) => sameDraftKingsGame(row, item));
      const matchingProps = liveProps.filter((item) => sameDraftKingsGame(row, item));
      if (!pregame) {
      ...
      matchingSplits.forEach((item) =>
        snapshotRecords.push(snapshotRecordFromSplit(item, livePayload.updatedAt)),
      );
      matchingProps.forEach((item) =>
        snapshotRecords.push(snapshotRecordFromProp(item, livePayload.updatedAt)),
      );

      const fields: SheetRow = {
        "Public Data Status": "PREGAME LIVE",
        "Public Data Updated": livePayload.updatedAt,
    ...
    for (const matrixRow of trackerMatrix.rows) {
      const row = matrixRow.object;
      if (isoPublicDate(row.Date) !== todayIso) continue;
      const resultValue = String(row.Result || "").trim().toUpperCase();
      if (resultValue && resultValue !== "PENDING") continue;
      const slateRow = slateByGameKey.get(String(row["Game Key"] || "").trim());
      if (!slateRow || !isPregameRow(slateRow, now)) continue;

      const market = textKey(row.Market || "");
      const betType = normalizeType(row["Bet Type"] || "");
      let fields: SheetRow = {};

      if (market.includes("moneyline") || betType.includes("MONEYLINE")) {
        const targetTeam = teamFromSelection(row.Team || row.Selection || "");
        const item = currentLiveSplitsForGame(slateRow, livePayload, "Moneyline").find(
          (split) => split.selectionTeam === targetTeam,
        );
        if (item) fields = trackerSplitFields(item, livePayload.updatedAt);
      } else if (market.includes("game total") || betType.includes("TOTAL")) {
        const side = betType.includes("UNDER") ? "Under" : "Over";
        const item = currentLiveSplitsForGame(slateRow, livePayload, "Total").find(
          (split) => split.side === side,
        );
        if (item) fields = trackerSplitFields(item, livePayload.updatedAt);
      } else if (market.includes("pitcher strikeout")) {
        const pitcher = textKey(extractPitcherFromSelection(row.Selection || ""));
        const props = currentLivePropsForGame(slateRow, livePayload).filter(
          (item) => textKey(item.pitcher) === pitcher,
        );
        if (props.length) {
    ...
    if (snapshotRecords.length) {
      ...
      snapshotRecords.forEach((row) => snapshotMap.set(snapshotRecordKey(row), row));
    ...
    result.status =
      snapshotRecords.length || slateUpdates.length || trackerUpdates.length
        ? "SAVED"
        : "NO_CHANGES";
    draftKingsPersistenceCache = { key: persistenceKey, savedAt: Date.now(), result };
    return result;
...
function trackerTotalSide(row: SheetRow): "Over" | "Under" | "" {
  ...
  if (type.includes("UNDER") || selection.startsWith("under")) return "Under";
  if (type.includes("OVER") || selection.startsWith("over")) return "Over";
  return "";
}
...
function buildDraftKingsSignalRows(
  ...
  for (const row of completedRows) {
    ...
    const common = {
      ...
      sideGroup,
      betType: normalizeType(row["Bet Type"] || row.Market || ""),
      modelVersion: String(row["Model Version"] || ""),
      qualified: isGreenType(row["Bet Type"] || row.Market || ""),
      result,
      odds,
      units: unitsFromResult(row),
    };
    ...
    const movementSignal = String(snapshot?.["Line Movement Signal"] || "").trim();
    if (movementSignal) {
      const toneRaw = String(snapshot?.["Line Movement Tone"] || "").trim();
...
export async function GET() {
  try {
    ...
    const persistence = await persistFinalPregameDraftKings(draftKings, today);
