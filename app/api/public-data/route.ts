...
function buildTrendRecordRows(completedRows: SheetRow[]): TrendRecordResult[] {
  ...
  for (const row of completedRows) {
    ...
    // The saved row is the official pregame record. Never re-run the trend
    // formula after the game. Prefer the dedicated frozen sheet columns because
    // they are exactly what was displayed at the final snapshot; the JSON is
    ...
    const frozenScore = Number.isFinite(rowScore)
      ? rowScore
    ...
    const selection = String(row.Selection || row.Team || row.Pick || storedPlay?.selection || "");
    const side = market === "Total" ? trackerTotalSide(row) : "";
    const sideGroup: TrendPlay["sideGroup"] = market === "Total"
      ? side
      : odds < 0
        ? "Favorite"
        : odds > 0
    ...
    const officialPlay: TrendPlay = storedPlay
      ? {
          ...storedPlay,
          score: frozenScore,
          tier: frozenTier,
