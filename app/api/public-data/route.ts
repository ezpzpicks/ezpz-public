...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  // This stops a Mets NRFI tracker row from marking a Mets pitcher prop as handpicked.
  if (playIsMoneyline) {
    return rowIsMoneyline && Boolean(selectionNorm) && (selectionNorm === playerTeamText || playText.includes(selectionNorm) || playerTeamText.includes(selectionNorm));
  }

  if (playIsNrfi) {
    return rowIsNrfi && (selectionNorm === gameText || gameText.includes(selectionNorm) || betType === playType);
  }
...
function applyFavoriteInfoToPlays(plays: Play[], trackerRows: SheetRow[], today: string) {
  return plays.map((play) => {
    const trueMatch = trackerRows.find((row) => trackerRowMatchesPlay(row, play, today) && isTrueValue(favoriteValue(row)));
    if (!trueMatch) {
      return {
        ...play,
        favoritePick: "",
    ...
    return {
      ...play,
      favoritePick: "TRUE",
      favoriteRank: firstValue(trueMatch, ["Favorite Rank", "Handpicked Rank", "Rank"]),
      favoriteTag: firstValue(trueMatch, ["Favorite Tag", "Handpicked Tag", "Tag"]),
      favoriteNotes: firstValue(trueMatch, ["Favorite Notes", "Handpicked Notes", "Notes"]),
    };
  });
}
