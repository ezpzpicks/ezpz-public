...
async function persistFinalPregameDraftKings(
  ...
  try {
    ...
    for (const matrixRow of trendMatrix.rows) {
      ...
      const storedTrendPlay = parseStoredTrendPlay(row);
      const storedTrendIsFinal =
        storedTrendPlay?.snapshotStatus === "FINAL_PREGAME" &&
        Boolean(storedTrendPlay.frozenAt) &&
        storedTrendPlay.baseScore != null;

      if (alreadyLocked) {
        // Normalize legacy snapshots that saved only the pre-comparison score.
        // The signal records inside Trend Score Details were already frozen, so
        // this backfill derives the final ranking without using newer results.
        if (!storedTrendIsFinal) {
          const normalizedFrozenPlay =
          ...
          if (normalizedFrozenPlay) {
            trendUpdates.push({
              sheetRow: matrixRow.sheetRow,
              fields: trendSnapshotFields(normalizedFrozenPlay),
            });
          }
        }
        continue;
...
function buildTrendRecordRows(completedRows: SheetRow[]): TrendRecordResult[] {
  type FrozenRow = {
    row: SheetRow;
    market: "Moneyline" | "Total";
    result: "W" | "L" | "P";
  ...
  for (const row of completedRows) {
    const market = trackerMarket(row);
    const result = resultCode(row.Result);
    const finalSnapshot = textKey(
      row["Public Split Match Confidence"] || "",
    ).includes("15 minute tracking snapshot");
    if (!market || !result || !finalSnapshot) continue;

    const play = parseStoredTrendPlay(row);
    if (!play) continue;

    frozenRows.push({ row, market, result, play, finalSnapshot });
  }

  // New snapshots already contain the final head-to-head score and tier in the
  // stored JSON. A small number of snapshots locked before that metadata was
  // introduced contain only the frozen pre-comparison score. Recover those
  // legacy rows from their own frozen pregame details only. No live data,
  // postgame result, or newer trend history is used in this migration fallback.
  const grouped = new Map<string, FrozenRow[]>();
  for (const item of frozenRows) {
    const date = isoPublicDate(item.row.Date || "");
    const savedGameKey = String(item.row["Game Key"] || "")
      .trim()
      .replace(/\.0$/, "");
    const matchupKey = `${normalizeTeam(item.play.awayTeam)}|${normalizeTeam(
      item.play.homeTeam,
    )}`;
    const gameTime = String(item.row["Game Time"] || "").trim();
    const groupKey = `${date}|${
      savedGameKey || `${matchupKey}|${textKey(gameTime || item.row.Game || "")}`
    }`;
  ...
  for (const items of grouped.values()) {
    const allAlreadyFinal = items.every(
      ({ play }) =>
        play.snapshotStatus === "FINAL_PREGAME" &&
        Boolean(play.frozenAt) &&
        play.baseScore != null,
    );
    const scoredPlays = allAlreadyFinal
      ? items.map(({ play }) => play)
      : scoreHeadToHeadTrendPlays(items.map(({ play }) => play));
    const scoredBySelection = new Map(
      scoredPlays.map((play) => [
        `${trendMarketComparisonKey(play)}|${trendSideComparisonKey(play)}`,
        play,
      ]),
    );

    for (const item of items) {
      const selectionKey = `${trendMarketComparisonKey(
        item.play,
      )}|${trendSideComparisonKey(item.play)}`;
      const scoredPlay = scoredBySelection.get(selectionKey) || item.play;

      // The dedicated final-snapshot marker is the authoritative proof that the
      // row was frozen pregame. Do not discard a valid frozen row merely because
      // it predates the newer JSON metadata fields.
      const rowTier = String(item.row["Trend Tier"] || "").trim();
      const recoveredTier = scoredPlay.tier;
      const frozenTier =
        recoveredTier === "Good" ||
        recoveredTier === "Strong" ||
        recoveredTier === "Elite"
          ? recoveredTier
          : rowTier === "Good" || rowTier === "Strong" || rowTier === "Elite"
            ? rowTier
            : null;
      if (!frozenTier) continue;

      const rowScore = Number(item.row["Trend Score"]);
      const recoveredScore = Number(scoredPlay.score);
      const frozenScore = Number.isFinite(recoveredScore)
        ? recoveredScore
      ...
      const frozenAt = String(
        scoredPlay.frozenAt ||
          item.row["Public Split Snapshot Time"] ||
          scoredPlay.updatedAt ||
          "",
      ).trim();
      const odds = parseAmericanOdds(
        item.row["Public Split Odds"] ||
          item.row.Odds ||
          item.row["Odds/Line"],
      );

      output.push({
        date: isoPublicDate(item.row.Date || ""),
        game: String(item.row.Game || scoredPlay.game || ""),
        gameKey: String(item.row["Game Key"] || "").trim().replace(/\.0$/, ""),
        gameTime: String(item.row["Game Time"] || "").trim(),
        market: item.market,
        selection: String(
          item.row.Selection || item.row.Team || item.row.Pick || scoredPlay.selection || "",
        ),
        result: item.result,
        odds,
        units:
        ...
        frozenAt,
        snapshotStatus: "FINAL_PREGAME",
        trendScoreDetails: JSON.stringify({
          ...scoredPlay,
          score: frozenScore,
          tier: frozenTier,
          frozenAt,
...
function scoreHeadToHeadTrendPlays(plays: TrendPlay[]) {
  return plays.map((play) => {
    const playBaseScore = play.baseScore ?? play.score;
    const sameGameMarket = plays.filter(
      (candidate) =>
        trendGameComparisonKey(candidate) === trendGameComparisonKey(play) &&
        trendMarketComparisonKey(candidate) === trendMarketComparisonKey(play),
    );
    const sideKey = trendSideComparisonKey(play);
    const opponent = sameGameMarket
      .filter((candidate) => trendSideComparisonKey(candidate) !== sideKey)
      .sort(
        (a, b) =>
          (b.baseScore ?? b.score) - (a.baseScore ?? a.score),
      )[0];

    if (!opponent) {
      return {
        ...play,
        baseScore: playBaseScore,
        opponentScore: null,
        comparisonGap: 0,
    ...
    const opponentBaseScore = opponent.baseScore ?? opponent.score;
    const rawGap = playBaseScore - opponentBaseScore;
    const comparisonGap = Math.abs(rawGap);
    const comparisonWinner = rawGap > 0.01;
    const eligible =
      comparisonWinner && trendPlayHasData(play) && trendPlayHasData(opponent);
    const winnerScore = clampScore(
      50 + comparisonGap + (playBaseScore - 50) * 0.5,
    );
    const loserScore = Math.min(
      59,
      clampScore(50 - comparisonGap + (playBaseScore - 50) * 0.25),
    );
    const finalScore = Math.round(eligible ? winnerScore : loserScore);
...
function frozenTrendPlaysFromRows(
  ...
  const parsedRows = rows
    .filter((row) => isoPublicDate(row.Date || "") === isoPublicDate(referenceDate))
    .filter((row) =>
      textKey(row["Public Split Match Confidence"] || "").includes(
        "15 minute tracking snapshot",
      ),
    )
    .map((row) => parseStoredTrendPlay(row))
    .filter((play): play is TrendPlay => Boolean(play));

  const byGame = new Map<string, TrendPlay[]>();
  for (const play of parsedRows) {
    const key = trendGameComparisonKey(play);
    const current = byGame.get(key) || [];
    current.push(play);
    byGame.set(key, current);
  ...
  for (const gamePlays of byGame.values()) {
    // New snapshots already store the final head-to-head score. Legacy snapshots
    // stored the pre-comparison score, but their signal records were still frozen.
    // Re-score those legacy rows once from the frozen records, never from live history.
...
export async function GET(request: NextRequest) {
  try {
    ...
    const trendRecordRows = buildTrendRecordRows(completedAllGameTrendRows);
    ...
    const frozenTrendPlays = frozenTrendPlaysFromRows(
      allGameTrendRows,
      today,
    );
