...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v14-model-scores-5pct-edge";
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    // Pitcher props now enter with their differentiated strikeout-model score.
    // The live snapshot still needs 85+, while the final snapshot needs 86+,
    // the probability/value gates, and an affirmative external review.
    // Market-specific score/probability thresholds remain in place, while
    // every AI market now shares the same 5% minimum estimated advantage.
    ...
    const livePitcherReviewNote =
      livePitcherStrikeoutPreview && preliminarySelected
        ? `Pending final strikeout review: final selection still requires AI score 86+, estimated probability 55%+, estimated advantage ${AI_MINIMUM_ESTIMATED_ADVANTAGE.toFixed(1)}%+, and final AI approval.`
        : "";
    return {
      ...candidate,
      aiScore,
