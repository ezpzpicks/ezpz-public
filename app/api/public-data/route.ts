...
function favoriteValue(row: SheetRow) {
  // Only the actual Favorite Pick column controls whether a play is handpicked.
  // Do not fall back to tags/notes/other columns, because those can badge unrelated same-game plays.
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  if (playIsNrfi) {
    return rowIsNrfi && Boolean(selectionNorm) && Boolean(gameText) && (selectionNorm === gameText || gameText.includes(selectionNorm) || selectionNorm.includes(gameText));
  }
...
function applyFavoriteInfoToPlays(plays: Play[], trackerRows: SheetRow[], today: string) {
  // One TRUE tracker row should create one handpicked badge, not badge every similar play.
  ...
  return plays.map((play) => {
    const match = favoriteRows.find(({ row, index }) => !usedIndexes.has(index) && trackerRowMatchesPlay(row, play, today));
...
export async function GET() {
  try {
    ...
    const handpickedCompletedRows = completedTrackerRows.filter((row) => isFavoriteRow(row));
