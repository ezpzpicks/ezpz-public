...
function stripBetWords(value: unknown) {
  return String(value ?? "")
    .replace(/\bSTRONG\s+OVER\b/gi, "")
    .replace(/\bLEAN\s+OVER\b/gi, "")
    .replace(/\bSTRONG\s+UNDER\b/gi, "")
    .replace(/\bLEAN\s+UNDER\b/gi, "")
    .replace(/\bOVER\b/gi, "")
    .replace(/\bUNDER\b/gi, "")
    .replace(/\bPITCHER\s+STRIKEOUTS\b/gi, "")
    .replace(/\bLINE\b.*$/gi, "")
    .replace(/[0-9]+(?:\.[0-9]+)?/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function pitcherNameVariants(value: unknown) {
  const base = stripBetWords(value);
  const variants = new Set<string>();

  const add = (v: unknown) => {
    const normalized = normalizeText(stripBetWords(v));
...
function anyNameMatch(aValues: unknown[], bValues: unknown[]) {
  const aVariants = aValues.flatMap(pitcherNameVariants);
  const bVariants = bValues.flatMap(pitcherNameVariants);
  return aVariants.some((a) => bVariants.some((b) => a === b || a.includes(b) || b.includes(a)));
}

function isCompletedResult(value: unknown) {
  const result = String(value ?? "").trim().toUpperCase();
  return ["WIN", "W", "LOSS", "L", "PUSH", "P"].includes(result) || result.includes("WIN") || result.includes("LOSS") || result.includes("PUSH");
}
...
function trackerRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  ...
  if (["OVER", "UNDER", "LEAN OVER", "LEAN UNDER", "STRONG OVER", "STRONG UNDER"].includes(playType) || market === "PITCHER STRIKEOUTS") {
    const rowPitcherSources = [
      ...
      row["Pitcher"],
      row["Name"],
      extractPitcherFromSelection(selection),
    ];
    const playPitcherSources = [play.play];
    return market === "PITCHER STRIKEOUTS" && anyNameMatch(rowPitcherSources, playPitcherSources);
  }
...
export async function GET() {
  try {
    ...
    const completedTrackerRows = trackerRaw.filter((row) => isCompletedResult(row["Result"]));
    const qualifiedTrackerRows = completedTrackerRows.filter((row) => isGreenType(row["Bet Type"] || row["Market"]));
    const allHandpickedRows = trackerRaw.filter((row) => isTrueValue(row["Favorite Pick"] || row["Handpicked"] || row["Favorite"]));
    const todayHandpickedRows = allHandpickedRows.filter((row) => normalizeDate(row["Date"] || row["date"] || "") === today);
    ...
    return NextResponse.json({
      ...
      tiles: {
        ...
        handpickedToday: todayHandpickedRows.length,
