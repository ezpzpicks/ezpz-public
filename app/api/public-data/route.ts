...
const FROZEN_TREND_GRADING_VERSION = "frozen-h2h-display-v8";
...
function trendWinnerCalibratedScore(score: number) {
  // The ROI/win-rate curves already prevent one-dimensional records from reaching
  // this range. Once a head-to-head winner reaches the low 80s on that tougher
  // composite, restore separation at the top instead of compressing every strong
  // convergence into the same 80-85 band. A raw 84 maps to about 92, while 95+
  // still requires an already exceptional raw winner score.
