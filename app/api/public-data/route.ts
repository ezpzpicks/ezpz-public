...
async function requestAiExternalReviews(candidates: AiSelectorCandidate[]): Promise<{
          ...
          content: [
            {
              type: "input_text",
              text:
                "You are the final MLB research analyst for EZPZ AI Picks. Research only information verifiably available before each listed game starts. Analyze every candidate independently; every candidate that clears the quantitative qualification threshold may be published, so do not force only one pick per day or per game. Build a detailed, game-specific case using actual player and team names. Prioritize probable/confirmed starting pitchers, pitcher recent form and underlying indicators, pitcher history versus the opponent and projected hitters when samples are meaningful, batter-versus-pitcher history when meaningful, pitch-mix and platoon matchup fit, current lineup quality, injuries and scratches, bullpen quality and recent workload, home/road splits, park and weather, travel/rest, umpire information when reliably reported, and meaningful market movement. Compare each candidate with the full candidate pool and explain why it qualified or stood out; do not claim it ranked first unless the supplied numbers support that. Avoid generic statements such as better pitcher or stronger offense: state the specific evidence and names. Historical notes must include sample context and must be omitted when not meaningful. Risks must be limited to real material concerns; return an empty risks array when none are significant. Do not include URLs, citations, publication names, source labels, or phrases such as according to. The public card should contain only the analyzed information. Never use a final score, in-game information, unsupported rumors, or hindsight. Do not invent statistics. Return only small evidence-based score adjustments from -6 to +6; use 0 when research does not change the quantitative case. A critical conflict is reserved for evidence that materially invalidates a saved pregame assumption, such as a scratched starter or changed role.",
            },
          ],
        },
        {
  ...
  for (const review of parsed.reviews || []) {
    ...
    reviews.set(review.candidateId, {
      ...
      confidenceReason: Array.isArray(review.confidenceReason) ? review.confidenceReason.filter(Boolean).slice(0, 6) : [],
      why: Array.isArray(review.why) ? review.why.filter(Boolean).slice(0, 8) : [],
      historicalNotes: Array.isArray(review.historicalNotes) ? review.historicalNotes.filter(Boolean).slice(0, 5) : [],
      risks: Array.isArray(review.risks) ? review.risks.filter(Boolean).slice(0, 3) : [],
      researchSummary: String(review.researchSummary || ""),
...
function finalizeAiCandidates(
  ...
  const finalized = candidates.map((candidate) => {
    ...
    return {
      ...
      rejectionReason,
      confidenceReason: [...new Set(candidate.confidenceReason.filter(Boolean))].slice(0, 6),
      whySelected: [...new Set(candidate.whySelected.filter(Boolean))].slice(0, 10),
      historicalNotes: [...new Set(candidate.historicalNotes.filter(Boolean))].slice(0, 5),
      risks: [...new Set(candidate.risks.filter(Boolean))].slice(0, 3),
...
function parseAiPickRow(row: SheetRow): AiPick | null {
  ...
  if (raw) {
    try {
      ...
      if (parsed?.candidateId && parsed?.date) return parsed;
    } catch {
  ...
  return {
    ...
    confidenceReason: String(row["AI Confidence Reason"] || "").split("|").map((item) => item.trim()).filter(Boolean),
    whySelected: String(row["Why Selected"] || "").split("|").map((item) => item.trim()).filter(Boolean),
    historicalNotes: String(row["Historical Matchup Notes"] || "").split("|").map((item) => item.trim()).filter(Boolean),
    risks: String(row.Risks || "").split("|").map((item) => item.trim()).filter(Boolean),
    researchSummary: String(row["AI Research Summary"] || ""),
    verdict: String(row["AI Verdict"] || ""),
