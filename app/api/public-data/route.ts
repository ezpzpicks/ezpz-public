...
const AI_PICK_SELECTOR_VERSION = "hybrid-web-context-v2-detailed-review";
...
async function requestAiExternalReviews(candidates: AiSelectorCandidate[]): Promise<{
  ...
  // The final review can contain several detailed candidate reports. The old
  // 2,600-token default regularly truncated the structured JSON mid-string,
  // which produced "Unterminated string" parse failures. Keep a reasonable
  // ceiling while allowing the schema enough room to finish every object.
  const maxOutputTokens = Math.max(
    2400,
    Math.min(8000, Number(process.env.EZPZ_AI_MAX_OUTPUT_TOKENS || 5200)),
  );
      ...
      ? Object.fromEntries(
          Object.entries(candidate.slateRow)
            .filter(([key, value]) => {
              ...
              // Keep the details that materially affect an MLB matchup report while
              // excluding broad/repetitive slate fields that inflate API input cost.
              return (
                k.includes("pitcher") ||
                k.includes("starter") ||
                k.includes("bulk") ||
            ...
            .slice(0, 120)
            .map(([key, value]) => [key, String(value).slice(0, 700)]),
        )
      : {},
  }));
  const schema = {
      ...
      reviews: {
        ...
        items: {
          ...
          properties: {
            ...
            criticalConflictReason: { type: "string", maxLength: 300 },
            confidenceReason: {
              type: "array",
            ...
            researchSummary: { type: "string", maxLength: 1200 },
            verdict: { type: "string", maxLength: 500 },
            contextSummary: { type: "string", maxLength: 400 },
            ...
            selectionComparison: { type: "string", minLength: 30, maxLength: 500 },
            mainRisk: { type: "string", minLength: 10, maxLength: 320 },
            finalVerdict: { type: "string", minLength: 30, maxLength: 500 },
          },
        },
      },
    },
...
For every candidate, fully populate the dedicated report fields. startingPitching must name the confirmed/probable starter or relevant prop pitcher and explain the matchup. bullpenAnalysis must describe recent bullpen availability or explicitly state that no material workload concern was verified. recentTeamForm must summarize meaningful recent performance for the teams or pitcher involved. historicalMatchup must include useful opponent history with sample context, or clearly state that no meaningful sample was found. selectionComparison must explain why this wager cleared the selector compared with the other supplied candidates, using the actual score, probability, edge, and matchup evidence. mainRisk must identify the most important realistic way the wager could fail. finalVerdict must give a direct, wager-specific conclusion. Never leave these fields blank and never substitute generic phrases such as “no outside context changed the play” for the actual analysis.

The WHY section should contain 4-7 concise but detailed bullets. At least three bullets must be player-specific whenever verified information exists. One bullet should discuss recent pitcher form, one should discuss opponent or batter history, and one should discuss a relevant split such as day/night or home/road when meaningful. If a requested split or matchup history is unavailable or too small, say that the sample is not meaningful rather than replacing the entire section with generic filler. Historical notes must state the sample size or clearly describe the sample.

The confidence explanation must say why the candidate cleared the AI threshold relative to the other candidates using supplied scores and concrete matchup evidence. Do not claim it ranked first unless the supplied numbers support that. Risks must be limited to genuine material concerns. Even when there is no major red flag, mainRisk must still state the clearest baseball-related failure path rather than saying only that no concerns were found.

Use the supplied modelGameContext first to identify the actual starters, projected lineups, bullpen scores, model projections, and saved assumptions, then verify and deepen those facts through research. Keep web research efficient: use one focused search per game when possible and perform another search only when a starter, bullpen availability item, or important matchup fact cannot otherwise be verified. Prioritize official MLB data and reliable pregame reporting. Do not use generic schedule-strength statements, broad season scoring claims, or park/weather filler unless they directly affect the wager. Never use a final score, in-game information, unsupported rumors, or hindsight. Do not invent statistics.
...
Do not include URLs, markdown links, citations, domains, publication names, source labels, bracketed source markers, or phrases such as according to. Never repeat a source URL even if the search tool provides one. The public card should contain only clean analyzed information. Return only small evidence-based score adjustments from -6 to +6; use 0 when research does not change the quantitative case. A critical conflict is reserved for evidence that materially invalidates a saved pregame assumption, such as a scratched starter or changed role.`,
  ...
  // A Responses API call can return HTTP 200 while still being incomplete, most
  // commonly because max_output_tokens was reached. Never try to JSON.parse a
  // known partial response; surface a retryable error instead.
  ...
  let parsed: { reviews?: AiExternalReview[] };
  try {
    parsed = JSON.parse(raw) as { reviews?: AiExternalReview[] };
  } catch (error) {
    // Structured output should already be valid JSON. Include completion status
    // and a compact diagnostic without leaking the model response publicly.
    const message = error instanceof Error ? error.message : String(error);
    const status = String(payload?.status || "unknown");
    throw new Error(`OpenAI selector returned invalid structured JSON (${status}): ${message}`);
  }
  const requestedIds = new Set(candidates.map((candidate) => candidate.candidateId));
  const reviews = new Map<string, AiExternalReview>();
  for (const review of parsed.reviews || []) {
    if (!review?.candidateId || !requestedIds.has(review.candidateId)) continue;
  ...
  // A partially returned batch must never be marked WEB_REVIEWED. Previously,
  // one successful review made every candidate in the batch look complete, so a
  // candidate omitted by the model was permanently saved with the generic
  // pre-research text and was never retried.
  const missingCandidateIds = candidates
    .map((candidate) => candidate.candidateId)
    .filter((candidateId) => !reviews.has(candidateId));
  if (missingCandidateIds.length) {
    throw new Error(
      `OpenAI selector omitted ${missingCandidateIds.length} of ${candidates.length} requested review${candidates.length === 1 ? "" : "s"}`,
    );
  }

  return {
    reviews,
    status: reviews.size === candidates.length ? "WEB_REVIEWED" : "NO_VERIFIED_CONTEXT",
  };
}
