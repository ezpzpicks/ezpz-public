...
function scoreHeadToHeadTrendPlays(plays: TrendPlay[]) {
  return plays.map((play) => {
    ...
    const rawGap = play.score - opponent.score;
    ...
    const winnerScore = clampScore(
      50 + comparisonGap + (play.score - 50) * 0.5,
    );
    const loserScore = Math.min(
      59,
      clampScore(50 - comparisonGap + (play.score - 50) * 0.25),
    );
...
function buildTrendPlays(
  ...
  const rawPlays = splits
    ...
    .map((split) =>
      buildTrendPlayForSplit(split, history, referenceDate, updatedAt),
    )
