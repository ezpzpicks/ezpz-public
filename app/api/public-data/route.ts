...
async function requestAiExternalReviews(
  ...
  const worker = async () => {
    while (true) {
      ...
      if (!candidate) return;
      try {
        // Reliability-first review path: every wager gets its own Responses
        // request. A malformed, incomplete, or timed-out review can therefore
        // reject only this candidate instead of discarding every play from the
        // same game. Passing a one-item array preserves the existing strict
        // schema, grading prompt, one-search cap, caching, and final-veto logic.
        const candidateReviews = await requestSingleAiGameExternalReviews([candidate]);
        const review = candidateReviews.get(candidate.candidateId);
        if (review) {
          reviews.set(candidate.candidateId, review);
        } else {
