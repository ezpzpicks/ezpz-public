...
async function persistAiPickRows(existingRows: SheetRow[], finalPicks: AiPick[]) {
  ...
  for (const pick of finalPicks) {
    ...
    const existingPick = existingRow ? parseAiPickRow(existingRow) : null;

    // Defense in depth: a successfully published FINAL_PREGAME pick can never
    // be overwritten by a later selector pass. Grading is overlaid separately.
    if (existingPick && aiStoredFinalSelectionIsLocked(existingPick)) continue;
    ...
    slateRows
      .filter((row) => {
        ...
        return (
          ...
          (captureRequested && isFifteenMinuteTrackingWindow(row))
        );
      })
      .map((row) => draftKingsGameKey(row))
