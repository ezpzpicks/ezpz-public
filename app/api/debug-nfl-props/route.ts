...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    graded: graded.map((row) => ({Date: row.Date, Grade: row.Grade, GameID: row["Game ID"], Game: row.Game, Player: row.Player, Market: row.Market, PropType: row["Prop Type"], Team: row.Team, Opponent: row.Opponent, keys: Object.keys(row)})),
