...
function ezpzGameTimeInfo(value: unknown) {
  const raw = String(value || "").trim();
  if (!raw) return { label: "", minutes: Number.POSITIVE_INFINITY };
  ...
  if (/(?:Z|[+-]\d{2}:?\d{2})$/i.test(raw)) {
    ...
    if (!Number.isNaN(parsed.getTime())) {
      ...
      const period = String(parts.find((part) => part.type === "dayPeriod")?.value || "AM").toUpperCase();
      const hour24 = period === "PM" ? (hour % 12) + 12 : hour % 12;
      return { label: `${hour}:${String(minute).padStart(2, "0")} ${period} ET`, minutes: hour24 * 60 + minute };
    }
  }

  const normalized = raw.replace(/\./g, "");
  const withMinutes = normalized.match(/(?:^|[^\d])(\d{1,2}):(\d{2})\s*(AM|PM)?\b/i);
  const hourOnly = normalized.match(/(?:^|[^\d])(\d{1,2})\s*(AM|PM)\b/i);
  const hour = Number(withMinutes?.[1] || hourOnly?.[1]);
  const minute = Number(withMinutes?.[2] || 0);
  const period = String(withMinutes?.[3] || hourOnly?.[2] || "").toUpperCase();
  if (Number.isFinite(hour) && Number.isFinite(minute)) {
    let hour24 = hour;
    if (period === "AM") hour24 = hour % 12;
    else if (period === "PM") hour24 = (hour % 12) + 12;
    if (hour24 >= 0 && hour24 <= 23 && minute >= 0 && minute <= 59) {
      const hour12 = hour24 % 12 || 12;
      const suffix = period || (hour24 >= 12 ? "PM" : "AM");
      return { label: `${hour12}:${String(minute).padStart(2, "0")} ${suffix} ET`, minutes: hour24 * 60 + minute };
    }
  }

  return { label: /\bET$/i.test(raw) ? raw : `${raw} ET`, minutes: Number.POSITIVE_INFINITY };
}

function ezpzPickTimeSort(a: EzpzPick, b: EzpzPick) {
  const aTime = ezpzGameTimeInfo(a.gameTime).minutes;
  const bTime = ezpzGameTimeInfo(b.gameTime).minutes;
    ...
    return (
      <article className="footballHistoryPickCard footballHistoryTrendCard">
        <div className="footballHistoryTrendTop">
          <div className="footballHistoryStatusCluster">
            <span className={`footballHistoryResult ${statusTone}`}><span className="footballHistoryStatusDot">●</span>{statusLabel}</span>
            {gameTimeLabel ? <span className="footballHistoryGameTime">{gameTimeLabel}</span> : null}
          </div>
...
function FootballEzpzHistory({ sport, data }: { sport: Sport; data: FootballData }) {
  ...
  const picks = viewingToday
    ? currentPicks
    : savedHistory.filter((pick) => isoDate(pick.date) === selectedDate).sort(ezpzPickTimeSort);
