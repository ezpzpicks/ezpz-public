...
function FormPill({ play }: { play: NflPlay }) {
  ...
  return (
    <div className={`nflFormPill ${form.cls}`} title="EZPZ Best Play form uses the most recent 7 completed bets for this exact bet type.">
...
function Metric({ label, value, accent = false }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className={`nflMetric ${accent ? "accent" : ""}`}>
      <span>{label}</span>
      <strong>{value || "—"}</strong>
    </div>
  );
...
function NflModelCard({ play, index }: { play: NflPlay; index: number }) {
  ...
  return (
    <article className={`nflOptimizedCard ${isProp ? "prop" : "game"} ${play.formStatus === "HOT" ? "hotCard" : ""}`}>
      <div className="nflCardTop">
        <span className="nflRank">#{index + 1}</span>
        <span className={`nflMiniForm ${form.cls}`}>{form.icon} {form.label}</span>
      </div>
      ...
      {isProp ? (
        <div className="nflPropPickLine">
          <span>{play.propSide || "Pick"}</span>
          <strong>{play.propLine || "—"}</strong>
          <small>{play.playType || "Prop"}</small>
        </div>
      ) : <div className="nflGradeLine">{play.playType || "Model Play"}</div>}
      ...
      <FormPill play={play} />
      <div className="nflRuleHint">
        {play.formStatus === "HOT"
          ? "Best Play form gate passed • EZPZ still requires price -150 or better"
          : "Best Play form gate not currently passed"}
      </div>
    </article>
  );
}
  ...
  return (
    <section className="nflOptimizedSection">
      <div className="nflOptimizedHead">
        <div>
          <h2>NFL Today’s Model Plays</h2>
          <p>Regression model plays with MLB-style player headshots and exact Last-7 bet-type form badges.</p>
        </div>
        <span>{plays.length} plays</span>
      </div>
      {plays.length ? (
        <div className="nflOptimizedGrid">
          {plays.map((play, index) => <NflModelCard key={`${play.game}-${play.play}-${index}`} play={play} index={index} />)}
        </div>
      ) : <div className="nflOptimizedEmpty">No graded NFL Model Plays are saved for {data.today || "today"}.</div>}
    </section>
  );
}

function sourceLabel(source: NflEzpzPick["source"]) {
  if (source === "Best + Trend") return "BEST + TREND";
  return source === "Best Play" ? "BEST PLAY" : "TREND";
}

function NflEzpzCard({ pick }: { pick: NflEzpzPick }) {
  const hasBest = pick.source === "Best Play" || pick.source === "Best + Trend";
  ...
  return (
    <article className={`nflEzpzCard ${isProp ? "prop" : ""}`}>
      <div className="nflEzpzTop">
        <div className="nflEzpzBadges">
          <span className="nflSourceBadge">{sourceLabel(pick.source)}</span>
          <span className={`nflStatusBadge ${final ? "final" : "pending"}`}>{final ? "FINAL" : "PENDING"}</span>
        </div>
        <strong className="nflEzpzOdds">{displayOdds(pick.odds)}</strong>
      </div>
      ...
      <div className="nflGateRow">
        {hasBest ? (
          <div className={`nflGate best ${form.cls}`}>
            <span>Best Play Gate</span>
            <strong>{form.icon} HOT + -150 or better</strong>
            <small>{pick.formType || "Bet type"} • {pick.record || "—"}</small>
          </div>
        ) : null}
        {hasTrend ? (
...
function NflEzpzPicks({ data }: { data: FootballData }) {
  ...
  return (
    <section className="nflOptimizedSection">
      ...
      {picks.length ? (
        <div className="nflEzpzStack">{picks.map((pick, index) => <NflEzpzCard key={`${pick.game}-${pick.market}-${pick.selection}-${index}`} pick={pick} />)}</div>
      ) : <div className="nflOptimizedEmpty">No NFL EZPZ Picks qualify for {data.today || "today"} right now.</div>}
    </section>
  );
}
