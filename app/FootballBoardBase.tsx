...
const FOOTBALL_TREND_SIGNAL_DISPLAY_LABELS: Record<string, string> = {
...
function footballTrendSignalDisplayLabel(value: unknown) {
...
function FootballTrendRecordCell({ label, record }: { label: string; record: RecordTotals }) {
  return (
...
function FootballTrendSignalPanels({ play }: { play: TrendPlay }) {
  if (!play.signals?.length) return null;

  return (
    <div className="trendSignalList">
      {play.signals.map((signal, signalIndex) => {
        const tone = footballTrendRecordTone(signal.records.allTime);
        const effectiveSample = signal.TrendSampleSize ?? signal.records.allTime.totalBets;
        const historySource = signal.HistorySource || play.HistorySource || "all_game_trends";
        const fallbackReason = signal.FallbackReason || play.FallbackReason;

        return (
          <section className={`trendSignalPanel ${tone}`} key={`${signal.signal}-${signalIndex}`}>
            <div className="trendSignalHead">
              <div>
                <span className={`liveSignalPill ${tone}`}>
                  {footballTrendSignalDisplayLabel(signal.signal)}
                </span>
            ...
            <div className="trendRecordGrid">
              <FootballTrendRecordCell label="All Time" record={signal.records.allTime} />
              <FootballTrendRecordCell label="Last 30" record={signal.records.last30} />
              <FootballTrendRecordCell label="Last 7" record={signal.records.last7} />
            </div>

            <div className="trendWeightLine">
              Exact category: {signal.exactSample} bets • Effective sample {effectiveSample} • History {historySource}
...
function TrendSelectionRow({ play, selectionRank, initiallyOpen, sport }: { play: TrendPlay; selectionRank: number; initiallyOpen: boolean; sport: Sport }) {
  ...
  const compactSignals = play.signals?.map((signal) => footballTrendSignalDisplayLabel(signal.signal)).filter(Boolean).join(" • ") || "";
  const rawMovementSignal = String(play.lineMovementSignal || "").trim();
  const normalizedMovementSignal = textKey(rawMovementSignal);
  const movementAlreadyListed = Boolean(
    normalizedMovementSignal &&
    play.signals?.some((signal) => {
      const normalizedSignal = textKey(signal.signal);
      return Boolean(
        normalizedSignal &&
        (normalizedSignal === normalizedMovementSignal ||
          normalizedSignal.includes(normalizedMovementSignal) ||
          normalizedMovementSignal.includes(normalizedSignal))
      );
    })
  );
  ...
  return (
    <details className={`trendSelectionRow ${selectionRank === 1 ? "leader" : ""}`} open={initiallyOpen}>
      ...
      <div className="trendSelectionBody">
        ...
        <FootballTrendSignalPanels play={play} />

        {rawMovementSignal && !movementAlreadyListed ? <div className="trendMovement">{rawMovementSignal}</div> : null}
