...
function normalizedPick(pick: AnyRow) {
...
export default async function EzpzPicksFeedPage() {
  const currentDate = todayET();
  const snapshots = await Promise.all(
    SPORTS.map(async (sport) => ({ sport, snapshot: await readEzpzCurrentPicks(sport) })),
  );

  return (
    <main style={{ fontFamily: "monospace", whiteSpace: "pre-wrap", padding: "24px", lineHeight: 1.55 }}>
      <h1>EZPZ Picks Live Feed</h1>
      <p>DATE_ET: {currentDate}</p>
      <p>DATA_SOURCE: persisted-cron-snapshot</p>
      {snapshots.map(({ sport, snapshot }) => {
        ...
        const stale = !snapshot || snapshot.date !== currentDate || ageMs > MAX_SNAPSHOT_AGE_MS;
        const picks = snapshot?.picks.map(normalizedPick) || [];
        ...
        return (
          <section key={sport}>
            ...
            <p>SNAPSHOT_AGE_SECONDS: {Number.isFinite(ageMs) ? Math.max(0, Math.round(ageMs / 1000)) : ""}</p>
            ...
            {!stale
              ? picks.map((pick, index) => (
                  <div key={`${sport}-${index}`}>
                    <p>
                      PICK {index + 1}: GAME={pick.game} | MARKET={pick.market} | SELECTION={pick.selection || pick.play} | LINE={pick.line} | ODDS={pick.odds} | GRADE={pick.grade} | TIER={pick.tier} | BEST_PLAY_TYPE={pick.bestPlayType} | SOURCE={pick.source} | LABEL={pick.label} | SNAPSHOT_STATUS={pick.snapshotStatus}
                    </p>
                  </div>
                ))
              : null}
