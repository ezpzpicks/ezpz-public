...
const DRAFTKINGS_STORAGE_KEY = "ezpz-draftkings-last-loaded-v3";

function draftKingsStorageSplitKey(row: DraftKingsSplit) {
  const selectedSide =
    row.market === "Total"
      ? row.side || row.selection
      : row.selectionTeam || row.selection;
  return `${row.date}|${row.game}|${row.market}|${String(selectedSide || "").toLowerCase()}`;
}

function draftKingsStoragePropKey(row: DraftKingsProp) {
  return `${row.date}|${row.game}|${String(row.pitcher || "").toLowerCase()}`;
}

function mergeDraftKingsData(
  current: DraftKingsData | undefined,
  previous: DraftKingsData | undefined,
  today: string,
): DraftKingsData | undefined {
  if (!current && !previous) return undefined;

  const splitMap = new Map<string, DraftKingsSplit>();
  const propMap = new Map<string, DraftKingsProp>();
  const currentUpdatedAt = current?.updatedAt || previous?.updatedAt || "";

  for (const row of current?.splits || []) {
    splitMap.set(draftKingsStorageSplitKey(row), {
      ...row,
      retained: row.retained === true,
      lastSeenAt: row.lastSeenAt || currentUpdatedAt,
    });
  }
  for (const row of current?.props || []) {
    propMap.set(draftKingsStoragePropKey(row), {
      ...row,
      retained: row.retained === true,
      lastSeenAt: row.lastSeenAt || currentUpdatedAt,
    });
  ...
  for (const row of previous?.splits || []) {
    const key = draftKingsStorageSplitKey(row);
    if (normalizedPublicDate(row.date) === normalizedPublicDate(today) && !splitMap.has(key)) {
      splitMap.set(key, {
        ...row,
        retained: true,
        lastSeenAt: row.lastSeenAt || previous?.updatedAt || currentUpdatedAt,
      });
    }
  }
  for (const row of previous?.props || []) {
    const key = draftKingsStoragePropKey(row);
    if (normalizedPublicDate(row.date) === normalizedPublicDate(today) && !propMap.has(key)) {
      propMap.set(key, {
        ...row,
        retained: true,
        lastSeenAt: row.lastSeenAt || previous?.updatedAt || currentUpdatedAt,
  ...
  const currentUnavailable = !current || current.status === "UNAVAILABLE";

  return {
    ok: current?.ok ?? previous?.ok ?? false,
    status: currentUnavailable
      ? previous?.status || "UNAVAILABLE"
      : current.status,
    updatedAt: currentUpdatedAt,
    stale: Boolean(current?.stale || (currentUnavailable && retainedCount > 0)),
    splits,
    props,
    retainedCount,
...
function readStoredDraftKings(today: string) {
  if (typeof window === "undefined") return undefined;
  try {
    const raw = window.localStorage.getItem(DRAFTKINGS_STORAGE_KEY);
    if (!raw) return undefined;
    const parsed = JSON.parse(raw) as { today?: string; data?: DraftKingsData };
    return normalizedPublicDate(parsed.today) === normalizedPublicDate(today)
      ? parsed.data
      : undefined;
  } catch {
    return undefined;
...
function storeDraftKings(today: string, data?: DraftKingsData) {
  if (typeof window === "undefined" || !data) return;
  try {
    window.localStorage.setItem(
      DRAFTKINGS_STORAGE_KEY,
      JSON.stringify({ today, data }),
    );
  } catch {
    // Private browsing or storage limits should not stop the live page.
  }
}
...
function liveSplitInfoForPlay(
  ...
  return {
    kind: "split",
    source: match.retained ? "DraftKings Last Loaded" : "DraftKings Live",
    ...
    matchConfidence: match.retained
      ? "Last loaded game and selected-side match"
      : "Live game and selected-side match",
    updatedAt: match.lastSeenAt || draftKings.updatedAt,
  };
}
...
function livePropInfoForSummary(
  ...
  return {
    kind: "prop",
    source: prop.retained ? "DraftKings Last Loaded" : "DraftKings Live",
    rank: prop.rank,
    market: prop.market,
    line: prop.listedLine,
    odds: formatOdds(prop.odds),
    matchConfidence: prop.retained
      ? exactSide && exactLine
        ? "Last loaded exact pitcher side/line match"
        : "Last loaded pitcher match"
      : exactSide && exactLine
        ? "Live exact pitcher side/line match"
        : "Live pitcher match",
    updatedAt: prop.lastSeenAt || draftKings.updatedAt,
...
function LiveMarketSplits({
  ...
  const hasRetained = rows.some((split) => split.retained);
  const allRetained = rows.every((split) => split.retained);
  const shownUpdatedAt = allRetained
    ? rows.find((split) => split.lastSeenAt)?.lastSeenAt || draftKings?.updatedAt
    : draftKings?.updatedAt;

  return (
    <div className="publicSplitPanel liveMarketPanel">
      <div className="publicSplitTitle">
        <span>DraftKings {market} Splits</span>
        <strong>
          {allRetained ? "Last loaded" : hasRetained ? "Live + saved" : draftKings?.stale ? "Last available" : "Live"}
        </strong>
      </div>
      <div className="liveSplitTable">
        {rows.map((split) => (
          <div className="liveSplitRow" key={`${split.market}-${split.selection}`}>
            <strong>{split.selection}</strong>
            <span><small>Bets</small>{publicPctText(split.betsPct)}</span>
            <span><small>Money</small>{publicPctText(split.moneyPct)}</span>
            <span><small>Odds</small>{formatOdds(split.odds)}</span>
          </div>
        ))}
      </div>
      <div className="publicSplitMeta">
        {allRetained ? "Last loaded" : "Updated"} {shownUpdatedAt || "recently"}
        {allRetained
          ? " • Game is no longer listed live; keeping its final pregame splits visible"
          : hasRetained
            ? " • Some selections are retained from the last successful listing"
            : draftKings?.stale
              ? " • DraftKings refresh unavailable; showing the last successful pull"
              : ""}
      </div>
    </div>
  );
...
export default function Home() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setData((previousData) => {
        const previousDraftKings =
          previousData?.today === json.today
            ? previousData.draftKings
            : readStoredDraftKings(json.today);
        const mergedDraftKings = mergeDraftKingsData(
          json.draftKings,
          previousDraftKings,
          json.today,
        );
        storeDraftKings(json.today, mergedDraftKings);
        return { ...json, draftKings: mergedDraftKings };
      });
      setError("");
    } catch (err) {
  ...
  const draftKings = data?.draftKings || null;
  const draftKingsError = draftKings?.errors?.join(" • ") || "";
