...
function getPlayableOdds(play: Play, slateRows: SheetRow[] = []) {
  ...
  if (row && isKType(play.playType)) {
    ...
    const awayMatch = pitcherName && (pitcherName === awayName || awayName.includes(pitcherName) || pitcherName.includes(awayName));
    const homeMatch = pitcherName && (pitcherName === homeName || homeName.includes(pitcherName) || pitcherName.includes(homeName));

    const keys = awayMatch
      ? ["Away Pitcher K Odds", "Away K Odds", "Away Pitcher Odds", "Away Pitcher Prop Odds", "Away Pitcher Odds/Line", "Away Odds"]
      : homeMatch
        ? ["Home Pitcher K Odds", "Home K Odds", "Home Pitcher Odds", "Home Pitcher Prop Odds", "Home Pitcher Odds/Line", "Home Odds"]
        : ["Pitcher K Odds", "K Odds", "Prop Odds", "Odds/Line", "Odds"];

    const odds = formatOdds(firstValue(row, keys));
    if (odds !== "—" && parseAmericanOdds(odds)) return odds;
  }
...
function imageForBestPlay(play: Play, rows: SheetRow[]) {
  ...
  if (pitcherNameMatches(pitcherName, awayName)) {
    return imageFromRow(row, ["Away Pitcher Headshot URL", "Away Pitcher Headshot", "Away Pitcher Image URL", "Away Pitcher Image", "Away Pitcher Photo", "Away Headshot", "Away Player Image"]);
  }

  if (pitcherNameMatches(pitcherName, homeName)) {
    return imageFromRow(row, ["Home Pitcher Headshot URL", "Home Pitcher Headshot", "Home Pitcher Image URL", "Home Pitcher Image", "Home Pitcher Photo", "Home Headshot", "Home Player Image"]);
  }
