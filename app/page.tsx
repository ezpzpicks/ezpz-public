...
function inferredGameTotalType(play: Play) {
  const explicitType = normalizeType(play.playType);
  if (isTotalType(explicitType)) return explicitType;

  // Some legacy best-play rows arrive as generic OVER/UNDER even though their
  // role or description identifies them as a game total. Recognize that shape
  // before pitcher-prop classification so it cannot render as an empty K card.
  const roleText = String(play.role || "").toUpperCase();
  const playText = String(play.play || "").toUpperCase();
  const totalContext =
    roleText.includes("GAME TOTAL") ||
    playText.includes("GAME TOTAL") ||
    playText.includes("TOTAL RUN");

  if (!totalContext) return "";

  const directionText = `${explicitType} ${playText}`;
  if (/\bUNDER\b/.test(directionText)) return "TOTAL UNDER";
  if (/\bOVER\b/.test(directionText)) return "TOTAL OVER";
  return "";
}

function isGameTotalPlay(play: Play) {
  return Boolean(inferredGameTotalType(play));
}

function canonicalizeBestPlay(play: Play): Play {
  const totalType = inferredGameTotalType(play);
  if (!totalType) return play;

  return {
    ...play,
    playType: totalType,
    role: "Game Total",
    // Clear pitcher-only fields that can be populated accidentally when a
    // generic OVER/UNDER tracker row is parsed as a strikeout prop.
    projectedKs: "",
    sixInningKs: "",
    headshotUrl: "",
    playerTeam: "",
...
function isQualifiedGreenPlay(play: Play) {
  ...
  // Google Sheets/API payloads can occasionally serialize booleans as text.
  // Accept only explicit true-like values so qualified plays are not hidden.
  const greenValue = String(play.isGreen ?? "")
    .trim()
    .toLowerCase();
  return play.isGreen === true || greenValue === "true" || greenValue === "1";
}

function isBestPlay(play: Play) {
  if (!isQualifiedGreenPlay(play)) return false;

  // The admin model is the source of truth. The public page should not apply a
  // second odds or historical-record filter that can hide a qualified model play.
  if (isNRFIType(play.playType)) {
    const type = normalizeType(play.playType);
    return type === "ELITE NRFI" || type === "ELITE YRFI";
  }
...
function isPitcherPlay(play: Play) {
  if (isGameTotalPlay(play)) return false;
...
function recordTypeForPlay(play: Play) {
  const totalType = inferredGameTotalType(play);
  if (totalType) return totalType;
  return isPitcherPlay(play) ? pitcherGrade(play) : normalizeType(play.playType);
}
...
function getBaseModelScore(play: Play, slateRows: SheetRow[] = []) {
  if (isGameTotalPlay(play)) return getTotalModelScore(play, slateRows);
...
function calculateTrackerRecordSummary(
  ...
  rows.forEach((row) => {
    if (handpickedOnly && !isHandpickedRecordRow(row)) return;
    if (!trackerDateInMode(row, mode, today)) return;

    const betType = normalizeType(row["Bet Type"] || row["Play Type"] || row.Type || "");
...
function mergeRecordSummaries(primary: Summary[] | undefined, fallback: Summary[]) {
  ...
  (fallback || []).forEach((row) => merged.set(normalizeType(row.betType), row));
  (primary || []).forEach((row) => merged.set(normalizeType(row.betType), row));
...
function slateGameLabel(row: SheetRow) {
  return String(
    firstValue(row, ["Game Label", "Game", "Matchup"]) ||
      `${row["Away Team"] || ""} at ${row["Home Team"] || ""}`,
  ).trim();
}

function slateTeams(row: SheetRow) {
  const gameTeams = splitGameTeams(slateGameLabel(row));
  return {
    awayTeam: cleanTeamName(row["Away Team"] || gameTeams.awayTeam),
    homeTeam: cleanTeamName(row["Home Team"] || gameTeams.homeTeam),
  };
}

function qualifiedPitcherPlaysFromSlateRow(row: SheetRow): Play[] {
  const game = slateGameLabel(row);
  const teams = slateTeams(row);
  const awayHasBulk = Boolean(String(row["Away Bulk Pitcher K + Grade"] || "").trim());
  const homeHasBulk = Boolean(String(row["Home Bulk Pitcher K + Grade"] || "").trim());
  ...
  const slots = [
    {
      side: "Away" as const,
      role: awayHasBulk ? "Opener" : "Starter",
      summaryKey: "Away Pitcher K + Grade",
      scoreKey: "Away Pitcher K Score",
      reliabilityKey: "Away Pitcher K Reliability",
      probabilityKey: "Away Pitcher K Probability",
      team: teams.awayTeam,
      oddsKeys: ["Away Pitcher K Odds", "Away K Odds", "Away Pitcher Odds"],
      altLineKeys: ["Away Pitcher K Alt Line", "Away K Alt Line"],
      altOddsKeys: ["Away Pitcher K Alt Odds", "Away K Alt Odds"],
      sixInningKeys: ["Away Pitcher Six Inning Ks", "Away Pitcher 6 Inning Ks"],
      headshotKeys: [
        "Away Pitcher Headshot URL",
        "Away Pitcher Headshot",
        "Away Pitcher Image URL",
    ...
    },
    {
      side: "Home" as const,
      role: homeHasBulk ? "Opener" : "Starter",
      summaryKey: "Home Pitcher K + Grade",
      scoreKey: "Home Pitcher K Score",
      reliabilityKey: "Home Pitcher K Reliability",
      probabilityKey: "Home Pitcher K Probability",
      team: teams.homeTeam,
      oddsKeys: ["Home Pitcher K Odds", "Home K Odds", "Home Pitcher Odds"],
      altLineKeys: ["Home Pitcher K Alt Line", "Home K Alt Line"],
      altOddsKeys: ["Home Pitcher K Alt Odds", "Home K Alt Odds"],
      sixInningKeys: ["Home Pitcher Six Inning Ks", "Home Pitcher 6 Inning Ks"],
      headshotKeys: [
        "Home Pitcher Headshot URL",
        "Home Pitcher Headshot",
        "Home Pitcher Image URL",
    ...
    },
    {
      side: "Away" as const,
      role: "Bulk" as const,
      summaryKey: "Away Bulk Pitcher K + Grade",
      scoreKey: "Away Bulk Pitcher K Score",
      reliabilityKey: "Away Bulk Pitcher K Reliability",
      probabilityKey: "Away Bulk Pitcher K Probability",
      team: teams.awayTeam,
      oddsKeys: ["Away Bulk Pitcher K Odds", "Away Bulk K Odds"],
      altLineKeys: ["Away Bulk Pitcher K Alt Line", "Away Bulk K Alt Line"],
      altOddsKeys: ["Away Bulk Pitcher K Alt Odds", "Away Bulk K Alt Odds"],
      sixInningKeys: ["Away Bulk Pitcher Six Inning Ks", "Away Bulk Pitcher 6 Inning Ks"],
      headshotKeys: [] as string[],
    },
    {
      side: "Home" as const,
      role: "Bulk" as const,
      summaryKey: "Home Bulk Pitcher K + Grade",
      scoreKey: "Home Bulk Pitcher K Score",
      reliabilityKey: "Home Bulk Pitcher K Reliability",
      probabilityKey: "Home Bulk Pitcher K Probability",
      team: teams.homeTeam,
      oddsKeys: ["Home Bulk Pitcher K Odds", "Home Bulk K Odds"],
      altLineKeys: ["Home Bulk Pitcher K Alt Line", "Home Bulk K Alt Line"],
      altOddsKeys: ["Home Bulk Pitcher K Alt Odds", "Home Bulk K Alt Odds"],
      sixInningKeys: ["Home Bulk Pitcher Six Inning Ks", "Home Bulk Pitcher 6 Inning Ks"],
      headshotKeys: [] as string[],
    },
  ];

  return slots.flatMap((slot) => {
    const summary = String(row[slot.summaryKey] || "").trim();
    if (!summary || !isQualifiedKSummary(summary)) return [];

    const playType = normalizeType(summary);
    return [
      {
        playType,
        game,
        play: summary,
        oddsLine: firstValue(row, slot.oddsKeys),
        score: String(row[slot.scoreKey] || ""),
        isGreen: true,
        awayTeam: teams.awayTeam,
        homeTeam: teams.homeTeam,
        headshotUrl: imageFromRow(row, slot.headshotKeys),
        playerTeam: slot.team,
        projectedKs: extractProjectedK(summary),
        sixInningKs: firstValue(row, slot.sixInningKeys),
        altLine: firstValue(row, slot.altLineKeys),
        altOdds: firstValue(row, slot.altOddsKeys),
        reliability: String(row[slot.reliabilityKey] || ""),
        selectedProbability: String(row[slot.probabilityKey] || ""),
...
function qualifiedMoneylinePlayFromSlateRow(row: SheetRow): Play | null {
  const playType = normalizeType(
    firstValue(row, ["ML Grade", "Moneyline Grade", "Selected ML Grade"]),
  );
  if (playType !== "A MONEYLINE" && playType !== "B MONEYLINE") return null;

  const game = slateGameLabel(row);
  const teams = slateTeams(row);
  const selectedTeam = cleanMoneylineTeam(
    firstValue(row, ["Better ML", "ML Pick", "Moneyline Pick", "Selected ML"]),
  );

  return {
    playType,
    game,
    play: selectedTeam || playType,
    oddsLine: firstValue(row, ["ML Odds", "Moneyline Odds", "Selected ML Odds"]),
    score: firstValue(row, [
      "ML Score",
      "Moneyline Score",
      "ML Reliability",
    ...
    reliability: firstValue(row, ["ML Reliability", "Moneyline Reliability"]),
    selectedProbability: firstValue(row, [
      "ML Selected Probability",
      "Moneyline Selected Probability",
      "Win Probability",
    ]),
    modelVersion: String(row["Model Version"] || ""),
    role: "Moneyline",
...
function qualifiedFirstInningPlayFromSlateRow(row: SheetRow): Play | null {
  const playType = normalizeType(
    firstValue(row, [
      "NRFI Grade",
      "YRFI Grade",
  ...
  if (playType !== "ELITE NRFI" && playType !== "ELITE YRFI") return null;

  const game = slateGameLabel(row);
  const teams = slateTeams(row);
  const isYrfi = playType === "ELITE YRFI";
  ...
  return {
    ...
    oddsLine: firstValue(row, [
      isYrfi ? "YRFI Odds" : "NRFI Odds",
      "First Inning Odds",
      "1st Inning Odds",
    ]),
    score: firstValue(row, [
      isYrfi ? "YRFI Score" : "NRFI Score",
      "First Inning Score",
      "1st Inning Score",
    ]),
    isGreen: true,
    ...
    homeTeam: teams.homeTeam,
    selectedProbability: firstValue(row, [
      isYrfi ? "YRFI Probability" : "NRFI Probability",
      isYrfi ? "YRFI %" : "NRFI %",
...
function qualifiedTotalPlayFromSlateRow(row: SheetRow): Play | null {
  const playType = normalizeType(
    firstValue(row, ["Total Runs Grade", "Total Grade", "Game Total Grade"]),
  );
  if (!isTotalType(playType)) return null;

  const game = slateGameLabel(row);
  const teams = slateTeams(row);
  const line = firstValue(row, ["Total Runs Line", "Total Line", "Game Total Line"]);
  const side = playType === "TOTAL OVER" ? "Over" : "Under";

  return {
    playType,
    game,
    play: `${side}${line ? ` ${line}` : ""}`,
    oddsLine: firstValue(row, ["Total Odds", "Total Runs Odds", "Game Total Odds"]),
    score: firstValue(row, [
      "Total Runs Projection",
      "Projected Total",
      "Total Projection",
    ...
    awayTeam: teams.awayTeam,
    homeTeam: teams.homeTeam,
    reliability: firstValue(row, ["Total Reliability"]),
    selectedProbability: firstValue(row, ["Total Selected Probability"]),
    modelVersion: String(row["Model Version"] || ""),
    role: "Game Total",
  };
}

function qualifiedPlaysFromSlateRows(rows: SheetRow[] | undefined) {
  if (!rows?.length) return [] as Play[];

  return rows.flatMap((row) => {
    const plays: Play[] = [...qualifiedPitcherPlaysFromSlateRow(row)];
    const moneyline = qualifiedMoneylinePlayFromSlateRow(row);
    const firstInning = qualifiedFirstInningPlayFromSlateRow(row);
    const total = qualifiedTotalPlayFromSlateRow(row);

    if (moneyline) plays.push(moneyline);
    if (firstInning) plays.push(firstInning);
    if (total) plays.push(total);
    return plays;
  });
}

function bestPlayGameIdentity(play: Play) {
  // Prefer the structured team fields because the same matchup can be formatted
  // differently across API, slate, and tracker sources ("at", "@", abbreviations,
  // extra spacing, etc.). This keeps one canonical identity per game.
  let away = favoriteKeyText(cleanTeamName(play.awayTeam || ""));
  let home = favoriteKeyText(cleanTeamName(play.homeTeam || ""));

  if (!away || !home) {
    const parsed = splitGameTeams(String(play.game || ""));
    away = away || favoriteKeyText(cleanTeamName(parsed.awayTeam));
    home = home || favoriteKeyText(cleanTeamName(parsed.homeTeam));
  }

  if (away && home) return `${away}|${home}`;
  return favoriteKeyText(
    play.game || `${play.awayTeam || ""} at ${play.homeTeam || ""}`,
  );
}

function bestPlayIdentity(play: Play) {
  const totalType = inferredGameTotalType(play);
  const type = totalType || normalizeType(play.playType);
  const game = bestPlayGameIdentity(play);

  // Total detection must happen before generic OVER/UNDER pitcher detection.
  if (totalType) return `TOTAL|${game}|${totalType}`;

  if (isPitcherPlay(play)) {
    return `K|${game}|${pitcherNameKey(cleanPitcherName(play.play))}`;
  }
  if (isMoneylineType(type)) {
    return `ML|${game}|${favoriteKeyText(
      cleanMoneylineTeam(play.playerTeam || play.play),
    )}`;
  }
  if (isNRFIType(type)) return `FI|${game}|${type}`;

  return `OTHER|${game}|${type}|${favoriteKeyText(play.play)}`;
}

function mergeBestPlaySources(...sources: Array<Play[] | undefined>) {
  const merged: Play[] = [];
  const keyToIndex = new Map<string, number>();

  sources.forEach((source) => {
    (source || []).forEach((rawIncoming) => {
      const incoming = canonicalizeBestPlay(rawIncoming);
      const key = bestPlayIdentity(incoming);
      ...
      const combined = { ...incoming, ...existing } as Play;
      const fillableKeys: Array<keyof Play> = [
        "playType",
        "game",
        "play",
        "oddsLine",
      ...
      fillableKeys.forEach((field) => {
        const current = combined[field];
        if (current === undefined || current === null || String(current).trim() === "") {
          (combined as Record<string, unknown>)[field] = incoming[field];
        }
      });
      combined.isGreen =
        isQualifiedGreenPlay(existing) || isQualifiedGreenPlay(incoming);
...
function totalPlayFromTrackerRow(row: SheetRow): Play | null {
  ...
  const selectionText = String(
    row.Selection || row.Pick || row.Play || row["Bet Type"] || "",
  ).toUpperCase();

  if (!isTotalType(rawPlayType) && !market.includes("game total")) return null;

  let playType = rawPlayType;
  if (!isTotalType(playType)) {
    const directionText = `${rawPlayType} ${selectionText}`;
    if (/\bUNDER\b/.test(directionText)) playType = "TOTAL UNDER";
    else if (/\bOVER\b/.test(directionText)) playType = "TOTAL OVER";
  }
  if (!isTotalType(playType)) return null;
...
function BestPlayCard({
  ...
  const totalPlay = isGameTotalPlay(play);
  const kPlay = !totalPlay && isPitcherPlay(play);
...
export default function Home() {
  ...
  const slateQualifiedBestPlays = useMemo(
    () => qualifiedPlaysFromSlateRows(data?.slateToday),
    [data?.slateToday],
  );
  ...
  const bestPlays = useMemo(() => {
    ...
    const sourceBestPlays = mergeBestPlaySources(
      // Slate-derived plays are the most complete source for market details.
      // API/tracker rows may still fill optional metadata, but they cannot
      // replace the correctly structured total tile with a legacy empty card.
      slateQualifiedBestPlays,
      data.bestPlays,
      trackerTotalBestPlays,
    );
