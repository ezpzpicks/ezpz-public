...
function scoreTrendMarketPlays(plays: TrendPlay[]): RankedTrendPlay[] {
  ...
  return baseRows.map(({ play, metrics }) => {
    ...
    const opponents = sameMarket
      ...
      .sort((a, b) => {
        const aScore = a.play.baseScore ?? a.metrics.score;
        const bScore = b.play.baseScore ?? b.metrics.score;
    ...
    if (frozen) {
      const baseTrendScore = clampScore(play.baseScore ?? metrics.score);
      const opponentBaseScore =
        play.opponentScore ??
        (opponent
          ? clampScore(opponent.play.baseScore ?? opponent.metrics.score)
          : null);
      const comparisonGap =
        play.comparisonGap ??
        (opponentBaseScore == null
          ? 0
          : Math.abs(baseTrendScore - opponentBaseScore));
      const comparisonWinner =
        play.comparisonWinner ??
        Boolean(
          opponentBaseScore != null && baseTrendScore > opponentBaseScore + 0.01,
        );
      const storedTier: TrendTier =
        play.tier === "Elite" ||
        play.tier === "Strong" ||
        play.tier === "Good" ||
        play.tier === "Pass"
          ? play.tier
          : rankedTrendLabel(play.score, comparisonWinner);

      return {
        ...play,
        originalScore: play.baseScore ?? play.score,
        score: play.score,
        baseTrendScore,
...
function historicalTrendGameKey(row: TrendRecordResult, play: TrendPlay) {
  ...
  return `${row.date}|${trendGameMatchKey(play) || publicMatchKey(row.game)}`;
}
...
function buildHistoricalTrendOutcomes(rows: TrendRecordResult[]) {
  ...
  const grouped = new Map<string, HistoricalTrendOutcome[]>();
  unique.forEach((outcome) => {
    const group = grouped.get(outcome.gameKey) || [];
    group.push(outcome);
    grouped.set(outcome.gameKey, group);
  });

  const graded: Array<HistoricalTrendOutcome & { tier: TrendTier; score: number }> = [];
  grouped.forEach((outcomes) => {
    const outcomeBySelection = new Map(
      outcomes.map((outcome) => [historicalTrendSelectionKey(outcome.play), outcome]),
    );

    scoreTrendMarketPlays(outcomes.map((outcome) => outcome.play)).forEach((play) => {
      const outcome = outcomeBySelection.get(historicalTrendSelectionKey(play));
      if (!outcome || play.tier === "Pass") return;
      graded.push({ ...outcome, tier: play.tier, score: play.score });
    });
  });
  ...
  return (
    <div className="dkRecordsCard trendTierRecordsCard">
      <div className="dkRecordFilters trendRecordFilters">
        <label>
          ...
          <select
            value={period}
            onChange={(event) =>
              setPeriod(event.target.value as "all" | "30" | "7")
            }
          >
      ...
      <div className="trendRecordsNote">
        {gradedCount
          ? `${filteredCount} completed graded trend plays match these filters (${gradedCount} total available). Each historical play is re-graded with the current Good / Strong / Elite comparison method using its frozen pregame trend details. Older games without both opposing sides saved are excluded.`
      ...
      <>
        ...
        <div className="sectionHead trendRecordsHead">
          <div>
            ...
            <p>
              Good, Strong, and Elite results based on the exact trend grade frozen before each game. These records use the current side-versus-opponent scoring method.
            </p>
          </div>
        </div>
