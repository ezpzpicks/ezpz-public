...
function formatOdds(value: unknown) {
  ...
  // Pitcher prop cells from bet_tracker are usually stored as "5.5 / -150".
  // For the Odds bubble, only show the second part.
  if (raw.includes("/")) {
    const oddsPart = raw.split("/").slice(1).join("/").trim();
    const slashOdds = oddsPart.match(/[+-]\d{3,}/)?.[0] || oddsPart.match(/\b\d{3}\b/)?.[0];
    if (slashOdds) return slashOdds.startsWith("+") || slashOdds.startsWith("-") ? slashOdds : `+${slashOdds}`;
  }

  // Prefer explicitly signed American odds like -150 or +120.
  const signed = raw.match(/[+-]\d{3,}/)?.[0];
  if (signed) return signed;

  // Fallback: only format standalone 3-digit odds, not dates/years.
  const standalone = raw.match(/(^|\s)(\d{3})(?=$|\s)/);
...
function getPlayableOdds(play: Play, slateRows: SheetRow[] = []) {
  ...
  if (parseAmericanOdds(odds)) return odds;
  ...
  for (const key of possibleKeys) {
    const value = formatOdds(row[key]);
    if (value !== "—" && parseAmericanOdds(value)) return value;
  }
...
function normalizeProbability(value: unknown) {
  ...
  // Accept either 0.565 or 56.5 / 56.5% formats.
  return n > 1 ? n / 100 : n;
}
...
function calculateNRFIScoreFromProbability(probability: number, playType: unknown) {
  ...
  // A YRFI is strong when NRFI probability is low, so invert the score.
  if (normalizeType(playType) === "YRFI") {
    return Math.max(0, Math.min(100, 100 - rawNrfiScore));
  }
...
function nrfiScoreFromRow(play: Play, row?: SheetRow) {
  ...
  // Prefer true probability because route.ts may still send the old fallback score of 66.
  const probability = normalizeProbability(firstValue(row, [
    "NRFI %",
    "NRFI%",
    "NRFI Probability",
    "NRFI Prob",
  ...
  const calculatedFromProbability = calculateNRFIScoreFromProbability(probability, play.playType);
  if (calculatedFromProbability) return calculatedFromProbability;

  const directScore = firstValue(row, [
    "NRFI Score",
    "NRFI/YRFI Score",
    "YRFI Score",
    "NRFI Rank Score",
  ...
  if (directScore) {
    const score = parseScore(directScore);

    // 66 was the old hardcoded route.ts fallback for normal NRFI/YRFI plays.
    // Do not treat it as a true model score unless no better data exists.
    if (score !== 66) {
      return normalizeType(play.playType) === "YRFI" ? 100 - score : score;
    }
  }
...
function passesColdFilter(play: Play, recentSummary: Summary | null, slateRows: SheetRow[] = []) {
  ...
  // Cold plays can still make Best Plays only when the model signal is elite
  // or the pitcher prop has a clear ALT-line edge. No hard cap is applied.
  if (hasAltBadge(play)) return true;
  if (getRankScore(play, recentSummary, slateRows) >= 78) return true;
...
function TeamRow({ awayTeam, homeTeam }: { awayTeam: string; homeTeam: string }) {
  ...
  const awayLogo = teamLogoUrl(cleanAwayTeam);
  const homeLogo = teamLogoUrl(cleanHomeTeam);

  return (
    <div className="teamRow">
      <div className="teamSide">
        {awayLogo ? <img className="teamLogo" src={awayLogo} alt={`${cleanAwayTeam} logo`} /> : null}
        <div className="teamName">{cleanAwayTeam}</div>
      </div>

      <div className="vsText">AT</div>

      <div className="teamSide home">
        <div className="teamName">{cleanHomeTeam}</div>
        {homeLogo ? <img className="teamLogo" src={homeLogo} alt={`${cleanHomeTeam} logo`} /> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <div className={`card green fade-in best ${topPlay ? "top" : ""}`}>
      <div className="cardTop">
        ...
        <div className="scorePill">Rank {rankScore}</div>
      </div>
      ...
      <div className="marketStack">
        {row["ML Grade"] ? (
          <div className={`marketBubble ${mlGreen ? "green" : ""}`}>
            Moneyline: {cleanTeamName(row["Better ML"]) || "—"} • {row["ML Grade"]} • {formatOdds(row["ML Odds"] || "—")}
          </div>
        ) : null}
...
export default function Home() {
  ...
  const bestPlays = useMemo(() => {
    ...
    return data.bestPlays
      .filter((play) => {
        const summary = recentByType.get(normalizeType(play.playType)) || null;
        return isBestPlay(play) && passesColdFilter(play, summary, data.slateToday) && passesMinimumScoreFilter(play, summary, data.slateToday);
      })
      .sort((a, b) => {
