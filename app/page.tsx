...
type UfcTab = "Best Plays" | "Full Card" | "Records";

const SPORTS: Sport[] = ["MLB", "UFC"];
const TABS: Tab[] = ["Today’s Best Plays", "Full Slate", "Records"];
const UFC_TABS: UfcTab[] = ["Best Plays", "Full Card", "Records"];
const BEST_PLAY_MIN_ODDS = -200;
const BEST_PLAY_MIN_RANK_SCORE = 49;
...
function isBestPlay(play: Play) {
  ...
  // Best Plays should only include Elite NRFI or YRFI.
  // Regular NRFI, Strong NRFI, and Lean NRFI stay visible on Full Slate only.
  if (isNRFIType(play.playType)) {
    const type = normalizeType(play.playType);
    if (type !== "ELITE NRFI" && type !== "YRFI") return false;
  }

  // Best Plays must respect the -145 odds limit.
...
function getTotalLine(play: Play) {
...
function getTotalProjectionNumber(play: Play) {
...
function getTotalProjection(play: Play) {
  const n = getTotalProjectionNumber(play);
  if (n) return n.toFixed(1);
  const raw = String(play.score ?? "").trim();
  return raw || "—";
}

function getTotalModelScore(play: Play) {
  const line = toNumber(getTotalLine(play));
  const projection = getTotalProjectionNumber(play);
  if (!line || !projection) return 50;

  const type = normalizeType(play.playType);
  const edgeRuns = type === "TOTAL OVER" ? projection - line : line - projection;

  // Keep totals on the same 0-100 style as other public cards.
  // A half-run edge is around 58, a one-run edge around 65, and a two-run edge around 80.
  return clampScore(50 + edgeRuns * 15);
}

function getTotalPickLabel(play: Play) {
  const type = normalizeType(play.playType);
  const side = type === "TOTAL OVER" ? "Over" : type === "TOTAL UNDER" ? "Under" : "Total";
  return `${side} ${getTotalLine(play)}`;
}
...
function imageForBestPlay(play: Play, rows: SheetRow[]) {
  ...
  if (
    ...
    (pitcherName === awayName ||
      awayName.includes(pitcherName) ||
      pitcherName.includes(awayName))
  ) {
  ...
  if (
    ...
    (pitcherName === homeName ||
      homeName.includes(pitcherName) ||
      pitcherName.includes(homeName))
  ) {
    return imageFromRow(row, [
...
function nrfiScoreFromRow(play: Play, row?: SheetRow) {
  ...
  if (row) {
    for (const key of directKeys) {
      ...
      const lowerKey = key.toLowerCase();
      if (
        normalizeType(play.playType) === "YRFI" &&
        lowerKey.includes("nrfi") &&
        !lowerKey.includes("yrfi")
      )
        score = 100 - score;
...
function getBaseModelScore(play: Play, slateRows: SheetRow[] = []) {
  if (isTotalType(play.playType)) return getTotalModelScore(play);
...
function getRankScore(
  ...
  const missingOddsPenalty =
    isKType(play.playType) &&
    !parseAmericanOdds(play.oddsLine) &&
    !parseAmericanOdds(play.altOdds)
      ? -2
      : 0;
  return clampScore(
    baseScore + hotBoost + coldPenalty + altBoost + missingOddsPenalty,
  );
}

function passesMinimumScoreFilter(
  play: Play,
  recentSummary: Summary | null,
  slateRows: SheetRow[] = [],
) {
  // EZPZ score is for ranking/display only.
  // Best Plays qualification comes from the saved grade/isGreen flag plus odds and overall record.
  return true;
}

function passesColdFilter(
  play: Play,
  recentSummary: Summary | null,
  slateRows: SheetRow[] = [],
) {
  // Hot/Neutral/Cold is a badge only, not a Best Plays blocker.
  return true;
}

function passesOverallRecordFilter(
  play: Play,
  overallRows: Summary[],
  recentSummary: Summary | null,
) {
  ...
  // If no tracked record exists yet, allow the qualified play.
  if (!overall) return true;

  // Best Plays should only include qualified plays from bet types
  ...
  trackerTotalPlays.forEach((play) => {
    ...
    if (typeof existingIndex === "number") {
      ...
      merged[existingIndex] = {
        ...existing,
        score: getTotalProjectionNumber(existing) ? existing.score : play.score,
        play: existing.play || play.play,
        game: existing.game || play.game,
        oddsLine: existing.oddsLine || play.oddsLine,
  ...
  return (
    <div className="teamRow">
      <div className="teamSide">
        {awayLogo ? (
          <img className="teamLogo" src={awayLogo} alt={`${cleanAway} logo`} />
        ) : null}
      ...
      <div className="teamSide home">
        ...
        {homeLogo ? (
          <img className="teamLogo" src={homeLogo} alt={`${cleanHome} logo`} />
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="cardTop">
        ...
        {handpicked ? (
          <div className="handpickedPill">⭐ HANDPICKED</div>
        ) : null}
      </div>
      ...
      {kPlay ? (
        ...
        <>
          <div className="playMain">
            ...
            <div>
              ...
              <div className="playDetail">
                {moneylineGradeLabel(play.playType)}
              </div>
            </div>
          </div>
          ...
          <div className="bubbleGrid">
            <MiniBubble
              label="Odds"
              value={formatOdds(play.oddsLine || "—")}
              green
            />
            ...
            <MiniBubble
              label="Bet Type"
              value={normalizeType(play.playType)}
              green
            />
          </div>
        </>
        ...
        <>
          <div className="cardTitle">Game Total</div>
          <div className="cardSub">{play.game}</div>

          <div className="projectionBlock">
            <div className="projection">{getTotalPickLabel(play)}</div>
            <div className="grade">{normalizeType(play.playType)}</div>
          </div>
          ...
          <div className="bubbleGrid">
            <MiniBubble label="Line" value={getTotalLine(play)} green />
            <MiniBubble label="Projected" value={getTotalProjection(play)} green />
            <MiniBubble label="Odds" value={getPlayableOdds(play)} green />
            <MiniBubble label="EZPZ Score" value={rankScore || "—"} green />
          </div>
        </>
...
function SlateCard({ row }: { row: SheetRow }) {
  ...
  const totalType = normalizeType(row["Total Runs Grade"] || row["Total Grade"] || "");
  const mlGreen =
    mlType !== "" &&
    mlType !== "PASS" &&
    mlType !== "NON-EDGE MONEYLINE" &&
    slateMoneylinePassesBestPlayRules(row);
  const nrfiGreen = nrfiType !== "" && nrfiType !== "PASS";
  const totalGreen = isTotalType(totalType);
  const hasGreen = awayGreen || homeGreen || mlGreen || nrfiGreen || totalGreen;
      ...
      <div className="marketStack">
        {row["ML Grade"] ? (
          <div className={`marketBubble ${mlGreen ? "green" : ""}`}>
            Moneyline: {row["Better ML"] || "—"} • {row["ML Grade"]} •{" "}
            {formatOdds(row["ML Odds"] || "—")}
          </div>
        ) : null}

        {row["NRFI Grade"] ? (
          <div className={`marketBubble ${nrfiGreen ? "green" : ""}`}>
            NRFI/YRFI: {row["NRFI Grade"]}
          </div>
        ) : null}

        {isTotalType(totalType) ? (
          <div className={`marketBubble ${totalGreen ? "green" : ""}`}>
            Game Total: {totalType.replace("TOTAL ", "")} {row["Total Runs Line"] || "—"} • Projected {row["Total Runs Projection"] || "—"}
          </div>
        ) : null}
      </div>
...
function ufcDisplayOdds(value: unknown) {
  const raw = String(value ?? "").trim();
  if (!raw) return "—";
...
function ufcPercent(value: unknown) {
  const raw = String(value ?? "").trim();
  if (!raw) return "—";
  return raw.includes("%") ? raw : `${raw}%`;
}

function ufcRecordStatus(row: UfcRecordRow) {
...
function isUfcFavorite(row: SheetRow) {
  return isFavoriteValue(row["Favorite Pick"] || row["favorite pick"] || row["Favorite"]);
}

function isUfcHandpickedRecord(row: SheetRow) {
...
function UfcBestPlayCard({ row, index }: { row: SheetRow; index: number }) {
  const fighter = row["Fighter"] || "—";
  const opponent = row["Opponent"] || "—";
  const score = row["EZPZ Score"] || "—";
  const grade = row["Grade"] || "—";

  return (
    <div className={`card green fade-in best ${index < 3 ? "top" : ""}`}>
      <div className="cardTop">
        <div className="rankBadge">#{index + 1}</div>
        <div className="scorePill">EZPZ Score {score}</div>
      </div>

      {isUfcFavorite(row) ? (
        <div className="badgeRow">
          <span className="badge handpicked">⭐ Handpicked</span>
        </div>
      ) : null}

      <div className="ufcFightRow">
        <div>
          <div className="matchup">UFC MONEYLINE</div>
          <div className="playName">{fighter}</div>
          <div className="playDetail">vs {opponent}</div>
        </div>
        <div className="ufcGradePill">{grade}</div>
      </div>

      <div className="projectionBlock">
        <div className="projection">Moneyline</div>
        <div className="grade">{row["Event"] || "UFC Best Play"}</div>
      </div>

      <div className="divider" />

      <div className="bubbleGrid">
        <MiniBubble label="Odds" value={ufcDisplayOdds(row["Odds"])} green />
        <MiniBubble label="Model %" value={ufcPercent(row["Model %"])} green />
        <MiniBubble
          label="Implied %"
          value={ufcPercent(row["Implied %"])}
          green
        />
        <MiniBubble label="Edge" value={ufcPercent(row["Edge %"])} green />
      </div>

      <ConfidenceBar score={score || 50} />
    </div>
  );
}

function UfcFightCard({ row }: { row: SheetRow }) {
  const recommendation = String(row["Recommendation"] || "").trim();
  const isBest = recommendation.toLowerCase() === "best play";

  return (
    <div className={`card ${isBest ? "green" : ""}`}>
      <div className="cardTop">
        <div>
          <div className="cardTitle">{row["Fighter"] || "—"}</div>
          <div className="cardSub">vs {row["Opponent"] || "—"}</div>
        </div>
        <div className={`chip ${isBest ? "green" : "yellow"}`}>
          {row["Grade"] || "Pass"}
        </div>
      </div>
      ...
      <div className="marketStack">
        <div className={`marketBubble ${isBest ? "green" : ""}`}>
      ...
      <div className="bubbleGrid">
        <MiniBubble
          label="Odds"
          value={ufcDisplayOdds(row["Odds"])}
          green={isBest}
        />
        <MiniBubble
          label="Model %"
          value={ufcPercent(row["Model %"])}
          green={isBest}
        />
        <MiniBubble
          label="Edge"
          value={ufcPercent(row["Edge %"])}
          green={isBest}
        />
        <MiniBubble
          label="EZPZ Score"
          value={row["EZPZ Score"] || "—"}
          green={isBest}
        />
      </div>
    </div>
...
function UfcRecordsTable({ rows }: { rows: UfcRecordRow[] }) {
  return (
    <div className="tableWrap">
      <table>
        <thead>
        ...
        <tbody>
          {rows.map((row, index) => {
            ...
            const status = ufcRecordStatus(row);
            return (
              <tr key={`${row.Category}-${row.Period}-${index}`}>
                <td>{row.Category}</td>
                <td>{row.Period}</td>
                <td>
                  <span className={`chip ${statusClass(wins, losses)}`}>
                    {status}
                  </span>
                </td>
                <td>
...
function emptyUfcData(): UfcData {
  const empty = emptyRecord("UFC Moneyline");
  return {
    bestPlays: [],
    predictions: [],
    records: [],
    tiles: {
      bestPlaysToday: 0,
      overall: { ...empty, label: "UFC Moneyline - Running Total" },
      last7: { ...empty, label: "UFC Moneyline - Last 7 Days" },
      handpickedOverall: { ...empty, label: "UFC Handpicked - Running Total" },
      handpickedLast7: { ...empty, label: "UFC Handpicked - Last 7 Days" },
      pending: 0,
    },
  };
}
...
export default function Home() {
  ...
  const [activeUfc, setActiveUfc] = useState<UfcTab>("Best Plays");

  useEffect(() => {
    fetch("/api/public-data", { cache: "no-store" })
      .then((res) => res.json())
      .then((json) => {
        if (!json.ok) throw new Error(json.error || "Failed to load EZPZ data");
        setData(json);
      })
  ...
  const bestPlays = useMemo(() => {
    ...
    return sourceBestPlays
      .filter((play) => {
        const recentSummary =
          recentByType.get(normalizeType(play.playType)) || null;
        if (!isBestPlay(play)) return false;
        if (!passesMinimumScoreFilter(play, recentSummary, data.slateToday))
          return false;
        if (!passesColdFilter(play, recentSummary, data.slateToday))
          return false;
        if (!passesOverallRecordFilter(play, mergedOverallRecordSummary, recentSummary))
          return false;
        return true;
      })
      .sort((a, b) => {
        ...
        const aRecent = recentByType.get(normalizeType(a.playType)) || null;
        const bRecent = recentByType.get(normalizeType(b.playType)) || null;
        return (
  ...
  const ufc = data?.ufc || emptyUfcData();
  ...
  const content = useMemo(() => {
    ...
    if (activeSport === "UFC") {
      if (activeUfc === "Best Plays") {
        return (
          <>
            <div className="sectionHead">
              <div>
                <h2>UFC Best Plays</h2>
                <p>
                  Official UFC moneyline plays saved from your UFC dashboard.
                </p>
              </div>
              <span className="countPill">{ufc.bestPlays.length} plays</span>
            </div>

            {ufc.bestPlays.length ? (
              <div className="cards">
                {[
                  ...ufc.bestPlays.filter((row) => isUfcFavorite(row)),
                  ...ufc.bestPlays.filter((row) => !isUfcFavorite(row)),
                ].map((row, index) => (
                  <UfcBestPlayCard
                    key={`${row.Fighter}-${row.Opponent}-${index}`}
                    row={row}
                    index={index}
                  />
                ))}
              ...
              <div className="empty">
                No UFC Best Plays saved for {data.today} yet.
              </div>
            )}
          </>
        );
      ...
      if (activeUfc === "Full Card") {
        return (
          <>
            <div className="sectionHead">
              <div>
                <h2>UFC Full Card</h2>
                <p>Every fighter projection from the current UFC card.</p>
              </div>
            </div>

            {ufc.predictions.length ? (
              <div className="cards">
                {ufc.predictions.map((row, index) => (
                  <UfcFightCard
                    key={`${row.Fighter}-${row.Opponent}-${index}`}
                    row={row}
                  />
              ...
              <div className="empty">
                No UFC projections saved for {data.today} yet.
              </div>
            )}
          </>
        );
        ...
        <>
          <div className="sectionHead">
            <div>
              <h2>UFC Records</h2>
              <p>
                Moneyline performance from your UFC bet tracker and records
                engine.
              </p>
            </div>
          </div>
          ...
          <div className="qualifiedGrid">
            <Tile
              label="UFC Last 7 Days"
              value={ufc.tiles.last7.record}
              meta={`${ufc.tiles.last7.winPct}% • ${ufc.tiles.last7.unitsWon}u • ROI ${ufc.tiles.last7.roiPct}%`}
              green={ufc.tiles.last7.totalBets > 0}
            />
            <Tile
              label="UFC Running Total"
              value={ufc.tiles.overall.record}
              meta={`${ufc.tiles.overall.winPct}% • ${ufc.tiles.overall.unitsWon}u • ROI ${ufc.tiles.overall.roiPct}%`}
              green={ufc.tiles.overall.totalBets > 0}
            />
            <Tile
              label="UFC Handpicked Last 7"
              value={ufc.tiles.handpickedLast7.record}
              meta={`${ufc.tiles.handpickedLast7.winPct}% • ${ufc.tiles.handpickedLast7.unitsWon}u • ROI ${ufc.tiles.handpickedLast7.roiPct}%`}
              green={ufc.tiles.handpickedLast7.totalBets > 0}
            />
            <Tile
              label="UFC Handpicked Total"
              value={ufc.tiles.handpickedOverall.record}
              meta={`${ufc.tiles.handpickedOverall.winPct}% • ${ufc.tiles.handpickedOverall.unitsWon}u • ROI ${ufc.tiles.handpickedOverall.roiPct}%`}
              green={ufc.tiles.handpickedOverall.totalBets > 0}
            />
            <Tile
              label="Pending UFC Bets"
              value={String(ufc.tiles.pending)}
              meta="Open tracker rows"
              green={ufc.tiles.pending > 0}
            />
            <Tile
              label="Today’s UFC Best Plays"
              value={String(ufc.tiles.bestPlaysToday)}
              meta="Saved public plays"
              green={ufc.tiles.bestPlaysToday > 0}
            />
          </div>

          {ufc.records.length ? (
            <UfcRecordsTable rows={ufc.records} />
          ) : (
            <div className="empty">No UFC records saved yet.</div>
          )}
        </>
      );
    }
        ...
        <>
          <div className="sectionHead">
            <div>
              ...
              <p>
                Ranked by EZPZ score, odds rules, recent form, and long-term
        ...
        <>
          <div className="sectionHead">
            <div>
              <h2>Full Slate</h2>
              <p>
                Every saved game for {data.today}. Green highlights show
  ...
  return (
    <main className="shell">
      <section className="hero">
        ...
        <p className="heroSub">
          Algorithm-driven betting projections ranked by model edge, confidence,
          and long-term performance across every EZPZ sport.
        </p>
      </section>
      ...
      {data ? (
        <section className="tileGrid">
          {activeSport === "UFC" ? (
            <>
              <Tile
                label="UFC Best Plays Today"
                value={String(ufc.tiles.bestPlaysToday)}
                meta="Official moneyline plays"
                green={ufc.tiles.bestPlaysToday > 0}
              />
              <Tile
                label="UFC Last 7 Days"
      ...
      <nav className="tabs">
        {activeSport === "UFC"
          ? UFC_TABS.map((tab) => (
              <button
                key={tab}
                className={`tabBtn ${activeUfc === tab ? "active" : ""}`}
                onClick={() => setActiveUfc(tab)}
              >
                {tab}
              </button>
            ))
      ...
      <style jsx global>{`
        .handpicked {
          ...
          box-shadow:
            0 0 0 1px rgba(79, 156, 255, 0.35),
            0 18px 55px rgba(37, 99, 235, 0.28) !important;
        }
        .handpickedPill {
