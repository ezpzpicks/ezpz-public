...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    if (active === "Today’s Best Plays") return (
      <>
        <div className="sectionHead"><div><h2>Today’s Best Plays</h2><p>Today’s strongest qualified Best Plays, boosted with 7-day form tags. Last updated: {data.lastUpdated}</p></div></div>
        {bestPlaysFiltered.length ? <div className="cards">{[...bestPlaysFiltered].sort((a, b) => parseScore(b.score) - parseScore(a.score)).map((p, i) => <BestPlayCard key={`${p.game}-${p.play}-${i}`} play={p} index={i} recentSummary={recentByType.get(normalizeType(p.playType)) || null} />)}</div> : <div className="empty">No qualified Best Plays saved yet for {data.today}.</div>}
      </>
    );
      ...
      <main className="shell">
        ...
        {data ? (
          <section className="tileGrid">
            ...
            <Tile label="Today’s Best Plays" value={String(visibleBestPlaysToday)} meta="Pending qualified Best Plays" green={visibleBestPlaysToday > 0} />
          </section>
        ) : null}
