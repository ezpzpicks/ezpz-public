...
function AiPickSelectorCard({ pick }: { pick: AiPick }) {
  const schedule = scheduleInfoFromRaw(pick.gameTime, pick.date);
  const isPendingReview = pick.externalReviewStatus === "PENDING_FINAL_REVIEW";
  ...
  const why = cleanedWhy.length
    ? cleanedWhy
    : ["The candidate passed the EZPZ AI selection threshold and protection checks."];
  const historicalNotes = cleanAiDisplayList(pick.historicalNotes);
  const cleanedRisks = cleanAiDisplayList(pick.risks);
  const risks = cleanedRisks.length ? cleanedRisks : ["No significant concerns identified after final verification."];
  ...
  return (
    <details className="aiPickDropdown">
      <summary className="aiPickSummary">
        ...
        <div className="aiPickSummaryMain">
          <div className="aiPickSummaryMeta">
            <span>{pick.game}</span>
            {isPendingReview ? <span className="aiPendingBadge">PENDING</span> : null}
          </div>
      ...
      <div className="aiPickExpanded">
        ...
        <section className="aiPickDetailSection risk">
          <h3>Risks</h3>
          <ul>
            {risks.map((item, index) => <li key={`risk-${pick.candidateId}-${index}`}>{item}</li>)}
          </ul>
        </section>
...
function TrendTierRecords({
  ...
  const [period, setPeriod] = useState<"today" | "yesterday" | "all" | "30" | "7">("today");
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    const aiPicks = (data.aiPicks || []).filter((pick) => pick.selected);
        ...
        <section>
          ...
          {aiPicks.length ? (
            <div className="aiPickStack">
              {aiPicks.map((pick) => (
                <AiPickSelectorCard key={pick.candidateId} pick={pick} />
              ))}
            </div>
          ) : (
            <div className="empty">
    ...
    <main className="shell">
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .aiPickDetailSection.risk {
          border-color: rgba(247, 200, 92, 0.2);
          background: rgba(247, 200, 92, 0.045);
        }
