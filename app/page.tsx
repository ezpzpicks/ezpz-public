...
function AiPickSelectorCard({ pick }: { pick: AiPick }) {
  ...
  const confidenceReason = pick.confidenceReason?.length
    ? pick.confidenceReason
    : ["This play cleared the AI score, probability, value, and protection thresholds after final verification."];
  const why = pick.whySelected?.length
    ? pick.whySelected
    : ["The candidate passed the EZPZ AI selection threshold and protection checks."];
  const historicalNotes = pick.historicalNotes || [];
  const risks = pick.risks?.length ? pick.risks : ["No significant concerns identified after final verification."];
  const dataStatus = [
    ...(pick.dataStatus || []),
