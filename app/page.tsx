...
function formInfo(summary: Summary | null) {
  if (!summary || summary.totalBets < 2) return { label: "Neutral", icon: "➖", className: "neutral", detail: "small 7-day sample" };
...
function SlateCard({ row, greenSet }: { row: SheetRow; greenSet: Set<string> }) {
  ...
  return (
    <div className={`card ${hasGreen ? "green" : ""}`}>
      ...
      {hasGreen ? <div className="slateGreenCallout">Green bet type active</div> : null}
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    const bestPlaysFiltered = data.bestPlays.filter((p) => normalizeType(p.playType) !== "NON-EDGE MONEYLINE");

    if (active === "Today’s Best Plays") return (
      <>
        <div className="sectionHead"><div><h2>Today’s Best Plays</h2><p>Today’s strongest overall green bet types, boosted with 7-day form tags. Last updated: {data.lastUpdated}</p></div></div>
        {bestPlaysFiltered.length ? <div className="cards">{[...bestPlaysFiltered].sort((a, b) => parseScore(b.score) - parseScore(a.score)).map((p, i) => <BestPlayCard key={`${p.game}-${p.play}-${i}`} play={p} index={i} recentSummary={recentByType.get(normalizeType(p.playType)) || null} />)}</div> : <div className="empty">No overall green best plays saved yet for {data.today}.</div>}
      </>
    );

    if (active === "Full Slate") return (
      <>
        <div className="sectionHead"><div><h2>Full Slate</h2><p>Every saved game for {data.today}. Only all-time green best-play bet types use green bubbles; recent form appears as Hot / Neutral / Cold.</p></div></div>
  ...
  const visibleBestPlaysToday = data
    ? data.bestPlays.filter((p) => normalizeType(p.playType) !== "NON-EDGE MONEYLINE").length
    : 0;
    ...
    <>
      ...
      <main className="shell">
        <section className="hero">
          ...
          <p className="heroSub">Algorithm-driven MLB betting projections, ranked by model edge, all-time bet-type record, and 7-day form.</p>
        </section>
        {data ? (
          <section className="tileGrid">
            ...
            <Tile label="Today’s Best Plays" value={String(visibleBestPlaysToday)} meta="Pending overall green plays" green={visibleBestPlaysToday > 0} />
          </section>
        ) : null}
