...
export default function Home() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      const [publicResult, draftKingsResult] = await Promise.allSettled([
        fetch("/api/public-data", { cache: "no-store" }),
        fetch("/api/draftkings", { cache: "no-store" }),
      ]);

      if (publicResult.status === "rejected") throw publicResult.reason;
      const json = await publicResult.value.json();
      if (!publicResult.value.ok || !json.ok) {
        throw new Error(json.error || "Failed to load EZPZ data");
      }
      setData(json);
      setError("");

      if (draftKingsResult.status === "fulfilled") {
        const liveJson = (await draftKingsResult.value.json()) as DraftKingsData;
        if (draftKingsResult.value.ok && liveJson.ok) {
          setDraftKings(liveJson);
          setDraftKingsError("");
        } else {
          setDraftKingsError(liveJson.errors?.join(" • ") || "DraftKings feed unavailable");
        }
      } else {
        setDraftKingsError(
          draftKingsResult.reason instanceof Error
            ? draftKingsResult.reason.message
            : "DraftKings feed unavailable",
        );
      }
    } catch (err) {
