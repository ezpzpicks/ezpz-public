...
function isFavoriteValue(value: unknown) {
  // Handpicked must be controlled only by the real Google Sheets Favorite Pick cell.
  // Do not accept fallback words like "handpicked", "favorite", "x", etc.
...
function trackerSelectionText(row: SheetRow) {
  return normalizeMatchText(row["Selection"] || row["Play"] || row["Pick"] || row["Player"] || "");
}

function playIdentityText(play: Play) {
  if (isMoneylineType(play.playType)) {
    return normalizeMatchText(cleanMoneylineTeam(play.playerTeam || play.play));
  }

  if (isKType(play.playType)) {
    return normalizeMatchText(cleanPitcherName(play.play));
  }

  if (isNRFIType(play.playType)) {
    return normalizeMatchText(play.game || play.play);
  }

  return normalizeMatchText(play.play);
}

function exactFavoriteRowMatchesPlay(row: SheetRow, play: Play, today: string) {
  if (!isFavoriteValue(row["Favorite Pick"])) return false;
  if (!sameDateText(row.Date || row.date || row["Bet Date"] || "", today)) return false;

  const rowType = normalizeType(row["Bet Type"] || row["Market"] || "");
  const playType = normalizeType(play.playType);
  const rowMarket = normalizeMatchText(row["Market"] || "");
  const selection = trackerSelectionText(row);
  const identity = playIdentityText(play);

  if (!selection || !identity) return false;

  if (isMoneylineType(playType)) {
    if (!isMoneylineType(rowType) && rowMarket !== "moneyline") return false;
    return selection === identity;
  }

  if (isKType(playType)) {
    if (!isKType(rowType) && rowMarket !== "pitcher strikeouts") return false;
    return selection === identity || selection.startsWith(`${identity} `) || identity.startsWith(`${selection} `);
  }

  if (isNRFIType(playType)) {
    if (!isNRFIType(rowType) && !rowMarket.includes("nrfi") && !rowMarket.includes("yrfi")) return false;
    const rowGame = normalizeMatchText(row["Game"] || row["Game Label"] || row["Matchup"] || row["Selection"] || "");
    const playGame = normalizeMatchText(play.game || "");
    // NRFI/YRFI is intentionally strict because loose same-game fallback caused false badges.
    return rowType === playType && rowGame === playGame;
  }

  return rowType === playType && selection === identity;
}

function getExactFavoriteRowForPlay(play: Play, rows: SheetRow[] | undefined, today: string) {
  if (!rows?.length || !today) return null;
  return rows.find((row) => exactFavoriteRowMatchesPlay(row, play, today)) || null;
}
...
export default function Home() {
  ...
  const handpickedPlays = useMemo(() => {
    return bestPlays
      .filter((play) => Boolean(getExactFavoriteRowForPlay(play, data?.betTrackerRows, data?.today || "")))
          ...
          {bestPlays.length ? (
            <div className="cards">
              {bestPlays.map((play, index) => {
                const favoriteRow = getExactFavoriteRowForPlay(play, data.betTrackerRows, data.today);
                const displayPlay = favoriteRow
                  ? {
                      ...play,
                      favoritePick: "TRUE",
                      favoriteRank: favoriteRow["Favorite Rank"] || play.favoriteRank,
                      favoriteTag: favoriteRow["Favorite Tag"] || play.favoriteTag,
                      favoriteNotes: favoriteRow["Favorite Notes"] || play.favoriteNotes,
                    }
