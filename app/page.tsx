...
type Play = { playType: string; game: string; play: string; oddsLine: string; score: string | number; isGreen: boolean; awayTeam: string; homeTeam: string; headshotUrl?: string; };
...
const TEAM_IDS: Record<string, number> = {
  ...
  "Los Angeles Angels": 108, "LA Angels": 108, "Los Angeles Dodgers": 119, "Miami Marlins": 146,
  "Milwaukee Brewers": 158, "Minnesota Twins": 142, "New York Mets": 121, "New York Yankees": 147,
  "Athletics": 133, "Oakland Athletics": 133, "Philadelphia Phillies": 143, "Pittsburgh Pirates": 134,
  "San Diego Padres": 135, "San Francisco Giants": 137, "Seattle Mariners": 136, "St. Louis Cardinals": 138,
...
function TeamLogos({ row }: { row: SheetRow }) {
  const awayLogo = imageFromRow(row, ["Away Team Logo", "Away Logo", "Away Logo URL"]) || teamLogoUrl(row["Away Team"] || "");
  const homeLogo = imageFromRow(row, ["Home Team Logo", "Home Logo", "Home Logo URL"]) || teamLogoUrl(row["Home Team"] || "");
  return <div className="teamPair">{awayLogo && <img className="teamLogo" src={awayLogo} alt={`${row["Away Team"]} logo`} />}<span className="vsText">AT</span>{homeLogo && <img className="teamLogo" src={homeLogo} alt={`${row["Home Team"]} logo`} />}</div>;
}

function PitcherPhoto({ row, side, summary, url: directUrl }: { row?: SheetRow; side?: "Away" | "Home"; summary: string; url?: string }) {
  const url = directUrl || (row && side ? imageFromRow(row, [`${side} Pitcher Headshot`, `${side} Pitcher Headshot URL`, `${side} Pitcher Image`, `${side} Pitcher Image URL`]) : "");
...
function BestPlayTeamLogos({ play }: { play: Play }) {
  ...
  return <div className="teamPair">{awayLogo && <img className="teamLogo" src={awayLogo} alt={`${play.awayTeam} logo`} />}<span className="vsText">AT</span>{homeLogo && <img className="teamLogo" src={homeLogo} alt={`${play.homeTeam} logo`} />}</div>;
}
...
function BestPlayCard({ play }: { play: Play }) {
  return (
    <div className="card green">
      ...
      <div className="cardMedia"><BestPlayTeamLogos play={play} /></div>
      <div className="playMain">
        <PitcherPhoto summary={play.play} url={play.headshotUrl} />
        <div className="playMainText">
          <div className="playName">{play.play}</div>
          <div className="playDetail">{play.oddsLine || "Manual line"}</div>
        </div>
      </div>
      <span className="chip green">GREEN BEST PLAY</span>
...
function SlateCard({ row, greenSet }: { row: SheetRow; greenSet: Set<string> }) {
  ...
  const rawChips = [
    { label: "ML", value: row["ML Grade"] && row["ML Grade"] !== "PASS" ? `${row["Better ML"]} • ${row["ML Grade"]}` : "", type: row["ML Grade"] },
    { label: "NRFI", value: row["NRFI Grade"] && row["NRFI Grade"] !== "PASS" ? row["NRFI Grade"] : "", type: row["NRFI Grade"] },
    { label: "Away K", value: row["Away Pitcher K + Grade"] && !row["Away Pitcher K + Grade"].includes("PASS") ? row["Away Pitcher K + Grade"] : "", type: row["Away Pitcher K + Grade"] },
    { label: "Home K", value: row["Home Pitcher K + Grade"] && !row["Home Pitcher K + Grade"].includes("PASS") ? row["Home Pitcher K + Grade"] : "", type: row["Home Pitcher K + Grade"] },
  ].filter((chip) => chip.value);
  const chips = rawChips.map((chip) => ({ ...chip, green: greenSet.has(normalizeType(chip.type || "")) }));
  const hasGreen = chips.some((chip) => chip.green);
  ...
  return (
    <div className={`card ${hasGreen ? "green" : ""}`}>
      ...
      <div className="cardMedia"><TeamLogos row={row} /></div>
      <div className="playMain">
        <PitcherPhoto row={row} side="Away" summary={row["Away Pitcher K + Grade"] || "Away Pitcher"} />
        <div className="playMainText"><div className="playName">Away Pitcher</div><div className="playDetail">{row["Away Pitcher K + Grade"] || "No K projection"}</div></div>
      </div>
      <div className="playMain">
        <PitcherPhoto row={row} side="Home" summary={row["Home Pitcher K + Grade"] || "Home Pitcher"} />
        <div className="playMainText"><div className="playName">Home Pitcher</div><div className="playDetail">{row["Home Pitcher K + Grade"] || "No K projection"}</div></div>
      </div>
      <div>
        {chips.length ? chips.map((chip) => <span key={chip.label} className={`chip ${chip.green ? "green" : "soft"}`}>{chip.label}: {chip.value}</span>) : <span className="chip soft">No qualifying plays</span>}
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    const greenSet = new Set(data.recordSummary.filter((r) => r.wins > r.losses).map((r) => r.betType));
    ...
    if (active === "Full Slate") return <><div className="sectionHead"><div><h2>Full Slate</h2><p>Every saved game for {data.today}. Only green best plays use green bubbles.</p></div></div>{data.slateToday.length ? <div className="cards">{data.slateToday.map((row, i) => <SlateCard key={`${row["Game ID"]}-${i}`} row={row} greenSet={greenSet} />)}</div> : <div className="empty">No games saved today yet.</div>}</>;
  ...
  return (
    <main className="shell">
      ...
      {data && <section className="tileGrid"><RecordTile title="Last 7 Days Green Bets" totals={data.tiles.last7Days} /><RecordTile title="Overall Green Bets" totals={data.tiles.overallGreen} /><Tile label="Best Plays Today" value={data.tiles.bestPlaysToday} meta="Green plays on today’s board" green /></section>}
