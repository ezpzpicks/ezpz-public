...
function cleanTrackerPitcherSelection(value: unknown) {
  const raw = String(value ?? "").trim();
  const beforeLine = raw.replace(/Line.*$/i, "");
  const beforeNumbers = beforeLine.replace(/\d+(?:\.\d+)?\+?\s*(?:so|k|ks|strikeouts?)?.*$/i, "");
  return favoriteKeyText(beforeNumbers || raw);
}
...
function favoriteKeyFromPlay(play: Play, today: string) {
  ...
  if (isKType(type)) {
    return `K|${dateKey}|${favoriteKeyText(cleanPitcherName(play.play))}`;
  }
