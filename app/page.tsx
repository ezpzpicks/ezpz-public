...
function isValidAmericanOdds(value: number, signed: boolean) {
  if (!Number.isFinite(value)) return false;
  const abs = Math.abs(value);
  if (isLikelyYear(abs)) return false;
  if (abs < 100) return false;

  // Unsigned odds in our sheets are usually 100-999.
  // This prevents dates like 2026-05-04 from becoming +2026.
  if (!signed && abs > 999) return false;

  // Signed longshots can exist, but anything that looks like a year/date should be ignored above.
  return abs <= 2000;
}
...
function parseAmericanOdds(value: unknown) {
  ...
  // Some sheet cells look like "Line 5.5 -120". Grab the odds number, not the line/date.
  const signedMatches = [...raw.matchAll(/[+-]\d{3,4}/g)].map((match) => Number(match[0]));
  const signedOdds = signedMatches.find((odds) => isValidAmericanOdds(odds, true));
  if (signedOdds) return signedOdds;

  const allMatches = [...raw.matchAll(/\b\d{3,4}\b/g)].map((match) => Number(match[0]));
  const plainOdds = allMatches.find((odds) => isValidAmericanOdds(odds, false));
  return plainOdds || 0;
}
...
function getPlayableOdds(play: Play, slateRows: SheetRow[] = []) {
  ...
  const row = findSlateRowForPitcher(play, slateRows);
  ...
  const genericOdds =
    ...
    firstOddsByKeyMatch(row, ["prop", "odds"], ["moneyline", "ml", "nrfi", "yrfi"]) ||
    firstOddsByKeyMatch(row, ["k", "odds"], ["moneyline", "ml", "nrfi", "yrfi"]) ||
    firstOddsByKeyMatch(row, ["so", "odds"], ["moneyline", "ml", "nrfi", "yrfi"]) ||
    firstOddsByKeyMatch(row, ["strikeout", "odds"], ["moneyline", "ml", "nrfi", "yrfi"]);

  if (genericOdds) return genericOdds;

  // Final fallback: only use fields that are plausibly odds-related.
  // Do NOT scan every cell, because dates like 2026-05-04 can be mistaken for +2026.
  for (const [key, value] of Object.entries(row)) {
    const keyLower = key.toLowerCase();
    const looksLikeOddsField = ["odds", "price", "juice"].some((good) => keyLower.includes(good));
    if (!looksLikeOddsField) continue;
    if (["moneyline", "ml", "nrfi", "yrfi", "date", "game id"].some((bad) => keyLower.includes(bad))) continue;
    const odds = formatOdds(value);
    if (odds !== "—" && parseAmericanOdds(odds)) return odds;
  }
...
function rowHasPitcher(row: SheetRow, pitcherName: string) {
  if (!pitcherName) return false;
  return Object.entries(row).some(([key, value]) => {
    const keyLower = key.toLowerCase();
    if (!keyLower.includes("pitcher")) return false;
    if (keyLower.includes("headshot") || keyLower.includes("image") || keyLower.includes("photo") || keyLower.includes("url")) return false;
    return pitcherNameMatches(pitcherName, cleanPitcherName(String(value || "")));
  });
}

function findSlateRowForPitcher(play: Play, rows: SheetRow[]) {
  const byGame = findSlateRowForPlay(play, rows);
  if (byGame) return byGame;

  const pitcherName = cleanPitcherName(play.play);
  return rows.find((row) => rowHasPitcher(row, pitcherName));
}
...
function pitcherMlbIdFromRow(row: SheetRow | undefined, side: string) {
  if (!row) return "";
  const sideTitle = side === "away" ? "Away" : side === "home" ? "Home" : "";
  const directKeys = sideTitle
    ? [
        `${sideTitle} Pitcher ID`,
        `${sideTitle} Pitcher MLB ID`,
        `${sideTitle} Pitcher Player ID`,
        `${sideTitle} Player ID`,
        `${sideTitle} MLBAM ID`,
      ]
    : ["Pitcher ID", "Pitcher MLB ID", "Pitcher Player ID", "Player ID", "MLBAM ID"];

  const direct = firstValue(row, directKeys);
  if (direct) return mlbHeadshotUrlFromId(direct);

  if (side) {
    for (const [key, value] of Object.entries(row)) {
      const keyLower = key.toLowerCase();
      if (keyLower.includes(side) && keyLower.includes("pitcher") && keyLower.includes("id")) {
        const url = mlbHeadshotUrlFromId(value);
        if (url) return url;
      }
    }
  }
...
function imageForBestPlay(play: Play, rows: SheetRow[]) {
  const direct = extractUrl(play.headshotUrl);
  if (direct) return direct;
  if (!isKType(play.playType)) return "";

  const row = findSlateRowForPitcher(play, rows);
  if (!row) return "";
  ...
  if (sideTitle) {
    ...
    const idImage = pitcherMlbIdFromRow(row, side);
    if (idImage) return idImage;
  }

  const genericIdImage = pitcherMlbIdFromRow(row, side);
  if (genericIdImage) return genericIdImage;

  // Last resort: return any pitcher/player image URL on the matched row.
  return (
    firstUrlByKeyMatch(row, ["pitcher", "headshot"]) ||
    firstUrlByKeyMatch(row, ["pitcher", "image"]) ||
    firstUrlByKeyMatch(row, ["pitcher", "photo"]) ||
    firstUrlByKeyMatch(row, ["player", "headshot"]) ||
    firstUrlByKeyMatch(row, ["player", "image"]) ||
    firstUrlByKeyMatch(row, ["photo"]) ||
    firstUrlByKeyMatch(row, ["image"], ["logo", "team"]) ||
    ""
  );
}
...
function isHotPlay(play: Play, recentRows: Summary[]) {
  const summary = getRecentSummary(play.playType, recentRows);
  return getFormInfo(summary).className === "hot";
}

function compareBestPlays(a: Play, b: Play, recentRows: Summary[]) {
  const aHot = isHotPlay(a, recentRows) ? 1 : 0;
  const bHot = isHotPlay(b, recentRows) ? 1 : 0;
...
export default function Home() {
  ...
  const bestPlays = useMemo(() => {
    ...
    return data.bestPlays
      .filter(isBestPlay)
      .sort((a, b) => compareBestPlays(a, b, data.last7RecordSummary));
  }, [data]);
