...
function getTotalProjection(play: Play) {
  ...
  return Number.isFinite(n) && n > 0 ? n.toFixed(1) : raw;
}
...
function totalPlayFromTrackerRow(row: SheetRow): Play | null {
  ...
  return {
    ...
    score: row.Projection || row.Projected || row["Model Projection"] || row.Score || "",
    isGreen: true,
...
function mergeBestPlaysWithTrackerTotals(apiPlays: Play[] | undefined, trackerTotalPlays: Play[]) {
  ...
  const existingKeys = new Set(merged.filter((play) => isTotalType(play.playType)).map(trackerTotalPlayKey));
