...
function computedPublicWarning(betsPct?: number, moneyPct?: number) {
  ...
  if (betsPct >= 90 && moneyPct >= 90) {
    return { label: "Extreme Public Consensus", negative: true };
  }
  if (betsPct >= 80 && moneyPct >= 80) {
    return { label: "Heavy Public Consensus", negative: true };
  }
  if (betsPct >= 80 && betsPct - moneyPct >= 15) {
    return { label: "Major Public/Money Conflict", negative: true };
  }
  if (betsPct - moneyPct >= 10) {
    return { label: "Weak Money Support", negative: true };
  }
  if (betsPct < 50 && moneyPct - betsPct >= 10) {
    return { label: "Contrarian Money Support", negative: false };
  }
  if (betsPct < 80 && moneyPct - betsPct >= 10) {
    return { label: "Money Support", negative: false };
  }
  return { label: "", negative: false };
}
...
function PublicBettingPanel({ info }: { info: PublicBettingInfo | null }) {
  ...
  const warningClass = info.warningNegative ? "negative" : "positive";
  const warningIcon = info.warningNegative ? "⚠" : "↗";
  return (
    <div className="publicSplitPanel">
      <div className="publicSplitTitle">
        <span>{info.source} Public Splits</span>
        <strong>{info.selection || "Selected side"}</strong>
      </div>
      <div className="bubbleGrid publicSplitGrid">
        <MiniBubble label="Public Bets" value={publicPctText(info.betsPct)} />
        <MiniBubble label="Public Money" value={publicPctText(info.moneyPct)} />
        <MiniBubble
          label="Money − Bets"
          value={
            info.gapPct == null
        ...
        />
        <MiniBubble label="Split Odds" value={info.odds || "—"} />
      </div>
      {info.warning ? (
...
function BestPlayCard({
  ...
  const publicBettingInfo = getPublicBettingInfo(play, slateRows);
...
function compactSlateGrade(value: string) {
  const grade = normalizeType(value || "");
  if (!grade) return "PASS";
  if (grade === "A MONEYLINE") return "A ML";
  if (grade === "B MONEYLINE") return "B ML";
  if (grade === "NON-EDGE MONEYLINE") return "ML";
  if (grade === "ELITE NRFI") return "ELITE NRFI";
  if (grade === "ELITE YRFI") return "ELITE YRFI";
  return grade;
}

function slateMoneylineModelPct(row: SheetRow) {
  return formatModelPct(
    firstValue(row, [
      "Model %",
      "Win %",
      "Moneyline %",
...
function SlateMetric({
  ...
  return (
    <div className={`slateMetric ${accent ? "accent" : ""}`}>
      <span>{label}</span>
      <strong>{value || "—"}</strong>
    </div>
  );
...
function SlateQuickTile({
  ...
  return (
    <div className={`slateQuickTile ${green ? "green" : ""}`}>
      <div className="slateQuickTop">
        <span>{label}</span>
        {green ? <i aria-hidden="true" /> : null}
      </div>
      <strong>{value || "—"}</strong>
      <small>{meta || "Projection only"}</small>
    </div>
  );
}

function SlatePublicSplitInline({
  row,
  prefix,
}: {
  row: SheetRow;
  ...
  const betsPct = publicPctNumber(firstValue(row, [`${prefix} Public Bets %`]));
  const moneyPct = publicPctNumber(firstValue(row, [`${prefix} Public Money %`]));
  if (betsPct == null && moneyPct == null) return null;

  const savedWarning = firstValue(row, [`${prefix} Public Warning`]);
  const computed = computedPublicWarning(betsPct, moneyPct);
  const warning = savedWarning || computed.label;
  const negativeRaw = firstValue(row, [`${prefix} Public Warning Negative`]);
  const negative = hasSheetValue(negativeRaw)
    ? truthySheetFlag(negativeRaw)
    : computed.negative;
  const gap =
    betsPct != null && moneyPct != null
      ? Math.round((moneyPct - betsPct) * 10) / 10
  ...
  return (
    <div className="slateSplitLine">
      <span className="slateSplitSource">
        {firstValue(row, [`${prefix} Public Split Source`]) || "DraftKings"}
      </span>
      <span>Bets {publicPctText(betsPct)}</span>
      <span>Money {publicPctText(moneyPct)}</span>
      <span>
        Gap {gap == null ? "—" : `${gap >= 0 ? "+" : ""}${gap.toFixed(1)}%`}
      </span>
      {warning ? (
        <strong className={negative ? "negative" : "positive"}>{warning}</strong>
      ) : null}
    </div>
  );
}

function SlateMarketRow({
  eyebrow,
  title,
  subtitle,
  grade,
  ...
  return (
    <section className={`slateMarketRow ${isGreen ? "green" : ""}`}>
      <div className="slateMarketLead">
        <div>
          <span className="slateMarketEyebrow">{eyebrow}</span>
          <h4>{title || "Projection unavailable"}</h4>
          {subtitle ? <p>{subtitle}</p> : null}
        </div>
        <span className={`slateGradeChip ${isGreen ? "green" : "pass"}`}>
          {compactSlateGrade(grade)}
        </span>
      </div>
      <div className="slateMetricGrid">{children}</div>
      {footer ? <div className="slateMarketFooter">{footer}</div> : null}
    </section>
  );
}

function SlatePitcherRow({
  label,
  role,
  summary,
  score,
  ...
  const rank = firstValue(row, [`${popularityPrefix} Prop Popularity Rank`]);
  const popularityFlag = truthySheetFlag(
    firstValue(row, [`${popularityPrefix} Prop Popularity Flag`]),
  );
  const popularitySource =
    firstValue(row, [`${popularityPrefix} Prop Popularity Source`]) || "DraftKings";

  return (
    <SlateMarketRow
      eyebrow={`${label} • ${role}`}
      title={cleanPitcherName(summary) || label}
      subtitle={team}
      grade={normalizeType(summary)}
      isGreen={isGreen}
      footer={
        rank ? (
          <div className="slatePopularityLine">
            <span>{popularitySource} popularity</span>
            <strong>#{Math.round(toNumber(rank))}</strong>
            <span>{popularityFlag ? "Exact side/line match" : "Pitcher popularity match"}</span>
          </div>
        ) : null
      }
    >
      <div className="slatePitcherIdentity">
        <PitcherPhoto summary={summary} url={imageUrl} team={team} />
      </div>
      <SlateMetric label="Line" value={extractLine(summary)} accent={isGreen} />
      <SlateMetric label="Projected Ks" value={extractProjectedK(summary)} accent={isGreen} />
      <SlateMetric label="Hit Probability" value={percentText(probability)} />
      <SlateMetric
        label="Reliability"
        value={reliability ? `${Math.round(toNumber(reliability))}/100` : "—"}
      />
      <SlateMetric label="EZPZ Score" value={score || "—"} />
    </SlateMarketRow>
  );
}
...
function SlateCard({ row }: { row: SheetRow }) {
  ...
  const awayTeam = cleanTeamName(row["Away Team"] || "");
  const homeTeam = cleanTeamName(row["Home Team"] || "");
  ...
  const hasGreen = qualifiedCount > 0;
  const version = displayModelVersion(String(row["Model Version"] || ""));
  const gameTime = firstValue(row, [
    "Game Time",
    "Start Time",
    "Scheduled Time",
    "First Pitch",
  ...
  const mlPick = row["Better ML"] || "—";
  const mlOdds = formatOdds(row["ML Odds"] || "—");
  const mlModelPct = slateMoneylineModelPct(row);
  const totalLine = row["Total Runs Line"] || "—";
  const totalProjection = row["Total Runs Projection"] || "—";
  const totalProbability = percentText(row["Total Selected Probability"] || "");
  const totalReliability = row["Total Reliability"]
    ? `${Math.round(toNumber(row["Total Reliability"]))}/100`
    : "—";
  const totalDirection = totalType.includes("UNDER")
    ? "Under"
    : totalType.includes("OVER")
      ? "Over"
      : "Pass";
  const pitcherProjectionCount = [awayK, homeK, awayBulkK, homeBulkK].filter(Boolean)
    .length;
  const qualifiedPitcherCount = [
    awayGreen,
    homeGreen,
    awayBulkGreen,
    homeBulkGreen,
  ...
  return (
    <details className={`slateAccordion ${hasGreen ? "green" : ""}`}>
      <summary className="slateSummary">
        <div className="slateSummaryTop">
          <div className="slateMatchupBlock">
            <div className="slateLogoStack" aria-hidden="true">
              {awayLogo ? <img src={awayLogo} alt="" loading="lazy" /> : null}
              {homeLogo ? <img src={homeLogo} alt="" loading="lazy" /> : null}
            </div>
            <div className="slateMatchupCopy">
              <div className="slateMatchupTitle">{game}</div>
              <div className="slateMatchupMeta">
                {gameTime ? <span>{gameTime}</span> : null}
                <span>{version ? `Model ${version}` : "Calibrated projection"}</span>
              </div>
            </div>
          </div>

          <div className="slateSummaryActions">
            <span className={`slateQualifiedPill ${hasGreen ? "green" : "pass"}`}>
              {hasGreen
                ? `${qualifiedCount} qualified ${qualifiedCount === 1 ? "play" : "plays"}`
                : "Projection only"}
            </span>
            <span className="slateChevron" aria-hidden="true" />
          </div>
        </div>

        <div className="slateQuickGrid">
          <SlateQuickTile
            label="Moneyline"
            value={`${mlPick}${mlOdds !== "—" ? ` ${mlOdds}` : ""}`}
            meta={compactSlateGrade(mlType)}
            green={mlGreen}
          />
          <SlateQuickTile
            label="Game Total"
            value={`${totalDirection === "Pass" ? "Line" : totalDirection} ${totalLine}`}
            meta={`Projection ${totalProjection}`}
            green={totalGreen}
          />
          <SlateQuickTile
            label="First Inning"
            value={firstInningType || "PASS"}
            meta={percentText(firstInningProbability)}
            green={firstInningGreen}
          />
          <SlateQuickTile
            label="Pitcher Ks"
            value={`${pitcherProjectionCount} projection${pitcherProjectionCount === 1 ? "" : "s"}`}
            meta={
              qualifiedPitcherCount
                ? `${qualifiedPitcherCount} qualified`
                : "No qualified K play"
            }
            green={qualifiedPitcherCount > 0}
          />
        </div>
      </summary>

      <div className="slateExpanded">
        <div className="slateExpandedHead">
          <div>
            <span>Full model breakdown</span>
            <strong>All markets and pitcher projections</strong>
          </div>
          <div className="slateExpandedBadges">
        ...
        <div className="slateMarketStack">
          {row["ML Grade"] ? (
            <SlateMarketRow
              eyebrow="Moneyline"
              title={mlPick}
              subtitle="Model-selected side"
              grade={mlType}
              isGreen={mlGreen}
              footer={<SlatePublicSplitInline row={row} prefix="ML" />}
            >
              <SlateMetric label="Odds" value={mlOdds} accent={mlGreen} />
              <SlateMetric label="Model Win %" value={mlModelPct} accent={mlGreen} />
              <SlateMetric
                label="Market Implied"
                value={
                  americanOddsImpliedPercent(mlOdds)
                    ? `${americanOddsImpliedPercent(mlOdds).toFixed(1)}%`
                    : "—"
                }
              />
              <SlateMetric
                label="Model Edge"
                value={moneylineEdgeText(mlModelPct, mlOdds)}
              />
            </SlateMarketRow>
          ) : null}
          ...
          {row["Total Runs Projection"] || totalType ? (
            <SlateMarketRow
              eyebrow="Game Total"
              title={`${totalDirection} ${totalLine}`}
              subtitle="Full-game calibrated run projection"
              grade={totalType || "PASS"}
              isGreen={totalGreen}
              footer={<SlatePublicSplitInline row={row} prefix="Total" />}
            >
              <SlateMetric label="Line" value={totalLine} accent={totalGreen} />
              <SlateMetric
                label="Projected Runs"
                value={totalProjection}
                accent={totalGreen}
              />
              <SlateMetric label="Hit Probability" value={totalProbability} />
              <SlateMetric label="Reliability" value={totalReliability} />
              <SlateMetric
                label="Projection Edge"
                value={signedDifferenceText(totalProjection, totalLine)}
              />
            </SlateMarketRow>
          ) : null}
          ...
          {row["NRFI Grade"] ? (
            <SlateMarketRow
              eyebrow="First Inning"
              title={firstInningType || "PASS"}
              subtitle="Elite qualifier threshold"
              grade={firstInningType}
              isGreen={firstInningGreen}
            >
              <SlateMetric
                label="Probability"
                value={percentText(firstInningProbability)}
                accent={firstInningGreen}
              />
              <SlateMetric
                label="EZPZ Score"
                value={scoreText(firstInningScore)}
                accent={firstInningGreen}
              />
              <SlateMetric label="Odds" value={formatOdds(firstInningOdds)} />
            </SlateMarketRow>
          ) : null}
        </div>

        <div className="slatePitcherSection">
          <div className="slateSectionLabel">
            <span>Pitcher strikeout projections</span>
            <small>Starter, opener, and bulk roles shown separately</small>
          </div>
          <div className="slateMarketStack pitcherRows">
            <SlatePitcherRow
        ...
        <>
          <div className="sectionHead">
            <div>
              <h2>Full Slate</h2>
              <p>
                Scan every matchup in a compact scoreboard view, then expand any game
                for the complete moneyline, total, first-inning, pitcher, and public-betting breakdown.
              </p>
            </div>
            <span className="countPill">{data.slateToday.length} games</span>
          </div>

          <div className="slateLegend">
            <span><i className="legendDot green" /> Green indicates a qualified play</span>
            <span>Tap any matchup for full analytics</span>
          </div>
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .legendDot {
          ...
          height: 7px;
          border-radius: 50%;
          background: rgba(108, 139, 181, 0.56);
          box-shadow: 0 0 0 4px rgba(108, 139, 181, 0.08);
        }

        .legendDot.green {
          background: #2bd875;
          box-shadow: 0 0 0 4px rgba(43, 216, 117, 0.09), 0 0 14px rgba(43, 216, 117, 0.36);
        }
        ...
        .slateAccordion {
          ...
          overflow: hidden;
          min-width: 0;
          border: 1px solid rgba(94, 130, 179, 0.17);
          border-radius: 20px;
          background:
            radial-gradient(circle at 92% -15%, rgba(60, 145, 255, 0.11), transparent 35%),
            linear-gradient(145deg, rgba(8, 16, 30, 0.97), rgba(4, 9, 18, 0.98));
          box-shadow:
        ...
        .slateAccordion::before {
          content: "";
          position: absolute;
          z-index: 0;
          inset: 0 auto 0 0;
          width: 3px;
          background: linear-gradient(180deg, rgba(66, 157, 255, 0.85), rgba(44, 103, 214, 0.24));
          opacity: 0.38;
        }

        .slateAccordion.green {
          border-color: rgba(43, 216, 117, 0.3);
          background:
            radial-gradient(circle at 94% -14%, rgba(43, 216, 117, 0.105), transparent 36%),
            radial-gradient(circle at 8% 110%, rgba(28, 111, 224, 0.08), transparent 38%),
            linear-gradient(145deg, rgba(7, 22, 30, 0.98), rgba(4, 10, 19, 0.98));
          box-shadow:
            inset 0 1px 0 rgba(255, 255, 255, 0.03),
            0 16px 40px rgba(0, 0, 0, 0.28),
            0 0 0 1px rgba(43, 216, 117, 0.025);
        }

        .slateAccordion.green::before {
          background: linear-gradient(180deg, #2bd875, rgba(43, 216, 117, 0.16));
          opacity: 1;
          box-shadow: 0 0 18px rgba(43, 216, 117, 0.32);
        }
        ...
        .slateAccordion[open] {
          ...
          box-shadow:
            ...
            0 24px 58px rgba(0, 0, 0, 0.37),
            0 0 38px rgba(39, 120, 229, 0.07);
        }

        .slateAccordion.green[open] {
          border-color: rgba(43, 216, 117, 0.42);
          box-shadow:
            inset 0 1px 0 rgba(255, 255, 255, 0.035),
            0 24px 58px rgba(0, 0, 0, 0.37),
            0 0 40px rgba(43, 216, 117, 0.075);
        }
        ...
        .slateSummary::-webkit-details-marker {
          display: none;
        }

        .slateSummary:focus-visible {
          outline: 2px solid rgba(75, 166, 255, 0.74);
          outline-offset: -3px;
          border-radius: 18px;
        }
        ...
        .slateLogoStack::before {
          content: "";
          position: absolute;
          inset: 3px 4px 2px 2px;
          border: 1px solid rgba(102, 155, 218, 0.13);
          border-radius: 14px;
        ...
        .slateLogoStack img:first-child {
          left: 0;
          top: 0;
        }

        .slateLogoStack img:last-child {
          right: 0;
          bottom: 0;
        }
        ...
        .slateQualifiedPill {
          border: 1px solid rgba(102, 138, 183, 0.16);
          border-radius: 999px;
          padding: 6px 9px;
          color: rgba(179, 197, 221, 0.75);
        ...
        .slateQualifiedPill.green {
          color: #c4f8d7;
          border-color: rgba(43, 216, 117, 0.3);
          background: rgba(22, 112, 63, 0.18);
          box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.025), 0 0 18px rgba(43, 216, 117, 0.055);
        }
        ...
        .slateChevron {
          ...
          width: 27px;
          height: 27px;
          border: 1px solid rgba(111, 148, 195, 0.16);
          border-radius: 9px;
        ...
        .slateChevron::after {
          ...
          height: 2px;
          border-radius: 999px;
          background: rgba(189, 211, 237, 0.82);
        ...
        .slateAccordion[open] .slateChevron {
          border-color: rgba(66, 164, 255, 0.3);
          background: rgba(23, 72, 129, 0.28);
        ...
        .slateQuickTile {
          min-width: 0;
          border: 1px solid rgba(101, 136, 180, 0.12);
          border-radius: 12px;
          padding: 8px 9px;
          background:
            linear-gradient(145deg, rgba(13, 24, 41, 0.77), rgba(6, 13, 24, 0.7));
          box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.018);
        }

        .slateQuickTile.green {
          border-color: rgba(43, 216, 117, 0.19);
          background:
            radial-gradient(circle at 100% 0%, rgba(43, 216, 117, 0.09), transparent 45%),
            linear-gradient(145deg, rgba(10, 35, 39, 0.68), rgba(6, 15, 25, 0.76));
        }
        ...
        .slateQuickTop i {
          ...
          border-radius: 50%;
          background: #2bd875;
          box-shadow: 0 0 9px rgba(43, 216, 117, 0.58);
        }

        .slateQuickTile > strong {
          display: block;
          overflow: hidden;
          margin-top: 5px;
          color: #f4f9ff;
        ...
        .slateQuickTile.green > strong {
          color: #d9fbe6;
        }

        .slateQuickTile > small {
          display: block;
          overflow: hidden;
          margin-top: 4px;
          color: rgba(135, 160, 191, 0.66);
          font-size: 8px;
          line-height: 1.15;
          font-weight: 750;
          text-overflow: ellipsis;
        ...
        .slateExpanded {
          ...
          display: grid;
          gap: 13px;
          border-top: 1px solid rgba(89, 137, 194, 0.13);
          padding: 14px 15px 16px 17px;
          background:
            linear-gradient(180deg, rgba(8, 17, 31, 0.42), rgba(4, 9, 18, 0.16));
          animation: slateReveal 0.24s ease both;
        }

        @keyframes slateReveal {
          from {
            opacity: 0;
        ...
        .slateExpandedHead > div:first-child {
          display: grid;
          gap: 3px;
        }

        .slateExpandedHead > div:first-child span {
          color: #7ec4ff;
          font-size: 8px;
          font-weight: 950;
          letter-spacing: 0.1em;
        ...
        .slateExpandedHead > div:first-child strong {
        ...
        .slateExpandedBadges span {
          border: 1px solid rgba(94, 135, 185, 0.13);
          border-radius: 999px;
          padding: 4px 7px;
        ...
        .slateMarketRow {
          display: grid;
          grid-template-columns: minmax(170px, 0.82fr) minmax(0, 2.18fr);
          gap: 12px;
          align-items: center;
          min-width: 0;
          border: 1px solid rgba(97, 132, 177, 0.12);
          border-radius: 14px;
          padding: 10px;
          background:
            linear-gradient(145deg, rgba(10, 20, 35, 0.82), rgba(5, 11, 21, 0.78));
          box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.018);
        }

        .slateMarketRow.green {
          border-color: rgba(43, 216, 117, 0.18);
          background:
            radial-gradient(circle at 100% 0%, rgba(43, 216, 117, 0.08), transparent 35%),
            linear-gradient(145deg, rgba(8, 29, 35, 0.78), rgba(5, 12, 22, 0.82));
        }
        ...
        .slateGradeChip {
          flex: 0 0 auto;
          max-width: 112px;
          border: 1px solid rgba(119, 151, 190, 0.15);
          border-radius: 999px;
          padding: 5px 7px;
          color: rgba(192, 209, 230, 0.76);
          background: rgba(25, 38, 57, 0.66);
          font-size: 8px;
          line-height: 1.05;
          font-weight: 950;
          letter-spacing: 0.055em;
        ...
        .slateGradeChip.green {
          color: #c7f9d9;
          border-color: rgba(43, 216, 117, 0.27);
          background: rgba(19, 111, 61, 0.19);
        }
        ...
        .slateMetric {
          min-width: 0;
          border-left: 1px solid rgba(103, 139, 184, 0.11);
          padding: 2px 7px;
        }
        ...
        .slatePopularityLine {
          ...
          align-items: center;
          gap: 5px 10px;
          border-top: 1px solid rgba(96, 133, 180, 0.1);
          padding: 8px 3px 0;
          color: rgba(142, 167, 198, 0.7);
          font-size: 8px;
          line-height: 1.25;
          font-weight: 800;
        }
        ...
        .slateSplitLine strong,
        .slatePopularityLine strong {
          border-radius: 999px;
          padding: 3px 6px;
          color: #d7edff;
          background: rgba(31, 87, 151, 0.22);
          font-size: 7px;
          letter-spacing: 0.045em;
          text-transform: uppercase;
        }

        .slateSplitLine strong.negative {
          color: #ffc4c4;
          background: rgba(137, 31, 39, 0.2);
        }

        .slateSplitLine strong.positive {
          color: #baf3cf;
          background: rgba(21, 112, 63, 0.18);
        }

        .slatePitcherSection {
          display: grid;
          gap: 8px;
          border-top: 1px solid rgba(92, 136, 188, 0.11);
          padding-top: 12px;
        }
        ...
        .slateSectionLabel span {
          color: rgba(224, 238, 252, 0.9);
          font-size: 11px;
          font-weight: 900;
        }

        .slateSectionLabel small {
          color: rgba(133, 158, 190, 0.64);
          font-size: 8px;
          font-weight: 750;
        }

        .pitcherRows .slateMarketRow {
          grid-template-columns: minmax(190px, 0.88fr) minmax(0, 2.12fr);
        }

        .slatePitcherIdentity {
          display: grid;
          place-items: center;
          min-width: 48px;
          border-left: 0;
        ...
        .slatePitcherIdentity .headshotFrame {
          width: 43px;
          height: 43px;
          border-radius: 13px;
        }

        .slatePitcherIdentity .headshot,
        .slatePitcherIdentity .headshotFallback {
          border-radius: 12px;
        }

        .slatePitcherIdentity .headshotFallback::after {
          border-radius: 11px;
        }

        .slatePitcherIdentity .headshotFallback svg {
          width: 24px;
          height: 24px;
        }

        .slatePitcherIdentity .headshotTeamBadge {
          right: -4px;
          bottom: -4px;
          width: 18px;
          height: 18px;
        ...
        @media (hover: hover) and (pointer: fine) {
          .slateAccordion:hover {
            transform: translateY(-1px);
            border-color: rgba(76, 157, 250, 0.28);
            box-shadow:
              inset 0 1px 0 rgba(255, 255, 255, 0.03),
              0 19px 44px rgba(0, 0, 0, 0.32);
          }

          .slateAccordion.green:hover {
            border-color: rgba(43, 216, 117, 0.39);
          }
        }
        ...
        @media (max-width: 760px) {
          ...
          .slateMetric {
            border-left: 0;
            border-top: 1px solid rgba(103, 139, 184, 0.1);
            padding: 7px 3px 1px;
          }
        ...
        @media (max-width: 420px) {
          ...
          .slateMatchupMeta span:last-child {
            display: none;
          }
