...
.formPill.alt { color:#fef3c7; border-color:rgba(245,158,11,.52); background:rgba(245,158,11,.14); }
...
.valuePill.smash { color:#dcfce7; border-color:rgba(34,197,94,.50); background:rgba(34,197,94,.15); }
.valuePill.value { color:#dbeafe; border-color:rgba(96,165,250,.42); background:rgba(37,99,235,.13); }
.valuePill.thin { color:#fef3c7; border-color:rgba(245,158,11,.40); background:rgba(245,158,11,.13); }
.whyBox {
  margin-top:12px;
...
.whyList { margin:0; padding:0 14px 12px 28px; color:#cbd5e1; font-size:.82rem; font-weight:750; line-height:1.35; }
...
function getPropEdgePct(play: Play) {
  const extra = play as unknown as Record<string, unknown>;
  const candidates = [extra.edgePct, extra.edge, extra.modelEdge, extra["Edge"], extra["Edge %"], extra["Model Edge"]];
  for (const candidate of candidates) {
    const value = parsePct(candidate);
    if (value) return Math.abs(value);
  }
  return 0;
}

function hasAltValue(play: Play) {
  if (!isKType(play.playType)) return false;

  const type = normalizeType(play.playType);
  const isOver = type.includes("OVER");
  const isUnder = type.includes("UNDER");
  if (!isOver && !isUnder) return false;

  const line = toNum(extractLine(play.play, play.oddsLine));
  ...
  const volatility = String(play.volatility || "").trim().toLowerCase();
  const propEdgePct = getPropEdgePct(play);

  if (!line || !projectedKs || !sixInningKs) return false;
  if (volatility === "high") return false;

  // If a clean prop edge percentage is available in the feed, require it to be meaningful.
  // If it is not available, projection gap + 6-inning support + volatility carry the ALT signal.
  if (propEdgePct && propEdgePct < 8) return false;

  if (isOver) return projectedKs >= line + 1 && sixInningKs >= line + 0.5;
  if (isUnder) return projectedKs <= line - 1 && sixInningKs <= line - 0.5;
  return false;
}
...
function BestPlayCard({ play, index, recentSummary }: { play: Play; index: number; recentSummary: Summary | null }) {
  ...
  const altValue = hasAltValue(play);
  ...
  return (
    <div className="card green betSlip">
      ...
      <div className="formRow"><FormTag summary={recentSummary} />{altValue ? <div className="formPill alt">⭐ ALT</div> : null}</div>
  ...
  const content = useMemo(() => {
    ...
    if (active === "Today’s Best Plays") return (
      <>
        ...
        {curatedBestPlays.length ? <div className="cards">{curatedBestPlays.map((p, i) => <BestPlayCard key={`${p.game}-${p.play}-${i}`} play={p} index={i} recentSummary={recentByType.get(normalizeType(p.playType)) || null} />)}</div> : <div className="empty">No qualified Best Plays yet for {data.today}.</div>}
      </>
    );
      ...
      <main className="shell">
        ...
        {data ? (
          <section className="tileGrid">
            <Tile label="Green Plays - Last 7 Days" value={data.tiles.last7Days.record} meta={`${data.tiles.last7Days.winPct}% • ${data.tiles.last7Days.unitsWon}u • ROI ${data.tiles.last7Days.roiPct}%`} green />
            <Tile label="Green Plays - Running Total" value={data.tiles.overallGreen.record} meta={`${data.tiles.overallGreen.winPct}% • ${data.tiles.overallGreen.unitsWon}u • ROI ${data.tiles.overallGreen.roiPct}%`} green />
            <Tile label="Today’s Best Plays" value={String(visibleBestPlaysToday)} meta="Qualified" green={visibleBestPlaysToday > 0} />
          </section>
        ) : null}
