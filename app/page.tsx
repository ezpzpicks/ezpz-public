...
function favoriteRankValue(play: Play) {
  const n = Number(String(play.favoriteRank ?? "").replace(/[^\d.-]/g, ""));
...
function BestPlayCard({ play, index, recentSummary, slateRows }: { play: Play; index: number; recentSummary: Summary | null; slateRows: SheetRow[] }) {
  ...
  return (
    <div className={`card green fade-in best ${topPlay ? "top" : ""}`}>
      ...
      {isTrueValue(play.favoritePick) ? (
        <div className="handpickedMeta">
          <span>⭐ HANDPICKED{play.favoriteTag ? ` • ${String(play.favoriteTag).toUpperCase()}` : ""}</span>
          {play.favoriteNotes ? <p>{play.favoriteNotes}</p> : null}
        </div>
      ) : null}
...
export default function Home() {
  ...
  const handpickedPlays = useMemo(() => {
    ...
    return bestPlays
      .filter((play) => isTrueValue(play.favoritePick))
      .sort((a, b) => {
        const rankDiff = favoriteRankValue(a) - favoriteRankValue(b);
        if (rankDiff !== 0) return rankDiff;

        const aSummary = recentByType.get(normalizeType(a.playType)) || null;
        const bSummary = recentByType.get(normalizeType(b.playType)) || null;
        return getRankScore(b, bSummary, data.slateToday) - getRankScore(a, aSummary, data.slateToday);
      });
  }, [bestPlays, data]);
    ...
    if (active === "Today’s Best Plays") {
      ...
      return (
        <>
          ...
          {handpickedPlays.length || data.tiles.handpickedLast7 || data.tiles.handpickedOverall ? (
            <>
              <div className="sectionHead handpickedHead">
                <div>
                  <h2>EZPZ Handpicked Plays</h2>
                  <p>Your final conviction card, sorted by Favorite Rank.</p>
                </div>
              </div>

              {data.tiles.handpickedLast7 || data.tiles.handpickedOverall ? (
                <div className="qualifiedGrid handpickedGrid">
                  {data.tiles.handpickedLast7 ? (
                    <Tile
                      label="Handpicked - Last 7 Days"
                      value={data.tiles.handpickedLast7.record}
                      meta={`${data.tiles.handpickedLast7.winPct}% • ${data.tiles.handpickedLast7.unitsWon}u • ROI ${data.tiles.handpickedLast7.roiPct}%`}
                      green
                    />
                  ) : null}
                  {data.tiles.handpickedOverall ? (
                    <Tile
                      ...
                      value={data.tiles.handpickedOverall.record}
                      meta={`${data.tiles.handpickedOverall.winPct}% • ${data.tiles.handpickedOverall.unitsWon}u • ROI ${data.tiles.handpickedOverall.roiPct}%`}
                      green
                    />
                  ) : null}
                </div>
              ...
              {handpickedPlays.length ? (
                <div className="cards handpickedCards">
                  {handpickedPlays.map((play, index) => (
                    <BestPlayCard
                      key={`handpicked-${play.game}-${play.play}-${index}`}
                      play={play}
                      index={favoriteRankValue(play) === 999 ? index : favoriteRankValue(play) - 1}
                ...
                <div className="empty handpickedEmpty">No pending handpicked plays right now. Your Handpicked records stay visible here.</div>
              )}
            </>
          ) : null}
          ...
          <div className="sectionHead">
            <div>
              <h2>Full Best Plays List</h2>
              <p>All model-qualified plays after score, odds, and trend filters.</p>
            </div>
          </div>
  ...
  }, [active, bestPlays, handpickedPlays, data, error]);
