...
type TrendPlay = {
  ...
  gapPct: number;
  score: number;
  tier: "Strong Trend Play" | "Qualified Trend Play" | "Trend Lean";
...
function liveSplitInfoForPlay(
  ...
  return {
    ...
    matchConfidence: "Live game and selected-side match",
...
function computedPublicWarning(betsPct?: number, moneyPct?: number) {
  ...
  if (betsPct >= 90 && moneyPct >= 90) {
    return { label: "Extreme Public Consensus", tone: "negative" as const, negative: true };
  }
  if (betsPct >= 80 && moneyPct >= 80) {
    return { label: "Heavy Public Consensus", tone: "caution" as const, negative: true };
  }
  if (betsPct >= 70 && betsPct - moneyPct >= 10) {
    return { label: "Public Tickets, Weak Money", tone: "negative" as const, negative: true };
  }
  if (betsPct <= 35 && moneyPct >= 55 && gapPct >= 20) {
    return { label: "Strong Contrarian Money Support", tone: "positive" as const, negative: false };
  }
  if (betsPct <= 45 && moneyPct >= 50 && gapPct >= 12) {
    return { label: "Contrarian Money Support", tone: "positive" as const, negative: false };
  }
  if (moneyPct >= 60 && gapPct >= 10) {
    return { label: "Market Money Confirmation", tone: "positive" as const, negative: false };
  }
  return { label: "Balanced Market", tone: "neutral" as const, negative: false };
}
...
function TrendPlayCard({
  ...
  const pickLabel = play.market === "Total"
  ...
  return (
    <article className={`card fade-in best trendCard ${play.score >= 80 ? "top" : ""}`}>
      {badgeText ? (
      ...
      <div className="cardTop">
        ...
        <div className="scorePill trendScorePill" aria-label={`Trend Score ${play.score}`}>
          <span className="scorePillLabel">TREND</span>
      ...
      <div className="projectionBlock trendProjectionBlock">
        <div className="projection">{pickLabel}</div>
        <div className="grade">{play.tier}</div>
      </div>

      <div className="divider" />

      <div className="bubbleGrid">
        <MiniBubble label="Public Bets" value={publicPctText(play.betsPct)} />
        <MiniBubble label="Public Money" value={publicPctText(play.moneyPct)} />
        <MiniBubble
          label="Money − Bets"
          value={`${play.gapPct >= 0 ? "+" : ""}${play.gapPct.toFixed(1)}%`}
        />
        <MiniBubble label="Odds" value={formatOdds(play.odds)} />
      </div>
      ...
      <div className="trendSignalList">
        {play.signals.map((signal) => {
          ...
          const weightText = [
            weights.exact ? `Exact ${Math.round(weights.exact * 100)}%` : "",
            weights.market ? `Market ${Math.round(weights.market * 100)}%` : "",
            weights.overall ? `Overall ${Math.round(weights.overall * 100)}%` : "",
          ].filter(Boolean).join(" • ");
          return (
            <section className="trendSignalPanel" key={`${signal.signalType}-${signal.signalKey}`}>
              ...
              <div className="trendWeightLine">
                Exact category: {signal.exactSample} bets
      ...
      <div className="modelMeta trendMeta">
        <span>{play.market} • {play.sideGroup}</span>
...
> = [
  ...
  { signalType: "Public Split", signal: "Public Tickets, Weak Money", tone: "negative" },
  { signalType: "Public Split", signal: "Strong Contrarian Money Support", tone: "positive" },
  { signalType: "Public Split", signal: "Contrarian Money Support", tone: "positive" },
  { signalType: "Public Split", signal: "Market Money Confirmation", tone: "positive" },
  { signalType: "Public Split", signal: "Balanced Market", tone: "neutral" },
  { signalType: "Line Movement", signal: "Reverse Line Movement", tone: "negative" },
  { signalType: "Line Movement", signal: "Line Movement Confirmation", tone: "positive" },
];

function signalSampleLabel(totalBets: number) {
  if (totalBets < 10) return "Insufficient";
...
function summarizeDraftKingsSignals(rows: DraftKingsSignalResult[]) {
  ...
  for (const row of rows) {
    const key = `${row.signalType}|${row.signal}`;
    if (!map.has(key)) {
      map.set(key, {
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    if (active === "Today’s Best Plays") {
      ...
      return (
        <div className="playsBoard">
          <section className="playsBoardColumn">
            <div className="sectionHead">
              <div>
          ...
          <section className="playsBoardColumn trendBoardColumn">
            <div className="sectionHead">
              <div>
                <h2>Trend Plays</h2>
                <p>
                  DraftKings moneyline and total signals scored from exact market-type history,
                  with all-time, last-30-day, and last-7-day performance.
                </p>
              </div>
              <span className="countPill">{trendPlays.length} plays</span>
            </div>

            {trendPlays.length ? (
              <div className="cards boardCards">
                {trendPlays.map((trendPlay, index) => (
                  <TrendPlayCard
                    key={`${trendPlay.game}-${trendPlay.market}-${trendPlay.selection}-${index}`}
                    play={trendPlay}
                    index={index}
                    matchStatus={bestMatchForTrendPlay(
                      trendPlay,
                      orderedPlays,
                      data.slateToday,
                    )}
              ...
              <div className="empty">
                No Trend Plays currently meet the 60-point qualification threshold.
              </div>
            )}
          </section>
        </div>
