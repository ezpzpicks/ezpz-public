...
const TREND_WINNER_CALIBRATION_POINTS: TrendScorePoint[] = [
  [0, 0],
  [75, 75],
  [78, 78],
  [80, 82],
...
function trendWinnerCalibratedScore(score: number) {
  // Match route.ts: once the tougher ROI + win-rate composite and capped
  // head-to-head confirmation already produce a low-80s winner, reopen the
  // upper range. This is deliberately nonlinear: raw 84 becomes about 92,
  // while ordinary records never reach this calibration band.
  return clampScore(trendScaledScore(score, TREND_WINNER_CALIBRATION_POINTS));
}
...
function scoreTrendMarketPlays(plays: TrendPlay[]): RankedTrendPlay[] {
  ...
  return baseRows.map(({ play, metrics }) => {
    ...
    const rawWinnerScore = clampScore(metrics.score + comparisonBonus);
    const winnerScore = trendWinnerCalibratedScore(rawWinnerScore);
