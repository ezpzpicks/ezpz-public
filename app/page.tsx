...
function rankedTrendLabel(score: number) {
...
function groupRankedTrendPlays(trendPlays: TrendPlay[]): TrendGameGroup[] {
  const grouped = new Map<string, TrendGameGroup>();
  ...
  return [...grouped.values()]
    .map((group) => {
      const plays = [...group.plays].sort((a, b) => {
        if (b.score !== a.score) return b.score - a.score;
        const sampleDifference = trendExactSample(b) - trendExactSample(a);
    ...
    .sort((a, b) => {
      ...
      if (b.secondScore !== a.secondScore) return b.secondScore - a.secondScore;
      if (b.maxExactSample !== a.maxExactSample) return b.maxExactSample - a.maxExactSample;
    ...
    >
      <summary className="trendSelectionSummary">
        ...
        <span className="trendSelectionMarket">
          <small>{rankedTrendLabel(play.score)}</small>
...
function TrendGameCard({
  ...
  const gameTime = firstValue(slateRow, [
    "Game Time",
    "Start Time",
    "Scheduled Time",
    "First Pitch",
  ...
  return (
    <article className={`card trendGameCard ${group.topScore >= 70 ? "top" : ""}`}>
      ...
      <div className="trendGameLeader">
        <div>
          ...
          <small>{rankedTrendLabel(group.topScore)}</small>
        </div>
      ...
      <div className="trendSelectionStack">
        {group.plays.map((play, index) => (
          <TrendSelectionRow
            ...
            matchStatus={bestMatchForTrendPlay(play, bestPlays, slateRows)}
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    const rankedTrendGames = groupRankedTrendPlays(trendPlays);
        ...
        <section>
          <div className="sectionHead">
            <div>
              ...
              <p>
                One ranked tile per game. Each tile keeps both moneylines, the Over, and the Under
                together, with every market ranked by its historical DraftKings signal score.
              </p>
            </div>
