...
function favoriteKeyFromPlay(play: Play, today: string) {
  const dateKey = favoriteDateKey(today);
  const rawType = normalizeType(play.playType);
  // Qualified green YRFI plays still arrive from the API under the legacy YRFI
  // label, but their tracker/favorite row is stored as ELITE YRFI.
  const type =
    rawType === "YRFI" && play.isGreen === true ? "ELITE YRFI" : rawType;
...
function favoriteRowForPlay(
  play: Play,
  favoriteRows: Map<string, SheetRow>,
  today: string,
) {
  const exact = favoriteRows.get(favoriteKeyFromPlay(play, today));
  if (exact) return exact;

  // Compatibility fallback for current-day Elite YRFI favorites. Depending on
  // which workflow saved the row, the tracker may contain either ELITE YRFI or
  // the older YRFI label. This alias is used only for the active handpicked
  // badge lookup and does not combine the two record categories.
  const rawType = normalizeType(play.playType);
  if (rawType === "YRFI" || rawType === "ELITE YRFI") {
    const dateKey = favoriteDateKey(today);
    const gameKey = favoriteKeyText(play.game || play.play);
    const canonicalType =
      rawType === "YRFI" && play.isGreen === true ? "ELITE YRFI" : rawType;
    const alternateType = canonicalType === "ELITE YRFI" ? "YRFI" : "ELITE YRFI";
    return favoriteRows.get(`FI|${dateKey}|${alternateType}|${gameKey}`);
  }
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    if (active === "Today’s Best Plays") {
      const isActiveHandpicked = (play: Play) =>
        isFavoriteValue(play.favoritePick) ||
        Boolean(favoriteRowForPlay(play, favoriteRowMap, data.today));
      const handpickedPlays = bestPlays.filter(isActiveHandpicked);
      const regularBestPlays = bestPlays.filter(
        (play) => !isActiveHandpicked(play),
      );
      const orderedPlays = [
          ...
          {orderedPlays.length ? (
            <div className="cards">
              {orderedPlays.map((play, index) => {
                const favoriteRow = favoriteRowForPlay(
                  play,
                  favoriteRowMap,
                  data.today,
                );
                const isHandpicked =
                  Boolean(favoriteRow) || isFavoriteValue(play.favoritePick);
