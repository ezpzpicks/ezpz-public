...
function isFavoriteValue(value: unknown) {
  // Strict on purpose: only the actual Google Sheets TRUE value should count for today's badge/display.
  return String(value ?? "").trim().toLowerCase() === "true";
}

function isHandpickedRecordRow(row: SheetRow) {
  // Historical records can use a permanent marker, so old handpicked results do not disappear
  // if today's display TRUE is removed later. Badge logic still uses Favorite Pick only.
  return (
...
function parseDateOnly(value: unknown) {
  ...
  if (slash) {
    ...
    return new Date(year, Number(m) - 1, Number(d), 12, 0, 0);
  }
...
function calculateFavoriteRecord(rows: SheetRow[] | undefined, mode: "all" | "last7" | "today" = "all", today = "") {
  ...
  rows.forEach((row) => {
    ...
    if (mode === "today" && today && !sameDateText(rowDateText, today)) return;
...
export default function Home() {
  ...
  const handpickedLast7 = useMemo(() => {
    ...
    return calculated.totalBets > 0 ? calculated : data?.tiles.handpickedLast7 || calculated;
  }, [data]);
  const handpickedOverall = useMemo(() => {
    const calculated = calculateFavoriteRecord(data?.betTrackerRows, "all", data?.today);
    return calculated.totalBets > 0 ? calculated : data?.tiles.handpickedOverall || calculated;
  }, [data]);
