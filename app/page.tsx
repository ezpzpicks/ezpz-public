...
function mergeRecordSummaries(primary: Summary[] | undefined, fallback: Summary[]) {
  ...
  // Start with the API summary, then let the record rebuilt directly from the raw
  // tracker win whenever it exists. The tracker calculation groups by the exact
  // normalized bet type, so ELITE YRFI stays separate from regular YRFI instead of
  // being replaced by a broader API bucket that may already include elite plays.
