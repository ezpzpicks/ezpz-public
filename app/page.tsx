...
function qualifiesForPublicGreen(play: Play, greenTypes?: Set<string>) {
...
function DailyRecap({ data, displayedCount, qualifiedCount }: { data: ApiData; displayedCount: number; qualifiedCount: number }) {
  return (
    <div className="recapGrid">
      <div className="recapCard"><div className="recapLabel">Curated Card</div><div className="recapValue">Top {displayedCount}</div><div className="recapMeta">Best Plays shown publicly today</div></div>
      <div className="recapCard"><div className="recapLabel">Qualified Pool</div><div className="recapValue">{qualifiedCount}</div><div className="recapMeta">Green plays after public filters</div></div>
      <div className="recapCard"><div className="recapLabel">Pending Tracker</div><div className="recapValue">{data.tiles.pendingGreen}</div><div className="recapMeta">Pending qualified plays from tracker</div></div>
    </div>
  );
}
...
function SlateCard({ row, greenSet }: { row: SheetRow; greenSet: Set<string> }) {
  ...
  const mlGreen = greenSet.has(mlType) && slateMoneylinePassesJuice(row);
...
export default function Home() {
  ...
  const content = useMemo(() => {
    ...
    const bestPlaysFiltered = data.bestPlays.filter((p) => qualifiesForPublicGreen(p, greenSet));
    const curatedBestPlays = [...bestPlaysFiltered].sort((a, b) => parseScore(b.score) - parseScore(a.score)).slice(0, BEST_PLAY_LIMIT);

    if (active === "Today’s Best Plays") return (
      <>
        <div className="sectionHead"><div><h2>Today’s Best Plays</h2><p>Curated top plays only. Model %, Vegas implied %, edge, value label, and the reasoning dropdown are shown when the data is available. Last updated: {data.lastUpdated}</p></div></div>
        <DailyRecap data={data} displayedCount={curatedBestPlays.length} qualifiedCount={bestPlaysFiltered.length} />
        {curatedBestPlays.length ? <div className="cards">{curatedBestPlays.map((p, i) => <BestPlayCard key={`${p.game}-${p.play}-${i}`} play={p} index={i} recentSummary={recentByType.get(normalizeType(p.playType)) || null} />)}</div> : <div className="empty">No qualified Best Plays saved yet for {data.today}.</div>}
      </>
    );

    if (active === "Full Slate") return (
      <>
        <div className="sectionHead"><div><h2>Full Slate</h2><p>Every saved game for {data.today}. Only qualified winning bet types use green bubbles; recent form appears as Hot / Neutral / Cold.</p></div></div>
    ...
    return (
      <>
        <div className="sectionHead"><div><h2>All Qualified Plays</h2><p>Broad qualified plays record using the same public filter rules, kept separate from the curated Best Plays record shown at the top.</p></div></div>
        <div className="qualifiedGrid">
          <Tile label="Qualified Plays - Last 7 Days" value={data.tiles.last7Days.record} meta={`${data.tiles.last7Days.winPct}% • ${data.tiles.last7Days.unitsWon}u • ROI ${data.tiles.last7Days.roiPct}%`} green />
          <Tile label="Qualified Plays - Running Total" value={data.tiles.overallGreen.record} meta={`${data.tiles.overallGreen.winPct}% • ${data.tiles.overallGreen.unitsWon}u • ROI ${data.tiles.overallGreen.roiPct}%`} green />
        </div>
  ...
  const visibleBestPlaysToday = data
    ? Math.min(BEST_PLAY_LIMIT, data.bestPlays.filter((p) => qualifiesForPublicGreen(p, new Set<string>(data.recordSummary.filter((r) => r.wins > r.losses).map((r) => normalizeType(r.betType))))).length)
    : 0;
      ...
      <main className="shell">
        ...
        {data ? (
          <section className="tileGrid">
            <Tile label="Best Plays - Last 7 Days" value={data.tiles.last7Days.record} meta={`${data.tiles.last7Days.winPct}% • ${data.tiles.last7Days.unitsWon}u • ROI ${data.tiles.last7Days.roiPct}%`} green />
            <Tile label="Best Plays - Running Total" value={data.tiles.overallGreen.record} meta={`${data.tiles.overallGreen.winPct}% • ${data.tiles.overallGreen.unitsWon}u • ROI ${data.tiles.overallGreen.roiPct}%`} green />
            <Tile label="Today’s Best Plays" value={String(visibleBestPlaysToday)} meta={`Top ${BEST_PLAY_LIMIT} curated public plays max`} green={visibleBestPlaysToday > 0} />
          </section>
        ) : null}
