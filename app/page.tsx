...
function passesMinimumScoreFilter(play: Play, recentSummary: Summary | null, slateRows: SheetRow[] = []) {
  return getRankScore(play, recentSummary, slateRows) >= BEST_PLAY_MIN_RANK_SCORE;
}
...
function isFavoriteValue(value: unknown) {
  ...
  return ["true", "yes", "y", "1", "x", "favorite", "handpicked"].includes(text);
}
...
function trackerRowIsToday(row: SheetRow, today = "") {
  const rowDateText = String(row.Date || row.date || row["Bet Date"] || "").trim();
  return !today || rowDateText === today;
}

function trackerRowMatchesPlay(row: SheetRow, play: Play, today = "") {
  if (!trackerRowIsToday(row, today)) return false;
  if (!isFavoriteValue(row["Favorite Pick"])) return false;

  const rowBetType = normalizeType(row["Bet Type"] || "");
  const rowMarket = normalizeMatchText(row.Market || "");
  const rowSelection = normalizeMatchText(row.Selection || "");
  const rowSelectionPitcher = normalizeMatchText(cleanPitcherName(row.Selection || ""));
  const playType = normalizeType(play.playType);
  const playText = normalizeMatchText(play.play || "");
  const playGame = normalizeMatchText(play.game || "");
  const playTeam = normalizeMatchText(play.playerTeam || play.play || "");
  const playPitcher = normalizeMatchText(cleanPitcherName(play.play || ""));

  if (isMoneylineType(play.playType)) {
    return (
      rowMarket.includes("moneyline") &&
      rowSelection.length > 0 &&
      (playText.includes(rowSelection) || playTeam.includes(rowSelection) || rowSelection.includes(playTeam))
    );
  }

  if (isKType(play.playType)) {
    const rowLooksLikeK = rowMarket.includes("pitcher strikeouts") || isKType(rowBetType);
    if (!rowLooksLikeK || !playPitcher) return false;
    return rowSelectionPitcher.includes(playPitcher) || playPitcher.includes(rowSelectionPitcher) || rowSelection.includes(playPitcher);
  }

  if (isNRFIType(play.playType)) {
    const rowLooksLikeNRFI = rowMarket.includes("nrfi") || rowMarket.includes("yrfi") || isNRFIType(rowBetType);
    return rowLooksLikeNRFI && (rowSelection.includes(playGame) || playGame.includes(rowSelection) || rowBetType === playType);
  }

  return false;
}

function isHandpickedPlay(play: Play, rows: SheetRow[] | undefined, today = "") {
  if (isFavoriteValue(play.favoritePick)) return true;
  return Boolean(rows?.some((row) => trackerRowMatchesPlay(row, play, today)));
}

function countTodayHandpickedRows(rows: SheetRow[] | undefined, today = "") {
  if (!rows?.length) return 0;
  return rows.filter((row) => isFavoriteValue(row["Favorite Pick"]) && trackerRowIsToday(row, today)).length;
}

function countHandpickedBestPlays(plays: Play[] | undefined) {
  if (!plays?.length) return 0;
  return plays.filter((play) => isFavoriteValue(play.favoritePick)).length;
}

function getHandpickedRecordFromApiOrRows(data: ApiData | null, mode: "last7" | "all") {
  const apiTotals = mode === "last7" ? data?.tiles.handpickedLast7 : data?.tiles.handpickedOverall;
  if (apiTotals) return apiTotals;
  return calculateFavoriteRecord(data?.betTrackerRows, mode, data?.today);
}
...
export default function Home() {
  ...
  const todayHandpickedCount = useMemo(() => {
    const rowCount = countTodayHandpickedRows(data?.betTrackerRows, data?.today);
    return rowCount || countHandpickedBestPlays(data?.bestPlays);
  }, [data]);

  const handpickedLast7 = useMemo(() => getHandpickedRecordFromApiOrRows(data, "last7"), [data]);
  const handpickedOverall = useMemo(() => getHandpickedRecordFromApiOrRows(data, "all"), [data]);
        ...
        <>
          <div className="sectionHead">
            <div>
              ...
              <p>Model-qualified pool for {data.today}. Handpicked plays are marked with a badge on their normal Best Play tile.</p>
            </div>
          </div>
          ...
          {bestPlays.length ? (
            <div className="cards">
              {bestPlays.map((play, index) => {
                const handpicked = isHandpickedPlay(play, data.betTrackerRows, data.today);
                return (
                  <BestPlayCard
  ...
  }, [active, bestPlays, handpickedLast7, handpickedOverall, data, error]);
      ...
      {data ? (
        <section className="tileGrid">
          ...
          <Tile
            ...
            value={String(todayHandpickedCount)}
            meta="Marked TRUE today"
            green={todayHandpickedCount > 0}
          />
          <Tile
