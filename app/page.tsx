...
export default function Home() {
  ...
  const todayHandpickedCount = useMemo(() => countTodayHandpickedRows(data?.betTrackerRows, data?.today), [data]);

  const handpickedLast7 = useMemo(() => calculateFavoriteRecord(data?.betTrackerRows, "last7", data?.today), [data]);
  const handpickedOverall = useMemo(() => calculateFavoriteRecord(data?.betTrackerRows, "all", data?.today), [data]);
      ...
      {data ? (
        <section className="tileGrid">
          ...
          <Tile
            ...
            meta="Badge count from TRUE rows"
