...
function passesColdFilter(play: Play, recentSummary: Summary | null) {
  const form = getFormInfo(recentSummary);
  ...
  if (getRankScore(play, recentSummary) >= 78) return true;
  ...
  const bestPlays = useMemo(() => {
    ...
    return data.bestPlays
      .filter((play) => {
        ...
        return isBestPlay(play) && passesColdFilter(play, summary);
      })
      .sort((a, b) => {
