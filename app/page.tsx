...
function bestPlayIdentity(play: Play) {
  ...
  // The model publishes only one qualified game-total side per matchup. Do not
  // include line or odds in the identity: one source may store the line in
  // `play`, another in the slate row, and another in `Odds/Line`, which caused
  // the same total to be displayed more than once.
  if (isTotalType(type)) return `TOTAL|${game}|${type}`;
