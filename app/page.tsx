...
export default function Home() {
  ...
  const bestPlays = useMemo(() => {
    ...
    const recentByType = new Map(
      data.last7RecordSummary.map((row) => [normalizeType(row.betType), row]),
    );
    ...
    return data.bestPlays
      .filter((play) => {
        ...
        if (!passesColdFilter(play, recentSummary, data.slateToday))
          return false;
        if (!passesOverallRecordFilter(play, data.recordSummary, recentSummary))
          return false;
        return true;
      })
      .sort((a, b) => {
  ...
  const visibleLast7RecordSummary = useMemo(
    () => publicRecordRows(data?.last7RecordSummary || []),
    [data?.last7RecordSummary],
  );
  const visibleOverallRecordSummary = useMemo(
    () => publicRecordRows(data?.recordSummary || []),
    [data?.recordSummary],
  );
  const visibleHandpickedLast7RecordSummary = useMemo(
    () => publicRecordRows(data?.handpickedLast7RecordSummary || []),
    [data?.handpickedLast7RecordSummary],
  );
  const visibleHandpickedOverallRecordSummary = useMemo(
    () => publicRecordRows(data?.handpickedRecordSummary || []),
    [data?.handpickedRecordSummary],
  );
