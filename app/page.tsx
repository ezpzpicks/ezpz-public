...
function aiPitcherGateInfo(summary: Summary | null): AiPitcherGateInfo {
...
function trendSignalMetrics(signal: TrendSignalBreakdown): TrendScoreMetrics {
  ...
  // Match route.ts exactly: trend history is split evenly between actual
  // win-record performance and ROI. Recency is handled by the 25/25/50
  // all-time / last-30 / last-7 window weights above.
  const score =
        ...
        return (
          <section className="trendSignalPanel" key={`${signal.signalType}-${signal.signalKey}`}>
            ...
            <div className="trendWeightLine">
              Exact category: {signal.exactSample} bets • No sample penalty • Record 50% • ROI 50% • Windows 25% overall / 25% last 30 / 50% last 7
            </div>
          </section>
        );
      })}
...
function AiPickSelectorCard({
  ...
  const pitcherGate =
    pick.market === "Pitcher Strikeouts"
      ? aiPitcherGateInfo(lastSevenBetsSummary)
      : null;
  const isFinalReview =
  ...
  return (
    <details className="aiPickDropdown">
      ...
      <div className="aiPickExpanded">
        ...
        {pitcherGate ? (
          <section className={`aiPickPitcherGate ${pitcherGate.className}`}>
            <div className="aiPickPitcherGateHead">
              <div>
                <span>Pitcher Prop Qualification</span>
                <strong>
                  {normalizeType(pick.bestPlayType || "Pitcher Strikeouts")}
                </strong>
              </div>
              <span className={`formPill ${pitcherGate.className}`}>
                {pitcherGate.className === "hot"
                  ? "🔥"
                  : pitcherGate.className === "cold"
                    ? "❄️"
                    : pitcherGate.className === "sample"
                      ? "⚠️"
                      : "➖"}{" "}
                Last 7 Bets: {pitcherGate.label}
              </span>
            </div>
            <div className="aiPickGateGrid">
              ...
              <div className="aiPickGateMetric">
                ...
                <strong>{pitcherGate.score == null ? "Excluded" : `${pitcherGate.score}+`}</strong>
              </div>
              <div className="aiPickGateMetric">
                <span>Required Probability</span>
                <strong>
                  {pitcherGate.probability == null
                    ? "Excluded"
                    : `${pitcherGate.probability}%+`}
                </strong>
              </div>
              <div className="aiPickGateMetric">
                <span>Required Advantage</span>
                <strong>
                  {pitcherGate.advantage == null
                    ? "Excluded"
                    : `+${pitcherGate.advantage}%`}
                </strong>
              </div>
            </div>
          </section>
...
function trendTierFromTrackerRow(row: SheetRow): "Elite" | "Strong" | "Good" | "" {
  const tier = normalizeType(row["Trend Tier"] || "");
...
function calculateModelTrendCombinationRecords(
  ...
  rows.forEach((row) => {
    ...
    const tier = trendTierFromTrackerRow(row);
    const trendQualified = truthySheetFlag(row["Trend Play"]) || Boolean(tier);
    if (!trendQualified || !tier) return;
...
function CombinationRecords({
  ...
  const summaries = useMemo(
    () => calculateModelTrendCombinationRecords(rows, period, today),
    [period, rows, today],
  );
  const matchedPlays =
    ...
    <main className="shell">
      ...
      <style jsx global>{`
        ...
        * {
        ...
        .aiPickPitcherGateHead > div {
          display: grid;
          gap: 4px;
        }

        .aiPickPitcherGateHead > div > span {
          color: var(--ez-muted);
          font-size: 9px;
          font-weight: 900;
          letter-spacing: 0.08em;
        ...
        .aiPickPitcherGateHead > div > strong {
          font-size: 14px;
        }
