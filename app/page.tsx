...
function isRealPct(value: string) {
  return value !== "—" && /\d/.test(value);
}

function getMoneylineModelPct(play: Play, slateRows: SheetRow[]) {
  const direct = formatModelPct(play.moneylinePct);
  if (isRealPct(direct)) return direct;

  // Restore the older behavior where the % could be embedded in the team/play text.
  const embedded = formatModelPct(`${play.playerTeam || ""} ${play.play || ""}`);
  if (isRealPct(embedded)) return embedded;
  ...
  for (const key of directKeys) {
    const pct = formatModelPct(row[key]);
    if (isRealPct(pct)) return pct;
  }

  // Future-proof fallback: scan for likely moneyline/model win percentage columns.
  for (const [key, rawValue] of Object.entries(row)) {
    const lowerKey = key.toLowerCase();
    const looksLikeMoneylinePct =
      (lowerKey.includes("model") || lowerKey.includes("moneyline") || lowerKey.includes("ml") || lowerKey.includes("win")) &&
      (lowerKey.includes("%") || lowerKey.includes("pct") || lowerKey.includes("prob"));
    ...
    if (!looksLikeMoneylinePct || shouldIgnore) continue;

    const pct = formatModelPct(rawValue);
    if (isRealPct(pct)) return pct;
  }
...
function BestPlayCard({ play, index, recentSummary, slateRows }: { play: Play; index: number; recentSummary: Summary | null; slateRows: SheetRow[] }) {
  ...
  const modelPct = getMoneylineModelPct(play, slateRows);
