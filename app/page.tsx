...
function parseAmericanOdds(value: unknown) {
  ...
  // Some sheet cells look like "Line 5.5 -120". Grab the odds number, not the line.
  const signedMatches = [...raw.matchAll(/[+-]\d{3,4}/g)].map((match) => Number(match[0]));
  const signedOdds = signedMatches.find((odds) => Number.isFinite(odds) && Math.abs(odds) >= 100);
  if (signedOdds) return signedOdds;

  const allMatches = [...raw.matchAll(/\b\d{3,4}\b/g)].map((match) => Number(match[0]));
  const plainOdds = allMatches.find((odds) => Number.isFinite(odds) && odds >= 100);
  return plainOdds || 0;
}
...
function getPlayableOdds(play: Play, slateRows: SheetRow[] = []) {
  ...
  // Final fallback: use any non-moneyline odds on the matching row.
  for (const [key, value] of Object.entries(row)) {
    const keyLower = key.toLowerCase();
    if (["moneyline", "ml", "nrfi", "yrfi"].some((bad) => keyLower.includes(bad))) continue;
    const odds = formatOdds(value);
    if (odds !== "—" && parseAmericanOdds(odds)) return odds;
  }
