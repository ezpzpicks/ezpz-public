...
type Tab = "Today’s Best Plays" | "Handpicked" | "Full Slate" | "Records";

const TABS: Tab[] = ["Today’s Best Plays", "Handpicked", "Full Slate", "Records"];
...
export default function Home() {
  ...
  const content = useMemo(() => {
        ...
        <>
          <div className="sectionHead">
            <div>
              ...
              <p>Model-qualified pool for {data.today}. Last updated: {data.lastUpdated}</p>
            </div>
          </div>
    ...
    if (active === "Handpicked") {
      return (
        <>
          <div className="sectionHead">
            <div>
              <h2>Handpicked Records</h2>
              <p>Records are tracked from rows marked TRUE in the Favorite Pick column. No separate handpicked play tiles are shown.</p>
            </div>
          </div>
  ...
  }, [active, bestPlays, handpickedPlays, handpickedLast7, handpickedOverall, data, error]);
      ...
      {data ? (
        <section className="tileGrid">
          ...
          <Tile
            ...
            value={String(handpickedPlays.length)}
            meta="Your conviction plays"
            green={handpickedPlays.length > 0}
          />
          <Tile
