...
anchor = '''  const splits = Array.isArray(legacy.draftKings?.splits) ? legacy.draftKings.splits : [];
'''
insert = '''  const qualifiedPropTrackerForRecords = propTracker.filter((row) => qualifiedNflPropGrade(row.Grade));
...
if "const qualifiedPropTrackerForRecords" not in text:
    if anchor not in text:
        raise SystemExit("Could not find NFL public-record insertion point")
    text = text.replace(anchor, anchor + insert, 1)
    print("Added combined NFL Best Plays record calculations")
else:
    print("Combined NFL Best Plays record calculations already present")
...
'''
if new_tiles not in text:
    if old_tiles not in text:
        raise SystemExit("Could not find NFL public-record return block")
    text = text.replace(old_tiles, new_tiles, 1)
    print("Connected NFL public tiles and record summaries to game + prop trackers")
else:
    print("NFL public tiles already use combined records")
