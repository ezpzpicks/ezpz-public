...
function patchPage() {
  ...
  // This is a migration aid, not a formatter. If the page has evolved beyond
  // the historical placeholder block, leave its current football tile layout alone.
  if (text.includes(oldTiles) && !text.includes(newTiles)) {
    text = text.replace(oldTiles, newTiles);
  }
