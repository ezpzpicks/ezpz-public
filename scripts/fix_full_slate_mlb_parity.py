...
if "footballSlateDropdown" in source:
    print("Full Slate parity patch is already present.")
    raise SystemExit(0)

slate_rows_start = source.index("  const slateRows = useMemo(() => {")
slate_rows_end = source.index("\n\n  const summaryMap", slate_rows_start)
slate_rows = r'''  const slateRows = useMemo(() => {
    const rows: SheetRow[] = [];
...
source = source[:slate_rows_start] + slate_rows + source[slate_rows_end:]

card_start = source.index("function SlateCard({ row, splits }")
card_end = source.index("\n\nexport default function FootballBoard", card_start)
slate_card = r'''function SlateCard({ row, splits }: { row: SheetRow; splits: DraftKingsSplit[] }) {
...
source = source[:card_start] + slate_card + source[card_end:]

full_start = source.index('  } else if (tab === "Full Slate") {')
full_end = source.index("\n  } else {\n    const trackerRows", full_start)
full_slate = r'''  } else if (tab === "Full Slate") {
...
source = source[:full_start] + full_slate + source[full_end:]

css_marker = "        /* MLB visual system alignment for NFL + College Football */"
css_index = source.index(css_marker)
slate_css = r'''        /* MLB Full Slate parity for NFL + College Football */
...
'''
source = source[:css_index] + slate_css + source[css_index:]

path.write_text(source)
print("Patched football Full Slate: robust matchup dedupe + MLB-style dropdown/cards.")
